...
"%</a> ");e='<span class="date" style="background-color: #EEE;"></span>';if(!a.disable&&"General"!=g&&void 0!=a.details.plannedCompletionDate)if(d=new Date(a.details.plannedCompletionDate.replace(/:\d\d\d/,"")),isNaN(d.getTime()))b.attr("data-days",9999);else{var h=new Date;h.setHours(0);h.setMinutes(0);h.setSeconds(0);e="#D2E0FF";h=Math.round((d-h)/1E3/60/60/24);0>h?e="#FFCDCD":1>=h&&(e="#FFD7AB");var k;k=0<h?"days due: "+h:"days late: "+Math.abs(h);e='<div class="date" style="background-color: '+
e+';"><div class="days">'+k+"</div><div>"+ka(d.getMonth())+" "+d.getDate()+", "+d.getFullYear()+"</div></div>";b.attr("data-days",h)}else b.attr("data-days",9999);d="";null!=a.details.parent&&(d="<br><br>Parent Task: <br>"+a.details.parent.name);c.find(".name").attr("title",g+": <br>"+a.name+d);"General"!=g?(g=a.details.project.name,b.html(g).attr("title","Project: <br>"+g).attr("data-priority",a.details.priority),c.find(".c6").after(e)):b.html(a.details.description).attr("title",a.details.description)}});
