(function (c) {
    ...
    function f(d, c) {
        if ("function" !== typeof d)throw Error("listener is a required param of {fn}() and should be a Function.".replace("{fn}", c));
    }
    ...
    function m() {
        ...
        this.dispatch = function () {
            m.prototype.dispatch.apply(d, arguments)
        }
    }

    k.prototype = {active: !0, params: null, execute: function (d) {
        var c;
        this.active && this._listener && (d = this.params ? this.params.concat(d) :
            d, c = this._listener.apply(this.context, d), this._isOnce && this.detach());
        return c
    }, detach: function () {
    ...
    m.prototype = {VERSION: "1.0.0", memorize: !1, _shouldPropagate: !0, active: !0, _registerListener: function (d, c, f, m) {
        var C = this._indexOfListener(d, f);
        if (-1 !== C) {
            if (d = this._bindings[C], d.isOnce() !== c)throw Error("You cannot add" + (c ? "" : "Once") + "() then add" + (!c ? "" : "Once") + "() the same listener without removing the relationship first.");
        } else d = new k(this, d, c, f, m), this._addBinding(d);
        this.memorize && this._prevParams && d.execute(this._prevParams);
        return d
    },
        _addBinding: function (d) {
            var c = this._bindings.length;
            do--c; while (this._bindings[c] && d._priority <= this._bindings[c]._priority);
            this._bindings.splice(c + 1, 0, d)
        }, _indexOfListener: function (d, c) {
            for (var k = this._bindings.length, f; k--;)if (f = this._bindings[k], f._listener === d && f.context === c)return k;
            return-1
        }, has: function (d, c) {
            return-1 !== this._indexOfListener(d, c)
        }, add: function (d, c, k) {
            f(d, "add");
            return this._registerListener(d, !1, c, k)
        }, addOnce: function (d, c, k) {
            f(d, "addOnce");
            return this._registerListener(d, !0,
                c, k)
        }, remove: function (d, c) {
            f(d, "remove");
            var k = this._indexOfListener(d, c);
            -1 !== k && (this._bindings[k]._destroy(), this._bindings.splice(k, 1));
            return d
        }, removeAll: function () {
            for (var d = this._bindings.length; d--;)this._bindings[d]._destroy();
            ...
            if (this.active) {
                var c = Array.prototype.slice.call(arguments), k = this._bindings.length, f;
                this.memorize && (this._prevParams =
                    c);
                if (k) {
                    f = this._bindings.slice();
                    this._shouldPropagate = !0;
                    do k--; while (f[k] && this._shouldPropagate && !1 !== f[k].execute(c))
                }
            }
        }, forget: function () {
            this._prevParams = null
    ...
    "function" === typeof define && define.amd ? define(function () {
        return d
    }) : "undefined" !== typeof module && module.exports ? module.exports = d : c.signals =
        d
})(this);
...
(function () {
    var c = this.Store = function (c, f) {
        var m;
        this.name = c;
        if (void 0 !== f)for (m in f)f.hasOwnProperty(m) && void 0 === this.get(m) && this.set(m, f[m])
    };
    ...
    c.prototype.set = function (c, f) {
        if (void 0 === f)this.remove(c); else {
            if ("function" === typeof f)f = null; else try {
                f = JSON.stringify(f)
            } catch (m) {
                f = null
            }
    ...
    c.prototype.removeAll = function () {
        var c, f;
        c = "store." + this.name + ".";
        for (f = localStorage.length - 1; 0 <= f; f--)localStorage.key(f).substring(0, c.length) === c && localStorage.removeItem(localStorage.key(f));
        return this
    };
    c.prototype.toObject = function () {
        ...
        c = {};
        f = "store." + this.name + ".";
        for (m = localStorage.length - 1; 0 <= m; m--)localStorage.key(m).substring(0, f.length) === f && (d = localStorage.key(m).substring(f.length),
            s = this.get(d), void 0 !== s && (c[d] = s));
        return c
    };
    c.prototype.fromObject = function (c, f) {
        !0 !== f && this.removeAll();
...
var BrowserDetect = {init: function () {
    ...
    for (var k = 0; k < c.length; k++) {
        var f = c[k].string, m = c[k].prop;
        this.versionSearchString = c[k].versionSearch || c[k].identity;
        if (f) {
            if (-1 != f.indexOf(c[k].subString))return c[k].identity
...
var attask = function () {
    function c(a) {
        g.set("general", !1);
        var b = g.get("host").replace(/[|&;$%@"<>()+,]/g, ""), e = new Date;
        e.setHours(e.getHours() - e.getTimezoneOffset() / 60);
        var e = e.toJSON(), h = function (a) {
            ...
            data: JSON.stringify({dateTime: e, activity: {request: b}, userID: g.get("userID"), username: g.get("username")}), dataType: "json", type: "POST"}, b = jQuery.ajax(b);
        b.done(function (b, e, d) {
            if (b.isValid) {
                try {
                    b.message = JSON.parse(b.message), b.isWarn && (console.log(b.message.message), b.message.message && h(b.message.message))
                } catch (c) {
                    void 0 == g.get("access") ? U(g.get("username"), g.get("password"), V(g.get("host") + "." + g.get("domain")), a, function () {
                        W(a)
                    }, !0) : (!0 != g.get("is_auth") && (g.set("access", "demo"), g.set("is_auth",
                        !1)), a && a());
                    console.log(b.message);
                    return
                }
                g.set("ent", !0);
                g.set("updateAtInterval", void 0 != b.message.updateAtInterval ? b.message.updateAtInterval : !1);
                ...
                g.set("projectFilter",
                    void 0 != b.message.projectFilter ? b.message.projectFilter : !0);
                g.set("personalTasks", void 0 != b.message.personalTasks ? b.message.personalTasks : !0);
                g.get("hideTaskButtons") && g.set("sort", "project");
                jQuery("#licence").text("Enterprise");
                var f = function () {
                    g.set("is_auth", !0);
                    g.set("auth_key", !0);
                    b.message.apiKey && (g.set("password", null), g.set("apiKey", b.message.apiKey));
                    a && a();
                    jQuery("#top-bar").show();
                    fa()
                };
                (void 0 != b.message.forceLogin ? b.message.forceLogin : 1) ? void 0 == g.get("sessionID") ? I(f) : f() : !g.get("userID") &&
                    g.get("username") ? (e = {url: t() + "user/search", type: "get", data: {fields: "ID", emailAddr: g.get("username"), apiKey: b.message.apiKey}, dataType: "json"}, jQuery.ajax(e).done(function (b) {
                    void 0 != b.error ? ($error.html(b.error.message), q.hide(), void 0 !== a && I(f)) : (b = b.data, b.length && g.get("password") == b[0].ID.slice(-6) ? (g.set("userID", b[0].ID), f()) : I(f))
                })) : f()
            } else!0 != g.get("is_auth") && (g.set("access", "demo"), g.set("is_auth", !1), console.log(b.message), b.message && h(b.message)), a && a()
        });
        b.fail(function (b, e, h) {
            a &&
            a()
        })
    }
    ...
    function k(a) {
        ...
        if (g.get("ent"))c(a); else {
            var b = -1 == window.location.origin.indexOf("atappstore.com") ? "https://" + ga : window.location.origin, e = g.get("qr") ? "shiftwatch-qr" : "stopwatch", b = {url: b + "/api/authorize/", data: {product: e, host: g.get("host"), sessionID: g.get("sessionID"), user: g.get("username"), domain: g.get("domain")}, type: "GET", dataType: "json"};
            w(b);
            b = jQuery.ajax(b);
            b.done(function (b, e, T) {
                !1 != b.success && (g.set("auth_key", b.auth_key), g.set("access",
                    b.access), g.set("is_auth", b.is_auth), g.set("license_expiry_days", b.license_expiry_days), g.get("ent") ? jQuery("#licence").text("Enterprise") : g.get("license_expiry_days") ? jQuery("#licence").text("Expires in " + g.get("license_expiry_days") + " day(s)").css("color", "red") : Aa(), g.get("qr") && (g.set("view", "team"), g.set("sort", "project"), g.set("logout_on_select", !0), g.set("multi", !0), g.set("hide_team_done", !0), g.set("minuteBuffer", 30), g.set("minuteBufferSnapToSched", !0), TASK_DB.userId = g.get("userID")), g.get("minuteBufferSnapToSched") &&
                    ha(), g.get("restrict_sched") ? ha(function (b) {
                    var e;
                    void 0 == b && (e = "Today is not a work day");
                    var h = new Date, h = Date.parse("01/01/2011 " + h.getHours() + ":" + h.getMinutes()), l = Date.parse("01/01/2011 " + b.start), T = Date.parse("01/01/2011 " + b.stop), d = g.get("minuteBuffer");
                    null != d && (l.setMinutes(l.getMinutes() - d), T.setMinutes(T.getMinutes() + d));
                    void 0 == b || l > h || h > T ? (e = e ? e : "Please return at " + b.start, TASK_CNTRL.showWindow('<div style="text-align: center; font-weight: bold;"><h3>Not scheduled to work</h3></div><div class="content"><p>' +
                        e + '.</p><a href="#" class="btn btn-primary logout cancel" title="Logout">Logout</a></div>')) : a()
                }) : ia(a, function () {
                    Aa()
                }))
            });
            b.fail(function (a, b, e) {
            })
        }
    }
    ...
    function f() {
        ...
        a.done(function (a) {
            ...
            }) : d(u[0], "uCount," + JSON.stringify(a.error)), n.hide(), q.hide()) : (a = a.data, d(u[1], {uc: a.count}))
        });
        a.fail(function (a, e, h) {
            x({response: a.responseText, type: e, msg: h}, f, "uCount")
        })
    }

    function m() {
        var a =
        {url: t() + "user/search?ID=$$USER.ID&fields=customer:name,customer:fullUsers,customer:limitedUsers,customer:teamUsers", type: "get", data: {}, dataType: "json"};
        w(a);
        a = jQuery.ajax(a);
        a.done(function (a) {
            void 0 != a.error ? (a.error["class"] == y[0] ? v(function () {
            }) : d(u[0], "uInfo," + JSON.stringify(a.error)), n.hide(), q.hide()) : (a = a.data, d(u[1], a))
        });
        a.fail(function (a, e, h) {
            x({response: a.responseText, type: e, msg: h}, m, "uInfo")
        })
    }
    ...
    function s() {
        if (J || E || void 0 != g.get("host") &&
            "full" == getQueryString("view"))window.clearInterval(P), P = window.setInterval(function () {
            if (navigator.onLine && (E || !document.hasFocus()))jQuery("#search-bar").is(":visible") && jQuery("#search").click(), jQuery("#form-list").hide(), !g.get("apiKey") && !g.get("sessionID") ? K() : X()
        }, 1E3 * (Ba + 120))
    }

    function p(a, b) {
        var e;
        return(e = g.get("maxTaskTime")) && void 0 != e && e <= Number((b / 3600).toFixed(2)) ? (a = TASK_DB.get(a.ID), a.disable = 1, TASK_DB.setTask(a), TASK_CNTRL.toggleTimer(a), e = jQuery("#item" + a.ID), e.find(".timer").unbind("click").addClass("disabled"),
            e.find(".timer-info").text("Max Reached"), !0) : !1
    }
    ...
    function C() {
        ...
        var a = new Date;
        jQuery("#todayDate").html("Date: " + ja(a.getMonth()) + " " + a.getDate());
        z();
    ...
    function Y() {
        var a = g.get("view");
        void 0 == a ? Ea() : jQuery("#button-" + a).click()
    }
    ...
    function Aa() {
        if ("" == g.get("access") && !g.get("notified")) {
            ...
            jQuery("#licence").text("Expired").css("color", "red");
            d(u[1], {os: BrowserDetect.OS, browser: BrowserDetect.browser, version: BrowserDetect.version, extension: !J, user: g.get("userObj"), license_expiry_days: g.get("license_expiry_days")});
            f();
            m()
        } else g.get("license_expiry_days") && jQuery("#licence").text("Expires in " + g.get("license_expiry_days") + " day(s)").css("color", "red")
    }
    ...
    function Ma() {
        ...
        var a = new Date, b = '<div id="top-menu"><div class="btn-group"><a id="filter" title="Filter for other assigned tasks and issues" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">Filters<span class="caret"></span></a><ul class="dropdown-menu">',
            e;
        for (e in L)L.hasOwnProperty(e) && (b += '<li id="filter-' + e + '"><a href="#" id="button-' + e + '" class="filter" title="' + L[e].d + '" >' + L[e].t + "</a></li>");
        b += '<div id="custom-filters"><div id="task-filters" style="display:none;"></div><div id="opTask-filters" style="display:none;"></div></div><li><a href="#" class="logout" id="button-logout" >Login as New User</a></li></ul></div><span id="totalHours" title="Go To Timesheets" class="btn" style="">Today\'s Hours: 0</span><span id="todayDate" class="btn disabled" style="">Date: ' +
            ja(a.getMonth()) + " " + a.getDate() + "</span></div>";
        jQuery("#bar").html(b);
        Fa(L);
        ...
        var h = function (a, b) {
            for (var e in B)B.hasOwnProperty(e) && e != b && (B[e] = "desc");
        ...
        jQuery(document).on("click",
            ".done", function (a) {
                a.preventDefault();
                a = jQuery(this);
                var b = a.parent().parent(), e = b.children("span.name").attr("title"), b = b.children("span.details").attr("title");
                a = '<div style="text-align: center; font-weight: bold;"><h3>Confirm</h3></div><div class="content"><div>Are you sure you completed <strong>' + e + "</strong><br>from <br><strong>" + b + '?</strong><div><br><a href="#" id="done_confirm" class="btn btn-primary" rel="' + a.attr("rel") + '" title="Done">Done</a><a href="#" class="btn cancel" title="Cancel">Cancel</a><div>';
                TASK_CNTRL.showWindow(a)
            });
        ...
        jQuery(document).on("click", ".task-lower-block", e);
        ...
        jQuery(document).on("click", ".percent", function (a) {
            ...
            if (0 == a.children("input").length) {
                var e = a.text().slice(0, -1);
                a.html("<input type='number' class='inputbox percent' value=\"" +
                    e + '">');
                var h = function (a) {
                    a = jQuery(a);
                    var b = Number(a.val()).toFixed(0);
                    isNaN(b) || 0 > b || 100 < b || e == b ? b = e : TASK_DB.updateValidate(a.parent().attr("rel"), function (a) {
                        aa(a, b)
                    });
                    a.parent().text(b + "%")
                };
        ...
        jQuery(document).on("click", "#comment_confirm", function (a) {
            a.preventDefault();
            a = jQuery(this);
            var b = jQuery("#comment_is_private").is(":checked"), e = jQuery("#comment_message").val();
        ...
        q = jQuery(".spinner-bottom");
        n = jQuery(".spinner")
    }

    function Ga(a, b) {
        var e = jQuery("#" + a + "-filters").first(),
            h = "", l;
    ...
    function Fa(a) {
        var b = jQuery(".filter"), e = jQuery("#active-filter"), h;
        for (h in a)a.hasOwnProperty(h) && (jQuery("#active-filter").hide(), jQuery("#button-" + h).attr("name", h), jQuery(document).on("click", "#button-" + h, function () {
            var h = jQuery(this), d = h.attr("name");
            B = [];
            b.removeClass("btn");
            Ha();
            e.html("Filter: " + h.text());
    ...
    function D(a) {
        TASK_DB.loadAll(function (b) {
            var e =
                b.length, h, l;
            if (0 < e)for (h = 0; h < e; h++)l = b[h], null != l && "GEN" != l.details.objCode && jQuery("#item" + l.ID).hide();
            a()
        })
    }

    function v(a) {
        g.get("ent") ? ia(a) : U(g.get("username"), g.get("password"), V(g.get("host") + "." + g.get("domain")), a, function () {
            W(a)
        })
    }
    ...
    function Pa(a) {
        ...
        jQuery("#button-qrlogin").unbind("click").click(function (b) {
            ...
            b.preventDefault();
            q.show();
            U(g.get("username"), g.get("password"), V(g.get("host")), function () {
                ignoreForQR();
                jQuery("#bar").show();
                E ? h.hide() : h.slideUp();
                jQuery("#top-bar").show();
    ...
    function I(a) {
        ...
        jQuery("#bar").hide();
        jQuery(".form").hide();
        var b = jQuery("#form-login"), e = g.get("domain"), h = g.get("host");
        b.find(":input[name='username']").focus().val("");
        b.find(":input[name='host']").val(void 0 == h || "undefined" == h ? "" : "my.workfront.com" != e ? h + "." + e : h);
        ...
        jQuery("#button-login").unbind("click").click(function (e) {
            e.preventDefault();
            q.show();
            U(jQuery.trim(b.find(":input[name='username']").val()), jQuery.trim(b.find(":input[name='password']").val()), V(jQuery.trim(b.find(":input[name='host']").val())), function () {
                jQuery("#bar").show();
                E ? b.hide() : b.slideUp();
                jQuery("#top-bar").show();
                jQuery("#loading").show();
    ...
    function N(a, b, e) {
        ...
        a.duration = b;
        navigator.onLine ? ca(a, Qa, e) : (b = g.get("offlineTasks"), void 0 == b && (b = []), b.push(a), g.set("offlineTasks",
            b), n.hide(), q.hide(), !0 == g.get("logout_on_select") && K())
    }

    function la(a) {
        ca(a, function (a, e) {
            var h = new Date, l = {entryDate: h.getFullYear() + "-" + (h.getMonth() + 1) + "-" + h.getDate(), ownerID: g.get("userID")};
            l.extRefID = e && e.extRefID ? "ON!" + h.getHours() + ":" + h.getMinutes() + ";" + e.extRefID : "ON~" + h.getHours() + ":" + h.getMinutes() + ".";
            l.extRefID = l.extRefID.substring(0, 254);
            void 0 == e || -1 != e.permissions.actions.indexOf("ADD_HOURS") && -1 != e.permissions.actions.indexOf("CHANGE_STATUS") ? (l.hours = 0.01, l.method = "post") : (l.method =
                "put", l.ID = e.ID);
            ...
            h = jQuery.ajax(h);
            h.done(function (e) {
                void 0 != e.error ? e.error["class"] == y[0] ? v(function () {
                    setHourAttask()
                }) : x({response: e.error, type: null, msg: null}, function () {
                    la(a)
                }, "logStart," + a.syncID + " " + a.name + " url: " + t() + " " + JSON.stringify(l) + " " + G(a)) : (void 0 != l.taskID || void 0 != l.opTaskID) && Q(a)
            });
            h.fail(function (e, h, d) {
                x({response: e.responseText, type: h, msg: d}, function () {
                    la(a)
                }, "logStart-fail," + a.syncID + " " + a.name + " url: " + t() + " " + JSON.stringify(l) + " " + G(a) + "," + a.duration + ", " + e.responseText + ", " + d, e.responseText)
            })
        })
    }
    ...
    function w(a, b) {
        void 0 == b && (b = 1E4);
        a.timeout = b;
        J && !Ia && (a.dataType = "jsonp", a.crossDomain = !0, a.jsonp = "jsonp", a.cache = !0);
        var e = g.get("sessionID"), h = g.get("apiKey");
        h ? ("PUT" == a.data.method && (a.type = "POST"), a.data.apiKey = h) : a.data.sessionID = e
    }
    ...
    function U(a, b, e, h, l, d) {
        if (void 0 == a || void 0 == b || void 0 == e)l(); else {
            var f = jQuery(".error");
            f.html("");
            !d && 6 == b.length && "" != e ? (g.set("username", a), g.set("password", b), d = e.split("."), g.set("host", d[0]), d.shift(), g.set("domain", d.join(".")), c(function () {
                q.hide();
                F();
                s();
                void 0 != h && h()
            })) : (d = {url: "https://" + e + "/attask/api/v" + (-1 != location.origin.indexOf("localhost") ? 2 : 5) + ".0/login", type: "post", data: {username: a, password: b, method: "post"}, dataType: "json"}, w(d), M ? (d = g.get(a), void 0 != d ? (Ja(d.sessionID, d.userID, d.username,
                d.password, e), q.hide(), F(), s(), void 0 != h && h()) : f.html("OFFLINE")) : (d = jQuery.ajax(d), d.done(function (d) {
                void 0 != d.error ? (g.get("qr") ? f.html("Please Scan Again.") : f.html(d.error.message), q.hide(), void 0 !== l && l()) : (d = d.data, Ja(d.sessionID, d.userID, a, b, e), g.set("userObj", JSON.stringify(d)), q.hide(), k(function () {
                    g.get("ent") || F();
                    s();
                    void 0 != h && h()
                }))
            }), d.fail(function (a, b, h) {
                a.responseText && "" !== a.responseText ? -1 != a.responseText.indexOf("<html>") ? g.get("qr") ? f.html("Please Scan Again.") : f.html("Host Invalid: " +
                    e) : g.get("qr") ? f.html("Please Scan Again.") : f.html(JSON.parse(a.responseText).error.message.replace("Authentication Exception: Authentication Exception: {0}", "Wrong login, please try again.")) : g.get("qr") ? f.html("Please Scan Again.") : f.html("Host Invalid: " + e);
                q.hide();
                void 0 !== l && l()
            })))
        }
    ...
    function Ja(a, b, e, h, l) {
        ...
        l.shift();
        g.set("domain", l.join("."));
        g.get("allowOffline") && g.set(e, {sessionID: a,
            userID: b, username: e, password: h})
    }

    function X(a, b) {
        var e = g.get("sessionID");
        void 0 == e || "" == e ? v(a) : (n.show(), e = {url: t() + "user/" + g.get("userID"), data: {method: "put", updates: "{isActive: true}"}, type: "get", dataType: "json"}, w(e), e = jQuery.ajax(e), e.done(function (e) {
            void 0 != e.error ? (e.error["class"] == y[0] ? void 0 != b ? b(a) : v(a) : d(u[0], "sessionValid," + g.get("sessionID") + "," + JSON.stringify(e.error)), n.hide(), q.hide()) : (n.hide(), q.hide(), void 0 == g.get("auth_key") ? k(F) : F(), void 0 != a && a())
        }), e.fail(function (e, l, c) {
            401 ==
                e.status ? void 0 != b ? b(a) : v(a) : "timeout" == l ? X(a) : d(u[0], "sessionValid," + g.get("sessionID") + "," + e.responseText)
        }))
    }
    ...
    function F() {
        ...
        M ? g.get("allowOffline") ? (a = TASK_DB.userRunningTasks(a), void 0 != a && (A = !0, TASK_DB.insert(a.name, {syncID: a.syncID, details: a.details}, !0))) : TASK_CNTRL.displayError("OFFLINE") : (a = jQuery.ajax(b), a.done(function (a) {
            if (void 0 != a.error)a.error["class"] == y[0] ? v(function () {
                F()
            }) : d(u[0], "addGeneralTasks-done," + JSON.stringify(a.error)); else {
                a = a.data;
                if (0 != a.length) {
                    ...
                    if (null == a[0].extRefID)R(a[0].ID, b, 0); else if (g.get("updateAtInterval")) {
                        var l =
                            0, c = 1;
                        if (isNaN(parseInt(a[0].extRefID)))var f = JSON.parse(a[0].extRefID), l = f.last_launched, c = parseInt(f.count); else l = a[0].extRefID;
    ...
    function R(a, b, e, h) {
        var l = {url: t() + "timeSheet/", type: "get", data: {ID: a, extRefID: JSON.stringify({last_launched: b.getTime(), count: ++e}), method: "put"}, dataType: "json"};
        w(l);
        var c = jQuery.ajax(l);
        c.done(function (l) {
            void 0 != l.error ? l.error["class"] == y[0] ? v(function () {
                R(a, b, e, h)
            }) : d(u[0], "ownTimesheet," + a + JSON.stringify(l.error)) : void 0 != h && h()
        });
        c.fail(function (d, c, g) {
            x({response: d.responseText, type: c, msg: g, status: d.status}, function () {
                R(a, b, e, h)
            }, "ownTimesheet-fail" +
                a + " " + c + "-" + g + " " + d.responseText + " " + d.status + " " + JSON.stringify(l))
        })
    }
    ...
    function ka() {
        ...
        b.done(function (a) {
            if (void 0 != a.error)a.error["class"] == y[0] ? v(function () {
                ka()
            }) : d(u[0], "getStopwatchHourType-done," + JSON.stringify(a.error)); else if (0 !== a.data.length) {
                var b = {};
                a.data.forEach(function (a, e) {
                    b[a.name] = a.ID
                });
                g.set("sw_hour_types", b)
            }
        });
        b.fail(function (b, h, d) {
            x({response: b.responseText, type: h, msg: d, status: b.status}, ka, "getStopwatchHourType-fail" + h + "-" + d + " " + b.responseText + " " + b.status + " " + JSON.stringify(a))
        })
    }
    ...
    function fa() {
        ...
        jQuery.when(jQuery.ajax(a), jQuery.ajax(b)).done(function (a, b) {
            ...
            void 0 != l ? l["class"] == y[0] ? I(function () {
                fa()
            }) : d(u[0], "addCustomFilters," + JSON.stringify(r.error)) : (Ga("task", La("task", a.data)), Ga("opTask", La("opTask", b.data)), l = jQuery("#opTask-filters"), l.children().length && l.show(), l = jQuery("#task-filters"), l.children().length && l.show(), void 0 == L[g.get("view")] && Y())
        }).fail(function (a, b, d) {
                401 == a.status ? (console.log("auth error sessionID:" + g.get("sessionID") + " apiKey: " + g.get("apiKey")), g.get("apiKey") &&
                    jQuery(".error").html("Please contact your internal stopwatch support, API Key invalid"), I()) : x({response: a.responseText, type: b, msg: d, status: a.status}, fa, "addCustomFilters-fail" + b + "-" + d + " " + a.responseText + " " + a.status)
            })
    }

    function La(a, b) {
        var e = "task" == a ? "task" : "issue", h = {}, d = -1;
        b.sort(function (a, b) {
            return a.name.localeCompare(b.name)
        });
        for (var c in b)if (b.hasOwnProperty(c) && (d = b[c].name.indexOf(" - Stopwatch"), null != b[c].enteredByID && -1 != d)) {
            var f = {}, k, p;
            for (p in b[c].definition)k = b[c].definition[p].replace(/&/g,
                "%26").replace("$$USER.ID", g.get("userID")), -1 == k.indexOf("$$USER") && (f[p] = k.split("\t"));
            h[b[c].ID] = {q: jQuery.param(f).replace(RegExp("%5B%5D", "g"), ""), f: function () {
                return oa(a, this.q)
            }, t: b[c].name.substr(d), d: "Custom " + e + " filter: " + Ta(f)}
        }
        return h
    }
    ...
    function oa(a, b) {
        var e = {};
        a = void 0 == a ? "task" : a;
        e.fields = "parent,extRefID,priority,project,project:hourTypes,project:status,plannedCompletionDate,permissions,canStart,status";
        "task" == a && (e.fields += ",percentComplete");
        n.show();
        var h = {url: t("api/v9.0") + a + "/search?" + b, type: "get", data: e, dataType: "json"};
        w(h);
        h = jQuery.ajax(h);
        h.done(function (b) {
            void 0 != b.error ? (b.error["class"] == y[0] ? v(function () {
                oa(a, e)
            }) : d(u[0], "addFilteredTasks," + JSON.stringify(b.error)), n.hide(), q.hide()) : (b = b.data, 0 == b.length ? (A = !0, D(function () {
                TASK_CNTRL.showAll()
            })) : D(function () {
                var a = b.length;
                A = !0;
                for (var e = 0; e < a; e++) {
                    var h = -1 != ["CPL", "DED", "REJ", "CLS", "RLV"].indexOf(b[e].status) ? 1 : !1 == b[e].canStart ||
                        -1 == b[e].permissions.actions.indexOf("ADD_HOURS") && -1 == b[e].permissions.actions.indexOf("CHANGE_STATUS");
                    TASK_DB.insert(b[e].name, {syncID: b[e].ID, details: b[e], disable: h}, e == a - 1)
                }
            }))
        });
        h.fail(function (a, b, h) {
            x({response: a.responseText, type: b, msg: h}, oa(b, e), "addFilteredTasks")
        })
    }
    ...
    function na() {
        ...
        b.done(function (a) {
            if (void 0 != a.error)a.error["class"] == y[0] ? v(function () {
                na()
            }) : d(u[0], "addAllGeneralTasks-done," + JSON.stringify(a.error)); else {
                a = a.data;
                var b = 0, l;
        ...
        b.fail(function (b, h, d) {
            x({response: b.responseText, type: h, msg: d, status: b.status}, na, "addAllGeneralTasks-fail" + h + "-" + d + " " + b.responseText + " " + b.status + " " + JSON.stringify(a))
        })
    }
    ...
    function x(a, b, e, h) {
        if (401 == a.status)v(function () {
            ...
            if (void 0 != a.response && "" != a.response) {
                ...
                h : ""));
            d(u[0], e + "," + a.msg + " " + a.type + " " + JSON.stringify(a.response) + " " + h)
        }
    }
    ...
    function pa() {
        ...
        w(a);
        var b = jQuery.extend(!0, {}, a), e = g.get("userID");
        a.data["tasks:assignedToID"] = e;
        b.data.ownerID = e;
        jQuery.when(jQuery.ajax(a), jQuery.ajax(b)).done(function (a, b) {
            a = a[0];
            b = b[0];
            var e = void 0 != a.error ? a.error : b.error;
            void 0 != e ? e["class"] == y[0] ? v(function () {
                pa()
            }) : d(u[0], "addCurrentProjects," + JSON.stringify(r.error)) : D(function () {
                function e(a) {
                    for (var b = a.length,
                             h = 0; h < b; h++) {
                        var d = a[h];
                        d.project = jQuery.extend(!0, {}, d);
                        d.type = "Project";
                        TASK_DB.insert(d.name, {syncID: d.ID, details: d}, h == b - 1)
                    }
                }
                ...
                x({response: a.responseText, type: b, msg: e, status: a.status}, pa, "addCurrentProjects-fail" + b + "-" + e + " " + a.responseText + " " + a.status)
            })
    }
    ...
    function S(a, b, e) {
        ...
        h.data.$$LIMIT = 2E3;
        e && (h.data["project:projectedCompletionDate"] = "$$TODAYb-1w", h.data["project:projectedCompletionDate_Mod"] = "gt");
        ...
        h.done(function (h) {
            if (void 0 != h.error)h.error["class"] == y[0] ? v(function () {
                S(a, b, e)
            }) : d(u[0], "addAllAssignedTasks-" + a + "," + JSON.stringify(h.error)), n.hide(),
                q.hide(); else {
                h = h.data;
                var l = !0, c = g.get("view");
                "all" != c && "role" != c && (l = !1);
                var f = g.get("hour_types");
                if ("opTask" != a)S("opTask", b, e), D(function () {
                    var a = h.length;
                    if (a)for (var b = 0; b < a; b++) {
                        var e = h[b].task;
                        if (-1 != e.permissions.actions.indexOf("ADD_HOURS") && -1 != e.permissions.actions.indexOf("CHANGE_STATUS")) {
                            var d = !1, c = e.project.hourTypes.length;
                            if (void 0 != f && c)for (var g = 0; g < c; g++) {
                                if (void 0 != f[e.project.hourTypes[g].ID]) {
                                    d = !0;
                                    break
                                }
                            } else d = !0;
                            d && e.canStart && TASK_DB.insert(e.name, {syncID: e.ID, details: e},
                                l && b == a - 1)
                        }
                    }
                }); else {
                    ...
                    if (c)for (var k = 0; k < c; k++) {
                        ...
                        if (-1 != p.permissions.actions.indexOf("ADD_HOURS") && -1 != p.permissions.actions.indexOf("CHANGE_STATUS")) {
                            var s = !1, m = p.project.hourTypes.length;
                            if (void 0 != f && m)for (var t = 0; t < m; t++) {
                                if (void 0 != f[p.project.hourTypes[t].ID]) {
                                    s = !0;
                                    break
                                }
                            } else s = !0;
                            s && TASK_DB.insert(p.name, {syncID: p.ID, details: p}, l && k == c - 1)
                        }
        ...
        h.fail(function (a, b, e) {
            x({response: a.responseText, type: b, msg: e}, function (a, b, e) {
                S(a,
                    b, e)
            }, "addAllAssignedTasks-fail" + b + "-" + e + " " + a.responseText)
        })
    }
    ...
    function qa(a) {
        ...
        b.done(function (b) {
            if (void 0 != b.error)b.error["class"] == y[0] ? v(function () {
                qa(a)
            }) : d(u[0], "addAllRoleTasks," + JSON.stringify(b.error)), n.hide(), q.hide(); else if (b = b.data[0], 0 == b.length)a && a(), n.hide(), q.hide(); else {
                var h = jQuery("#role-filter"), c = function () {
                    S("task", function () {
                ...
                void 0 != k ? f(k.name, k.ID) : void 0 != b.role && f(b.role.name, b.role.ID);
                if (void 0 != b.roles && 0 != b.roles.length)for (var k = h.find(".dropdown-menu").html("").first(), p = 0, s = b.roles.length; p < s; p++)k.append('<li><a href="#" id="' + b.roles[p].ID + '">' + b.roles[p].name + "</a></li>"), k.find("#" +
                    b.roles[p].ID).click(function () {
                        f(jQuery(this).text(), jQuery(this).attr("id"))
                    }); else Ha(), n.hide(), q.hide(), Ea(), TASK_CNTRL.showWindow('<div style="text-align: center; font-weight: bold;"><h3>Roles</h3></div><div class="content"><div><p>No additional roles available.</strong></h3><div><a href="#" class="btn btn-primary cancel" title="Ok">Ok</a><div>')
            }
        });
        b.fail(function (a, b, d) {
            x({response: a.responseText, type: b, msg: d}, qa, "addAllRoleTasks")
        })
    }
    ...
    function ra(a) {
        ...
        b.done(function (b) {
            void 0 != b.error ? (b.error["class"] == y[0] ? v(function () {
                ra(a)
            }) : d(u[0], "addAllGroupTasks," + JSON.stringify(b.error)), n.hide(), q.hide()) : (b = b.data, 0 == b.length ? (a ? a() : jQuery("#form-list").hide(), n.hide(), q.hide(), TASK_CNTRL.showNoTaskWindow()) : D(function () {
                for (var h = b.length, d = function (b, e) {
                    if (0 == --h) {
                        var d = H.length;
                        A = !0;
                        if (0 == d)TASK_CNTRL.showAll(H);
                        else {
                            for (var c = 0; c < d; c++)TASK_DB.insert(H[c].name, {syncID: H[c].ID, details: H[c]}, c == d - 1);
                            H = [];
                            a && a()
                        }
                    }
                }, c = 0; c < h; c++)da("task", b[c].ID, d)
            }))
        });
        b.fail(function (a, b, d) {
            x({response: a.responseText, type: b, msg: d}, ra, "addAllGroupTasks")
        })
    }
    ...
    function da(a, b, e) {
        ...
        h.done(function (h) {
            if (void 0 != h.error)h.error["class"] == y[0] ? v(function () {
                da(a, b, e)
            }) : d(u[0], "addGroupTasks-" + a + "," + JSON.stringify(h.error)), n.hide(), q.hide(); else if (h = h.data, 0 == h.length)e && e(a, b); else if ("opTask" != a) {
                da("opTask",
                    b, e);
        ...
        h.fail(function (a, b, e) {
            x({response: a.responseText, type: b, msg: e}, da, "addGroupTasks-fail" + b + "-" + e + " " + a.responseText)
        })
    }
    ...
    function sa() {
        ...
        jQuery.when(jQuery.ajax(a), jQuery.ajax(b)).done(function (a, b) {
            ...
            if (void 0 != c)c["class"] == y[0] ? v(function () {
                sa()
            }) : d(u[0], "addRecentTasks," + JSON.stringify(c)), n.hide(), q.hide(); else {
    ...
    function ta() {
        function a(a, e) {
            ...
            h.data["project:statusEquatesWith_Mod"] = "in";
            e ? h.data[a + ":personal"] = !0 : (h.data["project:projectedCompletionDate"] = "$$TODAYb-1w", h.data["project:projectedCompletionDate_Mod"] =
                "gt");
            h.data.assignedToID = g.get("userID");
        ...
        n.show();
        jQuery.when(jQuery.ajax(a("task", !1)), jQuery.ajax(a("task", !0)), jQuery.ajax(a("opTask", !1))).done(function (a, e, h, c) {
            a = a[0];
            e = e[0];
            h = h[0];
            ...
            void 0 != c ? (c["class"] ==
                y[0] ? v(function () {
                ta()
            }) : d(u[0], "addCurrentTasks," + JSON.stringify(c)), n.hide(), q.hide()) : 0 == e.data.length && 0 == a.data.length && 0 == h.data.length ? (A = !0, D(function () {
                TASK_CNTRL.showAll()
            })) : D(function () {
                function d(a, b, e) {
                    for (var h = a.length, c = 0; c < h; c++) {
                        var g = a[c], g = g[b];
                        TASK_DB.insert(g.name, {syncID: g.ID, details: g}, e && c == h - 1)
                    }
                }

                A = !0;
                g.get("personalTasks") && d(e.data, "task", !(a.data.length + h.data.length));
                d(a.data, "task", !h.data.length);
                d(h.data, "opTask", !0)
            })
        }).fail(function (a, e, h) {
                x({response: a.responseText,
                    type: e, msg: h}, function () {
                    ta()
                }, "addCurrentTasks")
            })
    }
    ...
    function ua() {
        function a(a) {
            var e = {url: t() + "assgn/search?" + a + ":statusEquatesWith=NEW&" + a + ":statusEquatesWith=INP&" + a + ":statusEquatesWith=ROP&project:statusEquatesWith=CUR&project:statusEquatesWith=QUE", type: "get", data: {}, dataType: "json"};
            e.data[a + ":statusEquatesWith_Mod"] = "in";
            e.data[a + ":isComplete"] = !1;
            e.data.status = "AA";
            e.data["project:statusEquatesWith_Mod"] = "in";
            e.data["project:projectedCompletionDate"] = "$$TODAYb-1w";
            e.data["project:projectedCompletionDate_Mod"] =
                "gt";
            e.data.assignedToID = g.get("userID");
            e.data.fields = a + ":canStart," + a + ":parent," + a + ":priority," + a + ":project, " + a + ":project:hourTypes," + a + ":project:status," + a + ":status, " + a + ":extRefID," + a + ":permissions," + a + ":plannedCompletionDate";
            "task" == a && (e.data[a + ":canStart"] = !0, e.data.fields += ",task:percentComplete");
            w(e);
            return e
        }

        n.show();
        jQuery.when(jQuery.ajax(a("task")), jQuery.ajax(a("opTask"))).done(function (a, e) {
            a = a[0];
            e = e[0];
            var h = void 0 != a.error ? a.error : e.error;
            void 0 != h ? (h["class"] == y[0] ? v(function () {
                ua()
            }) :
                d(u[0], "addRequestedTasks," + JSON.stringify(h)), n.hide(), q.hide()) : 0 == a.data.length && 0 == e.data.length ? (A = !0, D(function () {
                TASK_CNTRL.showAll()
            })) : D(function () {
                function h(a, b) {
                    for (var e = a.length, d = 0; d < e; d++) {
                        var c = a[d][b];
                        c.canStart && TASK_DB.insert(c.name, {syncID: c.ID, details: c}, d == e - 1)
                    }
                }
                ...
                x({response: a.responseText, type: e, msg: h}, function () {
                    ua()
                }, "addRequestedTasks")
            })
    }
    ...
    function va() {
        function a(a) {
            var e = {url: t() + "assgn/search?" + a + ":statusEquatesWith=ONH&" +
            ...
            e.data["project:statusEquatesWith_Mod"] = "in";
            e.data["project:projectedCompletionDate"] = "$$TODAYb-1w";
            e.data["project:projectedCompletionDate_Mod"] = "gt";
            e.data.assignedToID = g.get("userID");
            e.data.fields = a + ":canStart," + a + ":parent," + a +
                ":priority," + a + ":project, " + a + ":project:hourTypes," + a + ":project:status," + a + ":status, " + a + ":extRefID," + a + ":permissions," + a + ":plannedCompletionDate";
            "task" == a && (e.data.fields += ",task:percentComplete");
            w(e);
            return e
        }
        ...
        jQuery.when(jQuery.ajax(a("task")), jQuery.ajax(a("opTask"))).done(function (a, e) {
            ...
            void 0 != h ? (h["class"] == y[0] ? v(function () {
                va()
            }) : d(u[0], "addOnHoldTasks," + JSON.stringify(h)), n.hide(), q.hide()) : 0 == a.data.length && 0 == e.data.length ? (A = !0, D(function () {
                TASK_CNTRL.showAll()
            })) : D(function () {
                function h(a, b) {
                    for (var e = a.length, d = 0; d < e; d++) {
                        var c = a[d], c = c[b];
                        TASK_DB.insert(c.name, {syncID: c.ID, details: c}, d == e - 1)
                    }
                }
    ...
    function Ka() {
        ...
        jQuery.when(jQuery.ajax(a), jQuery.ajax(b)).done(function (a, b) {
            ...
            b = b[0];
            if (void 0 != a.error)a.error["class"] == y[0] ? v(function () {
            }) : d(u[0], "getHourTypes," + JSON.stringify(a.error)), n.hide(), q.hide(); else {
                a = a.data;
                if (void 0 != a && 0 != a.length && 0 != a[0].hourTypes.length) {
                    g.set("hour_type_list", a[0].hourTypes);
                    for (var c = [], f = 0; f < a[0].hourTypes.length; f++)c[a[0].hourTypes[f].ID] = !0;
                    g.set("hour_types", c);
                    wa = !0
                }
                b = b.data;
                if (void 0 != b && 0 != b.length) {
                    ...
                    for (f = 0; f < b.length; f++) {
                        var k;
                        "default.hourtype.task" == b[f].nameKey ? k = "TASK" : "default.hourtype.optask" == b[f].nameKey && (k = "OPTASK");
                        k && (c[k] = b[f])
                    }
                ...
                }) : "timeout" == b ? Ka() : d(u[0], "getHourTypes," + a.responseText)
            })
    }

    function Ua(a, b) {
        if ("GEN" == a.details.objCode)b(a); else {
            var e = g.get("hour_type_list");
            e || (e = []);
            var d = g.get("default_hour_type_list");
            if (e && 1 == e.length)a.details.hourTypeID = e[0].ID, TASK_DB.updateDetails(a.ID, a.details), b(a); else if (a.details.project.hourTypes && 1 == a.details.project.hourTypes.length)a.details.hourTypeID = a.details.project.hourTypes[0].ID, TASK_DB.updateDetails(a.ID, a.details), b(a); else {
                var c = jQuery("#hourTypeSelector");
                c.text("");
                d[a.details.objCode] && c.append($("<option />").val(d[a.details.objCode].ID).text(d[a.details.objCode].name));
                e && e.length && jQuery.each(e, function () {
                    var a =
                        this.ID;
                    c.append($("<option />").val(a).text(this.name))
                });
                a.details.project.hourTypes && a.details.project.hourTypes.length && jQuery.each(a.details.project.hourTypes, function () {
                    c.append($("<option />").val(this.ID).text(this.name))
                });
                TASK_CNTRL.showWindow(null, "#hourTypeWindow");
                jQuery("#done_hourType").unbind("click").bind("click", function (e) {
                    e.preventDefault();
    ...
    function ha(a) {
        ...
        null != b ? a(b) : (b = {url: t("api/v7.0") + "user/search?ID=$$USER.ID&fields=schedule:*,schedule:nonWorkDays:*", type: "get", data: {}, dataType: "json"}, w(b), b = jQuery.ajax(b), b.done(function (b) {
            if (void 0 != b.error)b.error["class"] == y[0] ? v(function () {
            }) : d(u[0], "getSchedule," + JSON.stringify(b.error)), n.hide(), q.hide(); else if (b = b.data[0].schedule,
                0 != b.length) {
                var h = new Date, c = b["sunday monday tuesday wednesday thursday friday saturday".split(" ")[h.getDay()]], f;
                if (!0 == b.hasNonWorkDays) {
                    for (var k = 0; k < b.nonWorkDays.length; k++)if (b.nonWorkDays[k].nonWorkDate == h.getFullYear() + "-" + (h.getMonth() + 1) + "-" + h.getDate() && (void 0 != f && null != b.nonWorkDays[k].userID || void 0 == f))f = b.nonWorkDays[k].scheduleDay;
                    void 0 != f && (c = f)
                }
            ...
            401 == b.status ? v(function () {
                ...
                "timeout" == h ? ha(a) : d(u[0], "getSchedule," + b.responseText)
        }))
    }
    ...
    function ma() {
        ...
        a.done(function (a) {
            void 0 != a.error ? (a.error["class"] == y[0] ? v(function () {
            }) : d(u[0], "getTodaysTotalHours," + JSON.stringify(a.error)), n.hide(), q.hide()) : (a = a.data, jQuery("#totalHours").html("Today's Hours: " + (null == a.sum_hours ? "0" :
                a.sum_hours.toFixed(2))))
        });
        a.fail(function (a, e, h) {
            401 == a.status ? v(function () {
            }) : "timeout" == e ? ma() : d(u[0], "getTodaysTotalHours," + a.responseText)
        })
    }
    ...
    function Z(a) {
        ...
        b.done(function (b) {
            void 0 != b.error ? (b.error["class"] == y[0] ? v(function () {
                Z(a)
            }) : d(u[0], "getTaskAssignment," + a.syncID + "," + JSON.stringify(b.error)),
                n.hide(), q.hide()) : (b = b.data[0], a.running ? (a.done = 1, TASK_CNTRL.stopTask(a)) : xa(b, a))
        });
        b.fail(function (b, d, c) {
            x({response: b.responseText, type: d, msg: c}, function () {
                Z(a)
    ...
    function xa(a, b) {
        n.show();
        var e = {url: t() + "assignment/" + a.ID, data: {method: "PUT", status: "DN"}, type: "get", dataType: "json"};
        w(e);
        e = jQuery.ajax(e);
        e.done(function (e) {
            void 0 != e.error ? (e.error["class"] == y[0] ? v(function () {
                xa(a, b)
            }) : d(u[0], "setAssignmentDone," + a.ID + "," + JSON.stringify(e.error)), n.hide(), q.hide()) :
                (TASK_DB.remove(b.ID), n.hide(), q.hide(), d(u[2], G(b) + ",assignment done"), d(u[1], "assignment done"), ya(b))
        });
        e.fail(function (e, d, c) {
            x({response: e.responseText, type: d, msg: c}, function () {
                xa(a, b)
            }, "setAssignmentDone," + a.ID + ", " + e.responseText)
        })
    }
    ...
    function ya(a) {
        ...
        b.done(function (b) {
            void 0 != b.error ? (b.error["class"] ==
                y[0] ? v(function () {
                ya(a)
            }) : d(u[0], "getTaskDoneAssignmentCount," + taskID + "," + JSON.stringify(b.error)), n.hide(), q.hide()) : (b = b.data, n.hide(), q.hide(), 0 == b.count && ea(a))
        });
        b.fail(function (b, d, c) {
            x({response: b.responseText, type: d, msg: c}, function () {
                ya(a)
    ...
    function ea(a) {
        var b = a.syncID;
        n.show();
        var e = "OPTASK" == a.details.objCode ? "CLS" : "CPL", e = {url: t() + a.details.objCode.toLowerCase() + "/" + b, data: {method: "put", status: e}, type: "get", dataType: "json"};
        w(e);
        e = jQuery.ajax(e);
        e.done(function (e) {
            void 0 != e.error ? (e.error["class"] == y[0] ? v(function () {
                ...
                ea(a)
            }, "setTaskDone," + b), n.hide(), q.hide()) : (n.hide(), q.hide(), d(u[2], G(a) + ",task done"), d(u[1], "task done"))
        });
        e.fail(function (e, d, c) {
            x({response: e.responseText, type: d, msg: c}, function () {
                ea(a)
            }, "setTaskDone," + b + ", " + e.responseText)
        })
    }
    ...
    function Q(a) {
        ...
        b.done(function (b) {
            void 0 != b.error ? (b.error["class"] == y[0] ? v(function () {
                ...
                Q(a)
            }, "setTaskInProgress," + a.syncID), n.hide(), q.hide()) : (n.hide(), q.hide(), !0 == g.get("logout_on_select") && K())
        });
        b.fail(function (b, d, c) {
            x({response: b.responseText, type: d, msg: c}, function () {
                Q(a)
    ...
    function ba(a, b, d) {
        ...
        d.fail(function (d, e, c) {
            x({response: d.responseText, type: e, msg: c}, function () {
                ba(a, b)
    ...
    function Qa(a, b, e) {
        ...
        var f = Number((a.duration / 3600).toFixed(2));
        g.get("nearestHour") && (f = Math.round(f));
        l.hours = parseFloat((f + parseFloat(("" + c).split(".").join(".")) + " ").split(".").join(".")).toFixed(2);
        l.ownerID = g.get("userID");
        var k = new Date;
        e || (l.extRefID = ("OK~" + k.getHours() + ":" + k.getMinutes() + "H" + l.hours + "D" + f + "T" + TASK_CNTRL.hms(a.time) +
            ";" + (b && b.extRefID ? b.extRefID.replace("OK~", "OK!") : ".")).substring(0, 254));
        d(u[1], "setHourAttask, oldhr: " + c + " method: " + l.method + " start: " + a.dateStr + " task: " + JSON.stringify(a) + " sid: " + g.get("sessionID") + " time: " + k.toTimeString());
        var p = function () {
            n.show();
            ...
            b.done(function (b) {
                void 0 != b.error ? (b.error["class"] == y[0] ? v(function () {
                    ...
                    " url: " + t() + " " + JSON.stringify(l) + " " + G(a) + "," + a.duration, 'Time was not tracked, please click on "View" for task: ' + a.name + " to record time manually."), n.hide(), q.hide()) : (TASK_DB.setTask(a), TASK_CNTRL.toggleRunText(), ma(), void 0 != l.taskID || void 0 != l.opTaskID ? Q(a) : !0 == g.get("logout_on_select") && K(), n.hide(), q.hide(), d(u[2], G(a)), d(u[1], "time tracked newHr: " + l.hours + " " + a.duration + " oldHr: " + c), a.done && Z(a))
            });
            b.fail(function (b, d, c) {
    ...
    function Oa(a, b, d) {
        ...
        var g = function () {
            ...
            b.done(function (b) {
                ...
                void 0 != b.error && (b.error["class"] == y[0] ? v(function () {
                    g()
                }) : x({response: b.error, type: null, msg: null}, g, "setHourNoteAttask," + a.syncID + " " + a.name + " url: " + t() + " " + JSON.stringify(c) + " " + G(a) + "," + a.duration, "Timesheet hour note was not recorded."))
            });
            b.fail(function (b, d, e) {
                x({response: b.responseText, type: d, msg: e}, g, "setHourNoteAttask-fail," + a.syncID +
                    " " + a.name + " url: " + t() + " " + JSON.stringify(c) + " " + G(a) + "," + a.duration + ", " + b.responseText + ", " + e, b.responseText + "<br>Timesheet hour note was not recorded.")
            })
        };
        g()
    }
    ...
    function ca(a, b, c) {
        var h = new Date, f = {};
        f.entryDate = void 0 != a.dateStr ? a.dateStr : h.getFullYear() + "-" + (h.getMonth() + 1) + "-" + h.getDate();
        f.fields = "permissions,ID,hours,extRefID,description";
        f.ownerID = g.get("userID");
        if ("GEN" == a.details.objCode)f.hourTypeID = a.syncID; else {
            switch (a.details.objCode) {
                case "OPTASK":
                    f.opTaskID = a.syncID;
                    break;
                case "TASK":
                    ...
                    break;
                case "PROJ":
                    f.projectID = a.syncID
            }
            void 0 != a.details.hourTypeID && "" != a.details.hourTypeID && (f.hourTypeID = a.details.hourTypeID)
        }
        h = {url: t() + "hour/search", type: "get", data: f, dataType: "json", async: !1};
        w(h);
        h = jQuery.ajax(h);
        h.done(function (h) {
            if (void 0 != h.error)h.error["class"] == y[0] ? v(function () {
                ca(a, b, c)
            }) : d(u[0], "getHour," + a.syncID + "," + JSON.stringify(h.error)), n.hide(), q.hide(); else if (d(u[1], "getHour " + h.data.length + " " + JSON.stringify(h.data)), b) {
                for (var g = h.data.length, f = 0; f < g; f++)if (-1 !=
                    h.data[f].permissions.actions.indexOf("ADD_HOURS") && -1 != h.data[f].permissions.actions.indexOf("CHANGE_STATUS")) {
                    b(a, h.data[f], c);
                    return
                }
                b(a, h.data[0], c)
        ...
        h.fail(function (b, c, e) {
            d(u[0], "getHour," + a.syncID + "," + JSON.stringify(e))
        })
    }

    var y = ["com.attask.common.AuthenticationException"], u = ["error", "debug", "informational"], g, J = !(window.hasOwnProperty("chrome") && window.chrome.hasOwnProperty("windows")), E = isMobile(), ga = "www.atappstore.com", za = "secure.atappstore.com", Ia = -1 !== document.URL.indexOf("dashboard.directory") ||
        -1 !== document.URL.indexOf(ga) || -1 !== document.URL.indexOf(za), wa = !1, M = !1, Ra = "Force Desktop Stopwatch Access", Ba = 300, H = [], B = [], Ca, Da, L = {current: {f: ta, t: "Current", d: "Tasks and issues you're working on"}, recent: {f: sa, t: "Recent", d: "All tasks and issues tracked within 14 days"}, requested: {f: ua, t: "Requested", d: "Tasks and issues you can start on"}, onhold: {f: va, t: "On Hold", d: "Tasks and issues that are on hold"}, all: {f: S, t: "All Assigned", d: "All tasks and issues assigned to you"}, team: {f: ra, t: "Team Assigned",
    ...
    J && (!Ia && -1 == location.origin.indexOf("localhost")) && (location = "https://" + ga + "/client/enterprise-stopwatch/launch.html");
    z();
    g = J ? {settings: [], set: function (a, b) {
        ...
        b = encodeURI(b) + (null == d ? "" : "; expires=" + c.toUTCString());
        document.cookie = a + "=" + b
    }, getCookie: function (a) {
        var b, d, c, g = document.cookie.split(";");
        for (b = 0; b < g.length; b++)if (d = g[b].substr(0,
            g[b].indexOf("=")), c = g[b].substr(g[b].indexOf("=") + 1), d = d.replace(/^\s+|\s+$/g, ""), d == a)return decodeURI(c);
        return"undefined"
    }} : new Store("Attask");
    ...
    window.addEventListener("online", function (a) {
        ...
        if (void 0 != a)for (var b = a.length, d = 0; d < b; d++)N(a[d], a[d].duration);
        g.set("offlineTasks", []);
        M = !1
    });
    var P;
    ...
    OB.stop.add(function (a, b) {
        ...
        c = new Date(a.start);
        h = new Date;
        d(u[1], "stop: " + c + " == " + h + " dur:" + b + " totalDuration:" + a.time);
        if (g.get("minuteBufferSnapToSched")) {
            var f =
                g.get("todaySched"), k = g.get("minuteBuffer"), p = h.toLocaleDateString("en-US"), s = Date.parse(p + f.start), f = Date.parse(p + f.stop), p = new Date(s);
            c < s && c > p.setMinutes(p.getMinutes() - k) && (c = new Date(s));
            p = new Date(f);
            h > f && h < p.setMinutes(p.getMinutes() + k) && (h = new Date(f));
            if (c >= h) {
                n.hide();
                return
            }
            b = Math.floor((h.getTime() - c.getTime()) / 1E3);
            d(u[1], "minuteBufferSnapToSched: " + c + " == " + h + " dur:" + b)
        }
    ...
    OB.preStart.add(function (a) {
        a.running ? TASK_CNTRL.toggleTimer(a) : navigator.onLine && !a.disable && (p(a, a.time) || function () {
            TASK_DB.loadAll(function (b) {
                var d, c, f = !1, k = b.length;
                for (d = 0; d < k; d++)if (c = b[d], null != c && c.running && a.ID != c.ID) {
                    TASK_CNTRL.toggleTimer(c);
                    f = !0;
                    break
                }
    ...
    OB.postAllItemRender.add(function () {
        ...
        g.get("ent") ? (jQuery("#totalHours").attr("title", "").attr("data-hasqtip", !1).unbind("click").addClass("disabled"), g.get("projectFilter") || jQuery("#filter-projects").remove()) :
            (jQuery(".btn-comment").remove(), jQuery("#sort-order").remove(), jQuery(".hour-note").remove(), jQuery("#filter-projects").remove(), jQuery("#filter-onhold").remove());
        if (A) {
            ...
            void 0 != b ? ("string" === typeof b && (b = JSON.parse(b)), B[b.sort] = b.order, b = b.sort) : (b = g.get("sort"), void 0 == b && (b = "date"));
            jQuery("#sort-" + b).click();
            jQuery("#search-bar").is(":visible") && jQuery("#search").click();
            jQuery("#loading").hide();
            a.show();
    ...
    OB.postItemRender.add(function (a) {
        var b = jQuery("#item" + a.ID), d = b.find(".details");
        if (0 == b.find("#buttons .view").length) {
            var c = b.find("#buttons"), f = ["", {label: "Low", css: "default"}, {label: "Normal",
                css: "success"}, {label: "High", css: "warning"}, {label: "Urgent", css: "danger"}];
            "GEN" != a.details.objCode && (void 0 != a.details.priority && 0 != a.details.priority) && (f = f[a.details.priority], c.append('<span class="task priority ' + f.css + '" title="' + f.label + '" data-priority="' + a.details.priority + '">' + f.label[0] + "</span> "));
            var k;
            "GEN" == a.details.objCode ? (k = "General", b.attr("data-type", k), b.attr("data-force-order", 0), c.attr("class", "days priority"), b.attr("class", "item general")) : "OPTASK" == a.details.objCode ? (k =
                "Issue", b.attr("data-type", k), b.attr("data-force-order", 1), c.append('<span class="issue" title="Issue">I</span> ')) : "TASK" == a.details.objCode ? (k = "Task", b.attr("data-type", k), b.attr("data-force-order", 1), c.append('<span class="task" title="Task">T</span> ')) : "PROJ" == a.details.objCode && (k = "Project", b.attr("data-type", k), b.attr("data-force-order", 1), c.append('<span class="project" title="Project">P</span> '));
            void 0 != a.details.status && (d.attr("data-status", a.details.status), c.append('<span class="status" title="' +
                k + " status: " + a.details.status + ("Project" !== k ? "<br>Project status: " + a.details.project.status : "") + '">' + a.details.status + "</span> "));
            "General" != k && (c.append('<a href="https://' + g.get("host") + "." + g.get("domain") + "/" + k.toLowerCase() + "/view?ID=" + a.syncID + '" target="_blank" class="btn btn-mini view" rel="' + a.ID + '" title="View ' + k + '"><i class="fas fa-tasks"></i></a> '), a.disable ? void 0 != a.details.status && (b.find(".timer").text("status: " + a.details.status).unbind("click").addClass("disabled"), b.find(".timer-info").text("status: " +
                a.details.status)) : "Project" != k && c.append('<a href="#" class="btn btn-mini done" rel="' + a.ID + '" title="Done ' + k + '"><i class="fas fa-check"></i></a> '));
            "General" != k && c.append('<a href="#" class="btn btn-mini btn-comment" rel="' + a.ID + '" title="Add Comment"><i class="fas fa-comment-dots"></i></a> ');
            !a.disable && void 0 != a.details.percentComplete && ("Task" == k ? c.append('<a href="#" class="btn btn-mini percent" rel="' + a.ID + '" title="Percent Completed">' + a.details.percentComplete + "%</a> ") : "Project" == k &&
                c.append('<span class="btn-mini" rel="' + a.ID + '" title="Percent Completed">' + a.details.percentComplete + "%</span> "));
            c = '<span class="date" style="background-color: #EEE;"></span>';
            if (!a.disable && "General" != k && void 0 != a.details.plannedCompletionDate)if (f = a.details.plannedCompletionDate.replace(/:\d\d\d/, "").replace(/-/g, "/").replace(/[a-z]+/gi, " ").replace(/\/(\d\d\d\d)/, "-$1"), f = new Date(Date.parse(f)), isNaN(f.getTime()))d.attr("data-days", 9999); else {
                var p = new Date;
                p.setHours(0);
                p.setMinutes(0);
                p.setSeconds(0);
                c = "#D2E0FF";
                p = Math.round((f.getTime() - p.getTime()) / 1E3 / 60 / 60 / 24);
                0 > p ? c = "#FFCDCD" : 1 >= p && (c = "#FFD7AB");
                var s;
                s = 0 < p ? "days due: " + p : "days late: " + Math.abs(p);
                c = '<div class="date" style="background-color: ' + c + ';"><div class="days">' + s + "</div><div>" + ja(f.getMonth()) + " " + f.getDate() + ", " + f.getFullYear() + "</div></div>";
                d.attr("data-days", p)
            } else d.attr("data-days", 9999);
            f = "";
            null != a.details.parent && (f = "<br><br>Parent Task: <br>" + a.details.parent.name);
            p = "";
            a.details.extRefID && (p = "Reference: <br>" + a.details.extRefID);
            b.find(".name").attr("title", k + ": <br>" + a.name + p + f);
            "General" != k ? (k = "Project" == k ? "" : a.details.project.name, d.html(k).attr("title", "Project: <br>" + k + "<br>Status: " + a.details.project.status).attr("data-priority", a.details.priority), d = "", a.disable || (d = '<span class="hour-note" rel="' + a.ID + '">Add Note</span>'), b.find(".c6").after('<div class="task-lower-block">' + c + d + "</div>")) : (d.html(a.details.description).attr("title", a.details.description), d = a.time ? "block" : "hidden", b.find(".c6").after('<div class="general-lower-block"><span class="hour-note" style="display:' +
                d + ';" rel="' + a.ID + '">Add Note</span></div>'));
            E || b.find(".task-lower-block").mouseover(function () {
                ...
                if ((b = TASK_DB.get(b)) && b.time && !b.running && !b.disable) {
                    var d = jQuery(this).find(".date").hide(), c = a.show();
                    setTimeout(function () {
                        d.show();
                        c.hide()
                    }, 3E3)
...
function getQueryString(c) {
    if (null == QUERY_LOOKUP) {
        var k = location.search.substring(1), f = /([^&=]+)=([^&]*)/g, m;
        for (QUERY_LOOKUP = {}; m = f.exec(k);)QUERY_LOOKUP[decodeURIComponent(m[1])] = decodeURIComponent(m[2])
    }
...
function taskController(c) {
    var k, f = window.hasOwnProperty("chrome") && window.chrome.hasOwnProperty("windows"), m = !f && isMobile();
    OB.start.add(function (d) {
        startNotify(d)
    });
    OB.stop.add(function (d, c, f) {
        stopNotify(d)
    });
    OB.onLoad.add(function () {
    });
    OB.postAllItemRender.add(function () {
        ...
        if (!f || "full" == getQueryString("view")) {
            var d = function () {
                var d = jQuery("#form-list"), c;
                c = d.height();
                c += window.innerHeight - k;
                0 != window.innerHeight - k && d.height(c);
                k = window.innerHeight
            };
            jQuery(window).unbind("resize");
            jQuery(window).bind("resize", d);
            d()
        }
        var c = !1, d = {style: {tip: {corner: !0, width: 5, height: 5}, def: !1, classes: "qtip-info"}, position: {adjust: {method: "flip flip"}, at: "bottom left", my: "top left", viewport: jQuery(window)}, show: {}, events: {show: function (d, f) {
            c = !0
        }}, hide: {delay: m ? 0 : 100}};
        jQuery("span[title]").qtip(d);
        jQuery("#form-list").bind("scroll", function () {
            c && (jQuery("span[title]").qtip("hide"), c = !1)
        });
        m || (jQuery("a[title]").qtip(d), jQuery("#form-list").bind("scroll", function () {
            c && (jQuery("a[title]").qtip("hide"),
                c = !1)
        }));
        d.content = "time < 30 seconds<br> time was not tracked";
        d.show.event = !1;
        var p = jQuery(".timer");
        p.qtip(d);
        m || p.mouseover(function () {
            var d = jQuery(this).find(".time").hide(), c = jQuery(this).find(".timer-info").show();
            setTimeout(function () {
                d.show();
                c.hide()
            }, 3E3)
    ...
    return{display_intervals: [], update_intervals: [], taskDB: c, init: function () {
        ...
        this.bind();
        this.toggleRunText();
        var c = jQuery("#form-list");
        !f || "full" == getQueryString("view") ? c.addClass("full") : c.css("max-height", "439px");
        !f && (window.webkitNotifications && 0 != window.webkitNotifications.checkPermission()) && window.open("core/js/permission.html",
            "Permission", "height=540,width=447");
        k = 567;
        sortable("#form-list", {forcePlaceholderSize: !0, items: ".item", handle: ".handle"});
        sortable("#form-list", "disable");
        sortable("#form-list")[0].addEventListener("sortupdate", function (c) {
            var f = d.taskDB.get(jQuery(c.detail.item).attr("rel"));
            f.order = c.detail.destination.index + 1;
            d.taskDB.setTask(f)
        })
    }, bind: function () {
        ...
        jQuery("#cover").hide();
        jQuery(document).on("click", ".cancel", function (f) {
            f.preventDefault();
        ...
        jQuery(document).on("click", "#popout", function (c) {
            c.preventDefault();
            f && window.chrome.windows.create({url: "index.html?view=full", type: "popup", width: 450, height: 600})
        });
        ...
        jQuery(document).on("click", "#button-create", function () {
            var f = jQuery("#form-create");
            c.taskDB.insert(f.find(":input[name='task-name']").val());
            f.hide().find("input:text").val("")
        });
        jQuery("#task-name").keydown(function (f) {
            13 ==
                f.keyCode && (f = jQuery("#form-create"), c.taskDB.insert(f.find(":input[name='task-name']").val()), f.hide().find("input:text").val(""))
        });
        ...
        jQuery(document).on("click", "#button-update", function () {
            var f = jQuery("#form-update");
            f.hide();
            jQuery("#top-bar").show();
            jQuery("#bar").show();
            c.taskDB.update(f.find(":input[name='task-id']").val(), f.find(":input[name='task-name']").val(), f.find(":input[name='task-time']").val())
        });
        jQuery(document).on("click", ".reset", function (f) {
            f.preventDefault();
            f = jQuery(this).attr("rel");
            c.taskDB.reset(f)
        });
        var k = function (f) {
            f.preventDefault();
    ...
    }, showWindow: function (c, f) {
        var k = jQuery("#cover");
        f = void 0 == f ? "#window" : f;
        null != c && jQuery(f).html(c);
        m ? (k.css("height", "100%").css("opacity", "0.8").show(),
            jQuery(f).animate({top: 60 + window.pageYOffset + "px"}, {easing: "easeInCubic", duration: 400, complete: function () {
        ...
        m ? jQuery(c).animate({top: "-700px"}, {easing: "easeOutCubic", duration: 400, complete: function () {
            ...
            duration: 400, complete: function () {
                f.animate({opacity: "0"}, {duration: 400, complete: function () {
                    f.hide()
                }})
            }})
    }, update: function (c) {
        var f = jQuery("#form-update");
        f.find(":input[name='task-id']").val(c.ID);
        f.find(":input[name='task-name']").val(c.name);
        f.find(":input[name='task-time']").val(this.hms(c.time));
        f.slideDown()
        ...
        var f, k = "";
        f = jQuery("#item" + c.ID);
        0 != f.length ? f.show() : (k += '<div class="item' + (c.running ? " running" : "") + '" id="item' + c.ID + '" rel="' +
        ...
        if (void 0 != c) {
            var f, k = c.length;
            if (0 != k)for (f = 0; f < k; f++)this.showTask(c[f])
        }
        ...
        if (navigator.onLine) {
            ...
            f || window.clearInterval(k.update_intervals[c.ID]);
            c.running ? z = new Date(c.start) : this.taskDB.start(c.ID, z);
            k.display_intervals[c.ID] = window.setInterval(function () {
                var f = Number(c.time) + Math.floor(((new Date).getTime() - z.getTime()) / 1E3);
                jQuery("#item" + c.ID).find(".timer .time").text(k.hms(f));
                OB.displayInterval.dispatch(c, f)
            }, 500);
            f || (k.update_intervals[c.ID] = window.setInterval(function () {
        ...
        f || window.clearInterval(this.update_intervals[c.ID]);
        ...
        this.taskDB.running(function (m) {
            f ? window.chrome.browserAction.setBadgeText({text: "RUN"}) : document.title = "Stopwatch - Running: " + m.name;
            c.text("Running: " + m.name).attr("rel", m.ID).show();
            k.hide()
        }, function (m) {
            0 == m && (f ? window.chrome.browserAction.setBadgeText({text: ""}) : document.title = "Stopwatch", c.hide(), k.show())
        })
    }, hms: function (c) {
        c = [0, 0, c];
        var f;
        for (f = 2; 0 < f; f--)c[f - 1] = Math.floor(c[f] /
            60), c[f] %= 60, 10 > c[f] && (c[f] = "0" + c[f]);
        return c.join(":")
    }, sec: function (c) {
        c = String(c).split(":");
...
function taskDB() {
    return{dm: [], log: [], lastId: 0, sql: function (c, k, f, m, d, s) {
    }, init: function () {
        this.createDB();
        return this
        ...
        var f = this.log;
        20 == f.length && f.shift();
        f.push({type: c, message: k, time: (new Date).getTime()});
        this.log = f
    }, getEvents: function () {
        return this.log
    }, onError: function (c) {
        ...
        updateAtInterval: function (c, k, f) {
            this.dm[c].running = 1;
            this.dm[c].time = Number(k);
            this.dm[c].interval = 1;
            OB.updateAtInterval.dispatch(this.dm[c], Number(f))
        }, stop: function (c, k, f, m) {
            c = this.dm[c];
            c.running = 0;
            c.interval = 0;
            c.done = m;
            c.time = Number(k);
            OB.stop.dispatch(c, Number(f))
        }, setTask: function (c) {
            this.dm[c.ID] = c
        }, running: function (c, k) {
            for (var f in this.dm)if (null != this.dm[f] && this.dm[f].running)return c(this.dm[f]), !0;
            k();
            return!1
        }, insert: function (c, k, f) {
            var m = "null", d = "", s = 0;
            void 0 != k && (void 0 != k.syncID &&
                (m = k.syncID), void 0 != k.details && (d = k.details), void 0 != k.disable && (s = k.disable));
            if ("null" != m) {
                k = !1;
                ...
                k || (p = this.nextID(), this.dm[p] = {ID: p, syncID: m, name: c, details: d, time: 0, start: new Date, running: 0, done: 0, disable: s, interval: !1, order: 1E3}, OB.insert.dispatch(c, {syncID: m, details: d}));
                TASK_CNTRL.showTask(this.dm[p]);
                f && OB.postAllItemRender.dispatch()
            }
        }, nextID: function () {
            return this.lastId++
            ...
            null != this.dm[c] &&
            k(this.dm[c])
        }, update: function (c, k, f) {
            f = TASK_CNTRL.sec(f);
            var m;
            null != this.dm[c] && (this.dm[c].name = k, m = this.dm[c].time, this.dm[c].time = f);
            OB.update.dispatch(this.dm[c], f - m)
...
function taskDBls() {
    return{ls: null, ps: null, log: null, userId: null, sql: function (c, k, f, m, d, s) {
    }, init: function () {
        ...
        var f = this.log.get("trace");
        20 == f.length && f.shift();
        f.push({type: c, message: k, time: (new Date).getTime()});
        this.log.set("trace", f)
    }, getEvents: function () {
        ...
        onError: function (c) {
            ...
            var f = this.ls.get(c);
            null != f && void 0 != f && (f.running = 1, f.start = k, this.ls.set(c, f), null != this.userId && (this.ps.set(this.userId + "" + f.syncID, f), this.ps.set(this.userId, f.syncID)), OB.start.dispatch(f))
        }, updateAtInterval: function (c, k, f) {
            var m = this.ls.get(c);
            null != m && void 0 != m && (m.time =
                Number(k), m.running = 1, m.interval = 1, this.ls.set(c, m), OB.updateAtInterval.dispatch(m, Number(f)))
        }, stop: function (c, k, f, m) {
            c = this.ls.get(c);
            null != c && void 0 != c && (c.running = 0, c.interval = 0, c.done = m, c.time = Number(k), null != this.userId && (this.ps.remove(this.userId + "" + c.syncID), this.ps.remove(this.userId)), OB.stop.dispatch(c, Number(f)))
        }, setTask: function (c) {
            this.ls.set(c.ID, c)
        }, running: function (c, k) {
            var f = this.ls.get("list"), m, d = 0, s;
            for (s in f)f.hasOwnProperty(s) && (m = this.ls.get(s), null != m && (void 0 != m && m.running) &&
                (void 0 != c && c(m), d++));
            void 0 != k && k(d);
            return 0 != d
            ...
            var m = this, d = "null", s = "", p = 0;
            void 0 != k && (void 0 != k.syncID && (d = k.syncID), void 0 != k.details && (s = k.details), void 0 != k.disable && (p = k.disable));
            k = function (c, d, f, k, p) {
                var s = m.nextID();
                p = {ID: s, syncID: c, name: d, details: f, time: 0, start: new Date, running: 0, disable: p, interval: !1, order: 1E3};
                if (null != m.userId) {
                ...
                z[s] = c;
                m.ls.set("list", z);
                OB.insert.dispatch(d, {syncID: c, details: f});
                TASK_CNTRL.showTask(p);
                k && OB.postAllItemRender.dispatch()
            };
            if ("null" != d) {
                var z = m.ls.get("list"), O = !1, C;
                for (C in z)if (z.hasOwnProperty(C) && null != z[C] && z[C] == d) {
                    O = !0;
                    break
                }
                O ? (d = m.ls.get(C), d.name = c, s && Object.assign(d.details, s), m.ls.set(C, d), TASK_CNTRL.showTask(d), f && OB.postAllItemRender.dispatch()) : k(d, c, s, f, p)
            } else k(d, c, s, f, p)
        }, nextID: function () {
            ...
            f = TASK_CNTRL.sec(f);
            c = this.ls.get(c);
            null != c && void 0 != c && (c.name = k, k = c.time, c.time = f, OB.update.dispatch(c, f - k))
        }, updateDetails: function (c, k) {
            var f = this.ls.get(c);
            null != f && void 0 != f && (f.details = k, this.ls.set(c, f))
        }, remove: function (c) {
            ...
            var k =
                this.ls.get("list"), f = [], m, d;
            for (d in k)k.hasOwnProperty(d) && (m = this.ls.get(d), null != m && void 0 != m && f.push(m));
            c(f)
        }, loadSyncIds: function (c) {
            for (var k = this.ls.get("list"), f = [], m, d = c.length, s = 0; s < d; s++)m = k.indexOf(c[s]), -1 != m && (m = this.ls.get(m), null != m && void 0 != m && f.push(m));
            return f
        }}.init()
};
