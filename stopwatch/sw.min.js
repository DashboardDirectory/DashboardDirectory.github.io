C:\Users\tobia\Documents\stopwatch\Web\modules\js\config.js:2976: WARNING - Suspicious code. The result of the 'getelem' operator is not being used.
                        hrt[r[0].hourTypes[i].ID];
                        ^
...
cases:e,forceStrings:e,sortFunction:null}};d.fn.extend({tinysort:function(Z,ra){Z&&"string"!=typeof Z&&(ra=Z,Z=null);var ba=d.extend({},d.tinysort.defaults,ra),ta,oa=d(this).length,r={},k=!(!Z||""==Z),K=!(null===ba.attr||""==ba.attr),ga=null!==ba.data,da=k&&":"==Z[0],ka=da?this.filter(Z):this,R=ba.sortFunction,V="asc"==ba.order?1:-1,W=[];d.each(l,function(b,a){a.call(a,ba)});R||(R="rand"==ba.order?function(){return 0.5>Math.random()?1:-1}:function(a,c){var k=e,l=!ba.cases?a.s&&a.s.toLowerCase?a.s.toLowerCase():
a.s:a.s,r=!ba.cases?c.s&&c.s.toLowerCase?c.s.toLowerCase():c.s:c.s;if(!ba.forceStrings){var x=l&&l.match(g),K=r&&r.match(g);if(x&&K){var N=l.substr(0,l.length-x[0].length),L=r.substr(0,r.length-K[0].length);N==L&&(k=!e,l=b(x[0]),r=b(K[0]))}}var R=V*(l<r?-1:l>r?1:0);d.each(na,function(b,a){R=a.call(a,k,l,r,R)});return R});this.each(function(b,a){var c=d(a),e=k?da?ka.filter(a):c.find(Z):c,g=ga?""+e.data(ba.data):K?e.attr(ba.attr):ba.useVal?e.val():e.text(),l=c.parent();r[l]||(r[l]={s:[],n:[]});0<e.length?
r[l].s.push({s:g,e:c,n:b}):r[l].n.push({e:c,n:b})});for(ta in r)r[ta].s.sort(R);for(ta in r){var R=r[ta],x=[],Y=oa,N=[0,0],qa;switch(ba.place){case "first":d.each(R.s,function(b,a){Y=c(Y,a.n)});break;case "org":d.each(R.s,function(b,a){x.push(a.n)});break;case "end":Y=R.n.length;break;default:Y=0}for(qa=0;qa<oa;qa++){var ea=a(x,qa)?!e:qa>=Y&&qa<Y+R.s.length,L=(ea?R.s:R.n)[N[ea?0:1]].e;L.parent().append(L);(ea||!ba.returns)&&W.push(L.get(0));N[ea?0:1]++}}this.length=0;Array.prototype.push.apply(this,
...
null,k)})}function B(a,c,d){function f(a){function d(v){v&&(delete M[v.src],clearTimeout(h.timers.img[v.src]),b(v).unbind(z));b.isEmptyObject(M)&&(c!==k&&h.reposition(Q.event),a())}var v,M={};if(0===(v=q.find("img[src]:not([height]):not([width])")).length)return d();v.each(function(a,v){if(M[v.src]===e){var c=0;(function Ca(){if(v.height||v.width||3<c)return d(v);c+=1;h.timers.img[v.src]=setTimeout(Ca,700)})();b(v).bind("error"+z+" load"+z,function(){d(this)});M[v.src]=v}})}var q=I.content;return!h.rendered||
!a?k:(b.isFunction(a)&&(a=a.call(w,Q.event,h)||""),d!==k&&F(n.content.deferred),a.jquery&&0<a.length?q.empty().append(a.css({display:"block"})):q.html(a),0>h.rendered?u.queue("fx",f):(fa=0,f(b.noop)),h)}function G(){function e(a){if(u.hasClass(X))return k;clearTimeout(h.timers.show);clearTimeout(h.timers.hide);var v=function(){h.toggle(r,a)};0<n.show.delay?h.timers.show=setTimeout(v,n.show.delay):v()}function g(a){if(u.hasClass(X)||f||fa)return k;var v=b(a.relatedTarget||a.target),c=v.closest(aa)[0]===
u[0],v=v[0]===m.show[0];clearTimeout(h.timers.show);clearTimeout(h.timers.hide);if("mouse"===q.target&&c||n.hide.fixed&&/mouse(out|leave|move)/.test(a.type)&&(c||v))try{a.preventDefault(),a.stopImmediatePropagation()}catch(d){}else 0<n.hide.delay?h.timers.hide=setTimeout(function(){h.hide(a)},n.hide.delay):h.hide(a)}function l(a){if(u.hasClass(X))return k;clearTimeout(h.timers.inactive);h.timers.inactive=setTimeout(function(){h.hide(a)},n.hide.inactive)}function G(a){h.rendered&&0<u[0].offsetWidth&&
h.reposition(a)}var q=n.position,m={show:n.show.target,hide:n.hide.target,viewport:b(q.viewport),document:b(a),body:b(a.body),window:b(d)},s=b.trim(""+n.show.event).split(" "),v=b.trim(""+n.hide.event).split(" ");b.browser.msie&&parseInt(b.browser.version,10);u.bind("mouseenter"+z+" mouseleave"+z,function(a){var v="mouseenter"===a.type;v&&h.focus(a);u.toggleClass(ya,v)});/mouse(out|leave)/i.test(n.hide.event)&&"window"===n.hide.leave&&m.window.bind("mouseout"+z+" blur"+z,function(a){!/select|option/.test(a.target.nodeName)&&
!a.relatedTarget&&h.hide(a)});n.hide.fixed?(m.hide=m.hide.add(u),u.bind("mouseover"+z,function(){u.hasClass(X)||clearTimeout(h.timers.hide)})):/mouse(over|enter)/i.test(n.show.event)&&m.hide.bind("mouseleave"+z,function(a){clearTimeout(h.timers.show)});-1<(""+n.hide.event).indexOf("unfocus")&&q.container.closest("html").bind("mousedown"+z+" touchstart"+z,function(a){var v=b(a.target);h.rendered&&u.hasClass(X);var c=0<v.parents(aa).filter(u[0]).length;v[0]!==w[0]&&v[0]!==u[0]&&!c&&!w.has(v[0]).length&&
!v.attr("disabled")&&h.hide(a)});"number"==typeof n.hide.inactive&&(m.show.bind("qtip-"+P+"-inactive",l),b.each(L.inactiveEvents,function(a,v){m.hide.add(I.tooltip).bind(v+z+"-inactive",l)}));b.each(v,function(a,v){var c=b.inArray(v,s),d=b(m.hide);-1<c&&d.add(m.show).length===d.length||"unfocus"===v?(m.show.bind(v+z,function(a){0<u[0].offsetWidth?g(a):e(a)}),delete s[c]):m.hide.bind(v+z,g)});b.each(s,function(a,v){m.show.bind(v+z,e)});"number"==typeof n.hide.distance&&m.show.add(u).bind("mousemove"+
z,function(a){var v=Q.origin||{},b=n.hide.distance,c=Math.abs;(c(a.pageX-v.pageX)>=b||c(a.pageY-v.pageY)>=b)&&h.hide(a)});"mouse"===q.target&&(m.show.bind("mousemove"+z,c),q.adjust.mouse&&(n.hide.event&&(u.bind("mouseleave"+z,function(a){(a.relatedTarget||a.target)!==m.show[0]&&h.hide(a)}),I.target.bind("mouseenter"+z+" mouseleave"+z,function(a){Q.onTarget="mouseenter"===a.type})),m.document.bind("mousemove"+z,function(a){h.rendered&&Q.onTarget&&!u.hasClass(X)&&0<u[0].offsetWidth&&h.reposition(a||
...
break;case "offset":a=C.offset(u,n.position.container);break;default:a=s(a.toLowerCase()),a=a[0][a[1]],a=a.precedance?a.string():a}return a},set:function(a,c){var d=/^position\.(my|at|adjust|target|container)|style|content|show\.ready/i,e=k,w=h.checks,m;return"string"==typeof a?(m=a,a={},a[m]=c):a=b.extend(r,{},a),b.each(a,function(c,v){var M=s(c.toLowerCase()),ha;ha=M[0][M[1]];M[0][M[1]]="object"==typeof v&&v.nodeType?b(v):v;a[c]=[M[0],M[1],v,ha];e=d.test(c)||e}),g(n),f=1,b.each(a,function(a,v){var b,
c,d;for(b in w)for(c in w[b])if(d=RegExp(c,"i").exec(a))v.push(d),w[b][c].apply(h,v)}),f=0,h.rendered&&0<u[0].offsetWidth&&e&&h.reposition("mouse"===n.position.target?K:Q.event),h},toggle:function(d,f,e){function w(){d?(b.browser.msie&&u[0].style.removeAttribute("filter"),u.css("overflow",""),"string"==typeof m.autofocus&&b(m.autofocus,u).focus(),m.target.trigger("qtip-"+P+"-inactive")):u.css({display:"",visibility:"",opacity:"",left:"",top:""});h._triggerEvent(d?"visible":"hidden")}if(f){if(/over|enter/.test(f.type)&&
/out|leave/.test(Q.event.type)&&n.show.target.add(f.target).length===n.show.target.length&&u.has(f.relatedTarget).length)return h;Q.event=b.extend({},f)}if(!h.rendered)return d?h.render(1):h;var q=d?"show":"hide",m=n[q],g=n.position,v=n.content,M=0<u[0].offsetWidth,ha=d||1===m.target.length,ia=!f||2>m.target.length||Q.target[0]===f.target;return(typeof d).search("boolean|number")&&(d=!M),!u.is(":animated")&&M===d&&ia?h:h._triggerEvent(q,[90])?(b.attr(u[0],"aria-hidden",!d),d?(Q.origin=b.extend({},
ja),h.focus(f),b.isFunction(v.text)&&B(v.text,k),b.isFunction(v.title.text)&&y(v.title.text,k),!ua&&"mouse"===g.target&&g.adjust.mouse&&(b(a).bind("mousemove.qtip",c),ua=r),h.reposition(f,e),!m.solo||b(aa,m.solo).not(u).qtip("hide",b.Event("tooltipsolo"))):(clearTimeout(h.timers.show),delete Q.origin,ua&&!b(aa+'[tracking="true"]:visible',m.solo).not(u).length&&(b(a).unbind("mousemove.qtip"),ua=k),h.blur(f)),m.effect===k||ha===k?(u[q](),w.call(u)):b.isFunction(m.effect)?(u.stop(1,1),m.effect.call(u,
h),u.queue("fx",function(a){w();a()})):u.fadeTo(90,d?1:0,w),d&&m.target.trigger("qtip-"+P+"-inactive"),h):h},show:function(a){return h.toggle(r,a)},hide:function(a){return h.toggle(k,a)},focus:function(a){if(!h.rendered)return h;var c=b(aa),d=parseInt(u[0].style.zIndex,10),f=L.zindex+c.length;a=b.extend({},a);return u.hasClass(pa)||h._triggerEvent("focus",[f],a)&&(d!==f&&(c.each(function(){this.style.zIndex>d&&(this.style.zIndex-=1)}),c.filter("."+pa).qtip("blur",a)),u.addClass(pa)[0].style.zIndex=
f),h},blur:function(a){return u.removeClass(pa),h._triggerEvent("blur",[u.css("zIndex")],a),h},reposition:function(c,e){if(!h.rendered||f)return h;f=1;var w=n.position.target,P=n.position,q=P.my,m=P.at,g=P.adjust,v=g.method.split(" "),M=u.outerWidth(k),ha=u.outerHeight(k),ia=0,sa=0,l=u.css("position"),G=P.viewport,z={left:0,top:0},s=P.container,r=0<u[0].offsetWidth,y=c&&"scroll"===c.type,J=b(d),U,X;if(b.isArray(w)&&2===w.length)m={x:W,y:V},z={left:w[0],top:w[1]};else if("mouse"===w&&(c&&c.pageX||
...
"event"!==w&&(Q.target=b(w.jquery?w:I.target));w=Q.target;w=b(w).eq(0);if(0===w.length)return h;w[0]===a||w[0]===d?(ia=C.iOS?d.innerWidth:w.width(),sa=C.iOS?d.innerHeight:w.height(),w[0]===d&&(z={top:(G||w).scrollTop(),left:(G||w).scrollLeft()})):C.imagemap&&w.is("area")?U=C.imagemap(h,w,m,C.viewport?v:k):C.svg&&w[0].ownerSVGElement?U=C.svg(h,w,m,C.viewport?v:k):(ia=w.outerWidth(k),sa=w.outerHeight(k),z=C.offset(w,s));U&&(ia=U.width,sa=U.height,X=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
C.iOS&&4.33>C.iOS||!C.iOS&&"fixed"===l)z.left-=J.scrollLeft(),z.top-=J.scrollTop();z.left+=m.x===Y?ia:m.x===N?ia/2:0;z.top+=m.y===x?sa:m.y===N?sa/2:0}return z.left+=g.x+(q.x===Y?-M:q.x===N?-M/2:0),z.top+=g.y+(q.y===x?-ha:q.y===N?-ha/2:0),C.viewport?(z.adjusted=C.viewport(h,z,P,ia,sa,M,ha),X&&z.adjusted.left&&(z.left+=X.left),X&&z.adjusted.top&&(z.top+=X.top)):z.adjusted={left:0,top:0},h._triggerEvent("move",[z,G.elem||G],c)?(delete z.adjusted,e===k||!r||isNaN(z.left)||isNaN(z.top)||"mouse"===w||!b.isFunction(P.effect)?
u.css(z):b.isFunction(P.effect)&&(P.effect.call(u,h,b.extend({},z)),u.queue(function(a){b(this).css({opacity:"",height:""});b.browser.msie&&this.style.removeAttribute("filter");a()})),f=0,h):h},disable:function(a){return"boolean"!=typeof a&&(a=!u.hasClass(X)&&!Q.disabled),h.rendered?(u.toggleClass(X,a),b.attr(u[0],"aria-disabled",a)):Q.disabled=!!a,h},enable:function(){return h.disable(k)},destroy:function(){var a=w[0],c=b.attr(a,la),d=w.data("qtip");h.destroyed=r;h.rendered&&(u.stop(1,0).remove(),
b.each(h.plugins,function(){this.destroy&&this.destroy()}));clearTimeout(h.timers.show);clearTimeout(h.timers.hide);J();if(!d||h===d)b.removeData(a,"qtip"),n.suppress&&c&&(b.attr(a,"title",c),w.removeAttr(la)),w.removeAttr("aria-describedby");return w.unbind(".qtip-"+P),delete S[h.id],w}})}function na(c,d){var e,U,s,E,O,t=b(this),A=b(a.body),x=this===a?A:t;U=t.metadata?t.metadata(d.metadata):K;E="html5"===d.metadata.type&&U?U[d.metadata.name]:K;var y=t.data(d.metadata.name||"qtipopts");try{y="string"==
...
var r=!0,k=!1,K=null,ga="x",da="y",ka="width",R="height",V="top",W="left",x="bottom",Y="right",N="center",qa="flipinvert",ea="shift",L,C,ja,ca="qtip",S={},T=["ui-widget","ui-tooltip"],aa="div.qtip."+ca,wa=ca+"-default",pa=ca+"-focus",ya=ca+"-hover",la="oldtitle",ua;L=b.fn.qtip=function(a,c,d){var l=(""+a).toLowerCase(),s=K,E=b.makeArray(arguments).slice(1),x=E[E.length-1],t=this[0]?b.data(this[0],"qtip"):K;if(!arguments.length&&t||"api"===l)return t;if("string"==typeof a)return this.each(function(){var a=
b.data(this,"qtip");if(!a)return r;x&&x.timeStamp&&(a.cache.event=x);if("option"!==l&&"options"!==l||!c)a[l]&&a[l].apply(a[l],E);else{if(!b.isPlainObject(c)&&d===e)return s=a.get(c),k;a.set(c,d)}}),s!==K?s:this;if("object"==typeof a||!arguments.length)return t=g(b.extend(r,{},a)),L.bind.call(this,t,x)};L.bind=function(a,d){return this.each(function(g){function l(a){function c(){A.render("object"==typeof a||s.show.ready);E.show.add(E.hide).unbind(t)}if(A.cache.disabled)return k;A.cache.event=b.extend({},
a);A.cache.target=a?b(a.target):[e];0<s.show.delay?(clearTimeout(A.timers.show),A.timers.show=setTimeout(c,s.show.delay),x.show!==x.hide&&E.hide.bind(x.hide,function(){clearTimeout(A.timers.show)})):c()}var s,E,x,t,A;g=b.isArray(a.id)?a.id[g]:a.id;g=!g||g===k||1>g.length||S[g]?L.nextid++:S[g]=g;t=".qtip-"+g+"-create";A=na.call(this,g,a);if(A===k)return r;s=A.options;b.each(C,function(){"initialize"===this.initialize&&this(A)});E={show:s.show.target,hide:s.hide.target};x={show:b.trim(""+s.show.event).replace(/ /g,
t+" ")+t,hide:b.trim(""+s.hide.event).replace(/ /g,t+" ")+t};/mouse(over|enter)/i.test(x.show)&&!/mouse(out|leave)/i.test(x.hide)&&(x.hide+=" mouseleave"+t);E.show.bind("mousemove"+t,function(a){c(a);A.cache.onTarget=r});E.show.bind(x.show,l);(s.show.ready||s.prerender)&&l(d)}).attr("data-hasqtip",r)};C=L.plugins={Corner:function(a){a=(""+a).replace(/([A-Z])/," $1").replace(/middle/gi,N).toLowerCase();this.x=(a.match(/left|right/i)||a.match(/center/)||["inherit"])[0].toLowerCase();this.y=(a.match(/top|bottom|center/i)||
...
y.width/2,top:y.position.top+y.height/2}:(a[d+k]||(y.position=g(y,A.slice(),d),e&&("flip"===e[0]||"flip"===e[1])&&(y.offset=g(y,A.slice(),{x:d.x===W?Y:d.x===Y?W:N,y:d.y===V?x:d.y===x?V:N}),y.offset.left-=y.position.left,y.offset.top-=y.position.top),a[d+k]=y),y=a[d+k]),y.width=y.height=0}return y.position.left+=C.left,y.position.top+=C.top,y};C.ie6=function(a){var c=b.browser,d=a.plugins.ie6;return!c.msie||"6"!==(""+c.version).charAt(0)?k:"object"==typeof d?d:a.plugins.ie6=new oa(a)};C.ie6.initialize=
...
f.set("minuteBufferSnapToSched",!0),TASK_DB.userId=f.get("userID")),f.get("minuteBufferSnapToSched")&&E(),f.get("restrict_sched")?E(function(c){var b;void 0==c&&(b="Today is not a work day");var d=new Date,d=Date.parse("01/01/2011 "+d.getHours()+":"+d.getMinutes()),M=Date.parse("01/01/2011 "+c.start),e=Date.parse("01/01/2011 "+c.stop),ha=f.get("minuteBuffer");null!=ha&&(M.setMinutes(M.getMinutes()-ha),e.setMinutes(e.getMinutes()+ha));void 0==c||M>d||d>e?(b=b?b:"Please return at "+c.start,TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Not scheduled to work</div></h3><div class="content"><p>'+
b+'.</p><a href="#" class="btn btn-primary logout cancel" title="Logout">Logout</a></div>')):a()}):(a&&a(),ra()))});c.fail(function(a,c,b){})}}function e(){var a={url:T()+"user/count",type:"get",data:{},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uCount,"+JSON.stringify(a.error)),m.hide(),q.hide()):a=a.data});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},e,"uCount")})}function b(){var a={url:T()+"user/search?ID=$$USER.ID&fields=customer:name,customer:fullUsers,customer:limitedUsers,customer:teamUsers",
type:"get",data:{},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uInfo,"+JSON.stringify(a.error)),m.hide(),q.hide()):a=a.data});a.fail(function(a,c,v){S({response:a.responseText,type:c,msg:v},b,"uInfo")})}function c(a,c,b){}function g(){if(fa||u||void 0!=f.get("host")&&"full"==getQueryString("view"))window.clearInterval(za),za=window.setInterval(function(){if(navigator.onLine&&(u||!document.hasFocus()))jQuery("#search-bar").is(":visible")&&
...
jQuery("#button-recent").removeClass("btn");pa();jQuery(a).toggleClass("btn")};jQuery("#button-current").live("click",function(){H=[];a("#button-current");f.set("view","current");w()});jQuery("#button-requested").live("click",function(){H=[];a("#button-requested");f.set("view","requested");n()});jQuery("#button-all").live("click",function(){H=[];a("#button-all");f.set("view","all");aa()});jQuery("#button-team").live("click",function(){H=[];a("#button-team");f.set("view","team");ya()});jQuery("#button-role").live("click",
...
e.error)e.error["class"]==h[0]?k(function(){aa(a,b,d)}):c(D[0],"addAllAssignedTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else{e=e.data;var g=!0,l=f.get("view");"all"!=l&&"role"!=l&&(g=!1);if("opTask"!=a)aa("opTask",b,d),r(function(){var a=e.length;if(a){for(var c=f.get("hour_types"),b=[],d=0;d<a;d++)if(-1!=e[d].task.permissions.actions.indexOf("ADD_HOURS")&&-1!=e[d].task.permissions.actions.indexOf("CHANGE_STATUS")){var v=!1,M=e[d].task.project.hourTypes.length;if(M)for(var l=0;l<M;l++){if(void 0!=
c[e[d].task.project.hourTypes[l].ID]){v=!0;break}}else v=!0;v&&(TASK_DB.insert(e[d].task.name,{syncID:e[d].task.ID,details:e[d].task},g&&d==a-1),b[e[d].task.ID]=!0)}TASK_DB.running(function(a){void 0==b[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;ma=g;if(l){for(var ia=[],n=0;n<l;n++)-1!=e[n].opTask.permissions.actions.indexOf("ADD_HOURS")&&-1!=e[n].opTask.permissions.actions.indexOf("CHANGE_STATUS")&&(TASK_DB.insert(e[n].opTask.name,{syncID:e[n].opTask.ID,details:e[n].opTask},
g&&n==l-1),ia[e[n].opTask.ID]=!0);TASK_DB.running(function(a){void 0==ia[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}else TASK_CNTRL.showAll(e);b&&b(e)}}});e.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},aa,"addAllAssignedTasks-fail"+c+"-"+b+" "+a.responseText)})}function wa(a){m.show();var b={url:T("v5.0")+"user/search",type:"get",data:{fields:"role:*,roles:*",ID:f.get("userID")},dataType:"json"};x(b);b=jQuery.ajax(b);b.done(function(b){if(void 0!=b.error)b.error["class"]==
h[0]?k(function(){wa(a)}):c(D[0],"addAllRoleTasks,"+JSON.stringify(b.error)),m.hide(),q.hide();else if(b=b.data[0],0==b.length)a&&a(),m.hide(),q.hide();else{var d=jQuery("#role-filter"),e=function(){aa("task",function(){d.css("display","inline-block");m.hide();q.hide()})},g=function(a,c){f.set("currentRole",{name:a,ID:c});d.find("button").html("<strong>Role: </strong><span>"+a+'</span><span class="caret"></span>');e()},M=f.get("currentRole");void 0!=M?g(M.name,M.ID):void 0!=b.role&&g(b.role.name,
b.role.ID);if(void 0!=b.roles&&0!=b.roles.length)for(var M=d.find(".dropdown-menu").html("").first(),l=0,n=b.roles.length;l<n;l++)M.append('<li><a href="#" id="'+b.roles[l].ID+'">'+b.roles[l].name+"</a></li>"),M.find("#"+b.roles[l].ID).click(function(){g(jQuery(this).text(),jQuery(this).attr("id"))});else pa(),m.hide(),q.hide(),Z(),TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Roles</div></h3><div class="content"><div><p>No additional roles available.</strong></h3><div><a href="#" class="btn btn-primary cancel" title="Ok">Ok</a><div>')}});
b.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},wa,"addAllRoleTasks")})}function pa(){var a=jQuery("#role-filter");a.hide();a.find(".dropdown-menu").first().html("")}function ya(a){m.show();var b={url:T()+"team/search",type:"get",data:{fields:"ID","users:ID":f.get("userID")},dataType:"json"};x(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){ya(a)}):c(D[0],"addAllGroupTasks,"+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,0==b.length?
(a?a():jQuery("#form-list").hide(),m.hide(),q.hide(),TASK_CNTRL.showNoTaskWindow()):r(function(){for(var c=b.length,d=function(b,d){if(0==--c){var e=va.length;ma=!0;if(0==e)TASK_CNTRL.showAll(va);else{for(var f=0;f<e;f++)TASK_DB.insert(va[f].name,{syncID:va[f].ID,details:va[f]},f==e-1);va=[];a&&a()}}},e=0;e<c;e++)la("task",b[e].ID,d)}))});b.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ya,"addAllGroupTasks")})}function la(a,b,d){a=void 0==a?"task":a;m.show();var e={url:T()+a+"/search?project:status=INF",
...
c(D[0],"addRecentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=Object.keys(a.data),0==a.length?(ma=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){ma=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))}))});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ua,"addRecentTasks")})}function w(){m.show();var a;f.get("ent")?aa("task",function(a){ma=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))},!0):(a={url:T()+"work/myWork",type:"get",data:{fields:"parent,priority,project,plannedCompletionDate"},
...
"setTaskDone,"+b+", "+c.responseText)})}function F(a){m.show();var c={url:T()+a.details.objCode.toLowerCase()+"/"+a.syncID,data:{status:"INP"},type:"PUT",dataType:"json"};fa&&x(c);c=jQuery.ajax(c);c.done(function(c){void 0!=c.error?(c.error["class"]==h[0]?k(function(){F(a)}):S({response:c.error,type:null,msg:null},function(){F(a)},"setTaskInProgress,"+a.syncID),m.hide(),q.hide()):(m.hide(),q.hide(),!0==f.get("logout_on_select")&&oa())});c.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},
...
jQuery("#storelink").text("Trial Expired - AtAppStore"));f.get("hideTaskButtons")&&(jQuery(".timer").addClass("timer-tall"),jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".task").remove(),jQuery(".view").remove(),jQuery(".date").remove(),jQuery("#sort-date").remove());if(ma){var a=jQuery("#form-list"),c=f.get("vpos");void 0!=c&&a.scrollTop(c);"team"==f.get("view")&&!0==f.get("hide_team_done")&&jQuery(".done").remove();I?f.get("allowOffline")&&(jQuery(".done").remove(),jQuery(".percent").remove(),
jQuery(".view").remove(),jQuery("#bar").hide()):jQuery("#bar").show();c=f.get("sort");void 0==c&&(c="date");jQuery("#sort-"+c).click();jQuery("#search-bar").is(":visible")&&jQuery("#search").click();jQuery("#loading").hide();a.show();a=jQuery("#holder")[0];a.style.display="none";a.style.display="block";m.hide();ma=!1}});OB.postItemRender.add(function(a){if(0==jQuery("#item"+a.ID+" #buttons .view").length){var c=jQuery("#item"+a.ID+" #buttons"),b=["",{label:"Low",css:"default"},{label:"Normal",css:"success"},
...
a.details.description).attr("title",a.details.description)}});OB.remove.add(function(a){jQuery("#item"+a).remove()});OB.reset.add(function(a,c){OB.update.dispatch(c,-c.time);jQuery("#item"+a+" .timer").text(TASK_CNTRL.hms(0))});var ma=!1,q,m,H=[]}();var QUERY_LOOKUP=null;function getQueryString(d){if(null==QUERY_LOOKUP){var a=location.search.substring(1),e=/([^&=]+)=([^&]*)/g,b;for(QUERY_LOOKUP={};b=e.exec(a);)QUERY_LOOKUP[decodeURIComponent(b[1])]=decodeURIComponent(b[2])}return d in QUERY_LOOKUP?QUERY_LOOKUP[d]:null}
