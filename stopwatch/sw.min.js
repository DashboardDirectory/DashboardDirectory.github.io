...
null,k)})}function B(a,c,d){function f(a){function d(w){w&&(delete L[w.src],clearTimeout(h.timers.img[w.src]),b(w).unbind(z));b.isEmptyObject(L)&&(c!==k&&h.reposition(Q.event),a())}var w,L={};if(0===(w=q.find("img[src]:not([height]):not([width])")).length)return d();w.each(function(a,w){if(L[w.src]===e){var c=0;(function Ca(){if(w.height||w.width||3<c)return d(w);c+=1;h.timers.img[w.src]=setTimeout(Ca,700)})();b(w).bind("error"+z+" load"+z,function(){d(this)});L[w.src]=w}})}var q=I.content;return!h.rendered||
!a?k:(b.isFunction(a)&&(a=a.call(v,Q.event,h)||""),d!==k&&F(n.content.deferred),a.jquery&&0<a.length?q.empty().append(a.css({display:"block"})):q.html(a),0>h.rendered?u.queue("fx",f):(fa=0,f(b.noop)),h)}function G(){function e(a){if(u.hasClass(X))return k;clearTimeout(h.timers.show);clearTimeout(h.timers.hide);var w=function(){h.toggle(r,a)};0<n.show.delay?h.timers.show=setTimeout(w,n.show.delay):w()}function g(a){if(u.hasClass(X)||f||fa)return k;var w=b(a.relatedTarget||a.target),c=w.closest(aa)[0]===
u[0],w=w[0]===m.show[0];clearTimeout(h.timers.show);clearTimeout(h.timers.hide);if("mouse"===q.target&&c||n.hide.fixed&&/mouse(out|leave|move)/.test(a.type)&&(c||w))try{a.preventDefault(),a.stopImmediatePropagation()}catch(d){}else 0<n.hide.delay?h.timers.hide=setTimeout(function(){h.hide(a)},n.hide.delay):h.hide(a)}function l(a){if(u.hasClass(X))return k;clearTimeout(h.timers.inactive);h.timers.inactive=setTimeout(function(){h.hide(a)},n.hide.inactive)}function G(a){h.rendered&&0<u[0].offsetWidth&&
h.reposition(a)}var q=n.position,m={show:n.show.target,hide:n.hide.target,viewport:b(q.viewport),document:b(a),body:b(a.body),window:b(d)},s=b.trim(""+n.show.event).split(" "),w=b.trim(""+n.hide.event).split(" ");b.browser.msie&&parseInt(b.browser.version,10);u.bind("mouseenter"+z+" mouseleave"+z,function(a){var w="mouseenter"===a.type;w&&h.focus(a);u.toggleClass(ya,w)});/mouse(out|leave)/i.test(n.hide.event)&&"window"===n.hide.leave&&m.window.bind("mouseout"+z+" blur"+z,function(a){!/select|option/.test(a.target.nodeName)&&
!a.relatedTarget&&h.hide(a)});n.hide.fixed?(m.hide=m.hide.add(u),u.bind("mouseover"+z,function(){u.hasClass(X)||clearTimeout(h.timers.hide)})):/mouse(over|enter)/i.test(n.show.event)&&m.hide.bind("mouseleave"+z,function(a){clearTimeout(h.timers.show)});-1<(""+n.hide.event).indexOf("unfocus")&&q.container.closest("html").bind("mousedown"+z+" touchstart"+z,function(a){var w=b(a.target);h.rendered&&u.hasClass(X);var c=0<w.parents(aa).filter(u[0]).length;w[0]!==v[0]&&w[0]!==u[0]&&!c&&!v.has(w[0]).length&&
!w.attr("disabled")&&h.hide(a)});"number"==typeof n.hide.inactive&&(m.show.bind("qtip-"+P+"-inactive",l),b.each(M.inactiveEvents,function(a,w){m.hide.add(I.tooltip).bind(w+z+"-inactive",l)}));b.each(w,function(a,w){var c=b.inArray(w,s),d=b(m.hide);-1<c&&d.add(m.show).length===d.length||"unfocus"===w?(m.show.bind(w+z,function(a){0<u[0].offsetWidth?g(a):e(a)}),delete s[c]):m.hide.bind(w+z,g)});b.each(s,function(a,w){m.show.bind(w+z,e)});"number"==typeof n.hide.distance&&m.show.add(u).bind("mousemove"+
z,function(a){var w=Q.origin||{},b=n.hide.distance,c=Math.abs;(c(a.pageX-w.pageX)>=b||c(a.pageY-w.pageY)>=b)&&h.hide(a)});"mouse"===q.target&&(m.show.bind("mousemove"+z,c),q.adjust.mouse&&(n.hide.event&&(u.bind("mouseleave"+z,function(a){(a.relatedTarget||a.target)!==m.show[0]&&h.hide(a)}),I.target.bind("mouseenter"+z+" mouseleave"+z,function(a){Q.onTarget="mouseenter"===a.type})),m.document.bind("mousemove"+z,function(a){h.rendered&&Q.onTarget&&!u.hasClass(X)&&0<u[0].offsetWidth&&h.reposition(a||
...
break;case "offset":a=C.offset(u,n.position.container);break;default:a=s(a.toLowerCase()),a=a[0][a[1]],a=a.precedance?a.string():a}return a},set:function(a,c){var d=/^position\.(my|at|adjust|target|container)|style|content|show\.ready/i,e=k,v=h.checks,m;return"string"==typeof a?(m=a,a={},a[m]=c):a=b.extend(r,{},a),b.each(a,function(c,w){var L=s(c.toLowerCase()),ha;ha=L[0][L[1]];L[0][L[1]]="object"==typeof w&&w.nodeType?b(w):w;a[c]=[L[0],L[1],w,ha];e=d.test(c)||e}),g(n),f=1,b.each(a,function(a,w){var b,
c,d;for(b in v)for(c in v[b])if(d=RegExp(c,"i").exec(a))w.push(d),v[b][c].apply(h,w)}),f=0,h.rendered&&0<u[0].offsetWidth&&e&&h.reposition("mouse"===n.position.target?K:Q.event),h},toggle:function(d,f,e){function v(){d?(b.browser.msie&&u[0].style.removeAttribute("filter"),u.css("overflow",""),"string"==typeof m.autofocus&&b(m.autofocus,u).focus(),m.target.trigger("qtip-"+P+"-inactive")):u.css({display:"",visibility:"",opacity:"",left:"",top:""});h._triggerEvent(d?"visible":"hidden")}if(f){if(/over|enter/.test(f.type)&&
/out|leave/.test(Q.event.type)&&n.show.target.add(f.target).length===n.show.target.length&&u.has(f.relatedTarget).length)return h;Q.event=b.extend({},f)}if(!h.rendered)return d?h.render(1):h;var q=d?"show":"hide",m=n[q],g=n.position,w=n.content,L=0<u[0].offsetWidth,ha=d||1===m.target.length,ia=!f||2>m.target.length||Q.target[0]===f.target;return(typeof d).search("boolean|number")&&(d=!L),!u.is(":animated")&&L===d&&ia?h:h._triggerEvent(q,[90])?(b.attr(u[0],"aria-hidden",!d),d?(Q.origin=b.extend({},
ja),h.focus(f),b.isFunction(w.text)&&B(w.text,k),b.isFunction(w.title.text)&&y(w.title.text,k),!ua&&"mouse"===g.target&&g.adjust.mouse&&(b(a).bind("mousemove.qtip",c),ua=r),h.reposition(f,e),!m.solo||b(aa,m.solo).not(u).qtip("hide",b.Event("tooltipsolo"))):(clearTimeout(h.timers.show),delete Q.origin,ua&&!b(aa+'[tracking="true"]:visible',m.solo).not(u).length&&(b(a).unbind("mousemove.qtip"),ua=k),h.blur(f)),m.effect===k||ha===k?(u[q](),v.call(u)):b.isFunction(m.effect)?(u.stop(1,1),m.effect.call(u,
h),u.queue("fx",function(a){v();a()})):u.fadeTo(90,d?1:0,v),d&&m.target.trigger("qtip-"+P+"-inactive"),h):h},show:function(a){return h.toggle(r,a)},hide:function(a){return h.toggle(k,a)},focus:function(a){if(!h.rendered)return h;var c=b(aa),d=parseInt(u[0].style.zIndex,10),f=M.zindex+c.length;a=b.extend({},a);return u.hasClass(qa)||h._triggerEvent("focus",[f],a)&&(d!==f&&(c.each(function(){this.style.zIndex>d&&(this.style.zIndex-=1)}),c.filter("."+qa).qtip("blur",a)),u.addClass(qa)[0].style.zIndex=
f),h},blur:function(a){return u.removeClass(qa),h._triggerEvent("blur",[u.css("zIndex")],a),h},reposition:function(c,e){if(!h.rendered||f)return h;f=1;var v=n.position.target,P=n.position,q=P.my,m=P.at,g=P.adjust,w=g.method.split(" "),L=u.outerWidth(k),ha=u.outerHeight(k),ia=0,ma=0,l=u.css("position"),G=P.viewport,z={left:0,top:0},s=P.container,r=0<u[0].offsetWidth,y=c&&"scroll"===c.type,J=b(d),U,X;if(b.isArray(v)&&2===v.length)m={x:W,y:V},z={left:v[0],top:v[1]};else if("mouse"===v&&(c&&c.pageX||
...
"event"!==v&&(Q.target=b(v.jquery?v:I.target));v=Q.target;v=b(v).eq(0);if(0===v.length)return h;v[0]===a||v[0]===d?(ia=C.iOS?d.innerWidth:v.width(),ma=C.iOS?d.innerHeight:v.height(),v[0]===d&&(z={top:(G||v).scrollTop(),left:(G||v).scrollLeft()})):C.imagemap&&v.is("area")?U=C.imagemap(h,v,m,C.viewport?w:k):C.svg&&v[0].ownerSVGElement?U=C.svg(h,v,m,C.viewport?w:k):(ia=v.outerWidth(k),ma=v.outerHeight(k),z=C.offset(v,s));U&&(ia=U.width,ma=U.height,X=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
...
"border-"+a[a.precedance]+"-color",n="transparent",v=G.titlebar,y=v&&(a.y===V||a.y===N&&f.position().top+D.height/2+B.offset<v.outerHeight(r))?v:G.content;l(function(){d=c(f,"background-color")||c(y,"background-color")||c(G.content,"background-color")||c(J,"background-color")||f.css("background-color");g=c(f,h,"color")||c(y,h,"color")||c(G.content,h,"color")||c(J,h,"color")||J.css(h);b("*",f).add(f).css("cssText","background-color:"+n+" !important;border:0 !important;")})}function M(a){var c=a.precedance===
...
a.precedance===ga?(D.width=B.height,D.height=B.width):(D.width=B.width,D.height=B.height);G.tip.css({width:v=D.width,height:l=D.height});A(a);"transparent"!==g?(f=O(a,K),0===B.border&&0<f&&(d=g),F.border=f=B.border!==r?B.border:f):F.border=f=0;q=sa(s,v,l);F.size=H=M(a);e.css(H).css("line-height",H.height+"px");a.precedance===da?m=[E(s.x===W?f:s.x===Y?H.width-v-f:(H.width-v)/2),E(s.y===V?H.height-l:0)]:m=[E(s.x===W?H.width-v:0),E(s.y===V?f:s.y===x?H.height-l-f:(H.height-l)/2)];fa?(n.attr(H),t=n[0].getContext("2d"),
...
10)?2:1:0,n.css({coordsize:v+f+" "+(l+f),antialias:""+(-1<s.string().indexOf(N)),left:m[0],top:m[1],width:v+f,height:l+f}).each(function(a){var c=b(this);c[c.prop?"prop":"attr"]({coordsize:v+f+" "+(l+f),path:q,fillcolor:d,filled:!!a,stroked:!a}).toggle(!!f||!!a);!a&&""===c.html()&&c.html(y("stroke",'weight="'+2*f+'px" color="'+g+'" miterlimit="1000" joinstyle="miter"'))}));c!==k&&F.position(a)},position:function(a){var c=G.tip,d={},f=Math.max(0,B.offset),e,h,n;return B.corner===k||!c?k:(a=a||F.corner,
e=a.precedance,h=M(a),n=[a.x,a.y],e===ga&&n.reverse(),b.each(n,function(c,b){var n,v,g;b===N?(n=e===da?W:V,d[n]="50%",d["margin-"+n]=-Math.round(h[e===da?ka:R]/2)+f):(n=O(a,b),v=O(a,b,G.content),g=t(a),d[b]=c?v:f+(g>n?g:-n))}),d[a[e]]-=h[e===ga?ka:R],c.css({top:"",bottom:"",left:"",right:"",margin:""}).css(d),d)},destroy:function(){G.tip&&G.tip.remove();G.tip=!1;J.unbind(".qtip-tip")}});F.init()}function ta(c){function e(){F=b(y,t).not("[disabled]").map(function(){return"function"==typeof this.focus?
...
l=c.options,s=g.tooltip,E=".ie6-"+c.id,O=1>b("select, object").length,t=0,A=k,x;c.checks.ie6={"^content|style$":function(a,c,b){redraw()}};b.extend(e,{init:function(){var c=b(d),v;O&&(g.bgiframe=b('<iframe class="qtip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0); -ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";"></iframe>'),g.bgiframe.appendTo(s),s.bind("tooltipmove"+E,e.adjustBGIFrame));
...
type:"get",data:{},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uInfo,"+JSON.stringify(a.error)),m.hide(),q.hide()):a=a.data});a.fail(function(a,c,w){S({response:a.responseText,type:c,msg:w},b,"uInfo")})}function c(a,c,b){}function g(){if(fa||u||void 0!=f.get("host")&&"full"==getQueryString("view"))window.clearInterval(za),za=window.setInterval(function(){if(navigator.onLine&&(u||!document.hasFocus()))jQuery("#search-bar").is(":visible")&&
...
jQuery("#button-recent").removeClass("btn");qa();jQuery(a).toggleClass("btn")};jQuery("#button-current").live("click",function(){H=[];a("#button-current");f.set("view","current");v()});jQuery("#button-requested").live("click",function(){H=[];a("#button-requested");f.set("view","requested");n()});jQuery("#button-all").live("click",function(){H=[];a("#button-all");f.set("view","all");aa()});jQuery("#button-team").live("click",function(){H=[];a("#button-team");f.set("view","team");ya()});jQuery("#button-role").live("click",
...
c.slideDown(400,function(){});jQuery("#button-qrlogin").unbind("click").click(function(c){var b=jQuery("#form-qrlogin");c.preventDefault();q.show();N(f.get("username"),f.get("password"),Y(f.get("host")),function(){ignoreForQR();jQuery("#bar").show();u?b.hide():b.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()},function(){resetForQR()})})}function ka(a){m.show();jQuery("#loading").hide();window.clearInterval(za);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();
...
q.show();N(jQuery.trim(c.find(":input[name='username']").val()),jQuery.trim(c.find(":input[name='password']").val()),Y(jQuery.trim(c.find(":input[name='host']").val())),function(){jQuery("#bar").show();u?c.hide():c.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()})})}function R(a,c){var b=new Date;a.dateStr=b.getFullYear()+"-"+(b.getMonth()+1)+"-"+b.getDate();a.duration=c;navigator.onLine?J(a,G):(b=f.get("offlineTasks"),void 0==b&&(b=[]),b.push(a),f.set("offlineTasks",
b),m.hide(),q.hide(),!0==f.get("logout_on_select")&&pa())}function V(a){return"jan. feb. mar. apr. may june jul. aug. sep. oct. nov. dec.".split(" ")[a]}function W(a){return void 0!=a.details.project?a.details.project.name.split(",").join(" ")+","+a.name.split(",").join(" "):a.details}function x(a,c){void 0==c&&(c=1E4);a.timeout=c;fa&&(!u&&!z)&&(a.dataType="jsonp",a.crossDomain=!0,a.jsonp="jsonp",a.cache=!0);fa&&"PUT"==a.type&&(a.data.method="put");var b=f.get("sessionID");f.get("ent")?("PUT"==a.type&&
...
e.error)e.error["class"]==h[0]?k(function(){aa(a,b,d)}):c(D[0],"addAllAssignedTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else{e=e.data;var g=!0,l=f.get("view");"all"!=l&&"role"!=l&&(g=!1);var ia=f.get("hour_types");if("opTask"!=a)aa("opTask",b,d),r(function(){var a=e.length;if(a){for(var c=[],b=0;b<a;b++){var d=e[b].task;if(-1!=d.permissions.actions.indexOf("ADD_HOURS")&&-1!=d.permissions.actions.indexOf("CHANGE_STATUS")){var w=!1,f=d.project.hourTypes.length;if(f)for(var L=0;L<f;L++){if(void 0!=
ia[d.project.hourTypes[L].ID]){w=!0;break}}else w=!0;w&&(TASK_DB.insert(d.name,{syncID:d.ID,details:d},g&&b==a-1),c[d.ID]=!0)}}TASK_DB.running(function(a){void 0==c[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;na=g;if(l){for(var n=[],s=0;s<l;s++){var t=e[s].opTask;if(-1!=t.permissions.actions.indexOf("ADD_HOURS")&&-1!=t.permissions.actions.indexOf("CHANGE_STATUS")){var ma=!1,u=t.project.hourTypes.length;if(u)for(var v=0;v<u;v++){if(void 0!=ia[t.project.hourTypes[v].ID]){ma=
...
c(D[0],"addRecentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=Object.keys(a.data),0==a.length?(na=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){na=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))}))});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ua,"addRecentTasks")})}function v(){m.show();var a;f.get("ent")?aa("task",function(a){na=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))},!0):(a={url:T()+"work/myWork",type:"get",data:{fields:"parent,priority,project,plannedCompletionDate"},
dataType:"json"},x(a),a=jQuery.ajax(a),a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){v()}):c(D[0],"addCurrentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,0==a.length?(na=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){var c=a.length;na=!0;for(var b=0;b<c;b++)TASK_DB.insert(a[b].name,{syncID:a[b].ID,details:a[b]},b==c-1)}))}),a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},v,"addCurrentTasks")}))}function n(){m.show();var a={url:T()+
...
4)))a=!0;return a}function s(a,c){1==jQuery("#hourTypeSelector").children().length?(a.details.hourTypeID=jQuery("#hourTypeSelector").children()[0].value,TASK_DB.updateDetails(a.ID,a.details),c(a)):(TASK_CNTRL.showWindow(null,"#hourTypeWindow"),jQuery("#done_hourType").unbind("click").bind("click",function(b){b.preventDefault();a.details.hourTypeID=jQuery("#hourTypeSelector").find(":selected").val();TASK_DB.updateDetails(a.ID,a.details);c(a);TASK_CNTRL.hideWindow("#hourTypeWindow")}))}function E(a){var b=
...
jQuery("#storelink").text("Trial Expired - AtAppStore"));f.get("hideTaskButtons")&&(jQuery(".timer").addClass("timer-tall"),jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".task").remove(),jQuery(".view").remove(),jQuery(".date").remove(),jQuery("#sort-date").remove());if(na){var a=jQuery("#form-list"),c=f.get("vpos");void 0!=c&&a.scrollTop(c);"team"==f.get("view")&&!0==f.get("hide_team_done")&&jQuery(".done").remove();I?f.get("allowOffline")&&(jQuery(".done").remove(),jQuery(".percent").remove(),
jQuery(".view").remove(),jQuery("#bar").hide()):jQuery("#bar").show();c=f.get("sort");void 0==c&&(c="date");jQuery("#sort-"+c).click();jQuery("#search-bar").is(":visible")&&jQuery("#search").click();jQuery("#loading").hide();a.show();a=jQuery("#holder")[0];a.style.display="none";a.style.display="block";m.hide();na=!1}});OB.postItemRender.add(function(a){if(0==jQuery("#item"+a.ID+" #buttons .view").length){var c=jQuery("#item"+a.ID+" #buttons"),b=["",{label:"Low",css:"default"},{label:"Normal",css:"success"},
{label:"High",css:"warning"},{label:"Urgent",css:"danger"}];"GEN"!=a.details.objCode&&(void 0!=a.details.priority&&0!=a.details.priority)&&(b=b[a.details.priority],c.append('<span class="task priority '+b.css+'" title="'+b.label+'" data-priority="'+a.details.priority+'">'+b.label[0]+"</span> "));var d;"GEN"==a.details.objCode?(d="General",jQuery("#item"+a.ID).css({background:"aliceblue"})):"OPTASK"==a.details.objCode?(d="Issue",c.append('<span class="issue" title="Issue">I</span> ')):"TASK"==a.details.objCode&&
(d="Task",c.append('<span class="task" title="Task">T</span> '));"General"!=d&&(c.append('<a href="https://'+f.get("host")+"."+f.get("domain")+"/"+d.toLowerCase()+"/view?ID="+a.syncID+'" target="_blank" class="btn btn-mini view" rel="'+a.ID+'" title="View '+d+'">View</a> '),c.append('<a href="#" class="btn btn-mini done" rel="'+a.ID+'" title="Done '+d+'">Done</a> '));"Task"==d&&void 0!=a.details.percentComplete&&c.append('<a href="#" class="btn btn-mini percent" rel="'+a.ID+'" title="Percent Completed">'+
a.details.percentComplete+"%</a> ");c="#D2E0FF";b='<span class="date" style="background-color: #EEE;"><span class="days" data-days="9999"></span></span>';if("General"!=d&&void 0!=a.details.plannedCompletionDate){var b=a.details.plannedCompletionDate.split("T")[0],b=b.split("-"),b=new Date(b[0],Number(b[1])-1,b[2]),e=new Date;e.setHours(0);e.setMinutes(0);e.setSeconds(0);e=Math.round((b-e)/1E3/60/60/24);0>e?c="#FFCDCD":1>=e&&(c="#FFD7AB");var g;g=0<e?"days due: "+e:"days late: "+Math.abs(e);b='<div class="date" style="background-color: '+
c+';"><div class="days" data-days="'+e+'">'+g+"</div><div>"+V(b.getMonth())+" "+b.getDate()+", "+b.getFullYear()+"</div></div>"}c="";null!=a.details.parent&&(c="<br><br>Parent Task: <br>"+a.details.parent.name);jQuery("#item"+a.ID+" .name").attr("title",d+": <br>"+a.name+c);"General"!=d?(d=a.details.project.name,jQuery("#item"+a.ID+" .details").html(d).attr("title","Project: <br>"+d).attr("data-priority",a.details.priority),jQuery("#item"+a.ID+" .c6").after(b)):jQuery("#item"+a.ID+" .details").html("~"+
