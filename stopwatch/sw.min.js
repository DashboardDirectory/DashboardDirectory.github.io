...
/out|leave/.test(Q.event.type)&&n.show.target.add(f.target).length===n.show.target.length&&u.has(f.relatedTarget).length)return h;Q.event=b.extend({},f)}if(!h.rendered)return d?h.render(1):h;var q=d?"show":"hide",m=n[q],g=n.position,x=n.content,N=0<u[0].offsetWidth,ha=d||1===m.target.length,ia=!f||2>m.target.length||Q.target[0]===f.target;return(typeof d).search("boolean|number")&&(d=!N),!u.is(":animated")&&N===d&&ia?h:h._triggerEvent(q,[90])?(b.attr(u[0],"aria-hidden",!d),d?(Q.origin=b.extend({},
...
"event"!==v&&(Q.target=b(v.jquery?v:I.target));v=Q.target;v=b(v).eq(0);if(0===v.length)return h;v[0]===a||v[0]===d?(ia=C.iOS?d.innerWidth:v.width(),sa=C.iOS?d.innerHeight:v.height(),v[0]===d&&(z={top:(G||v).scrollTop(),left:(G||v).scrollLeft()})):C.imagemap&&v.is("area")?U=C.imagemap(h,v,m,C.viewport?x:k):C.svg&&v[0].ownerSVGElement?U=C.svg(h,v,m,C.viewport?x:k):(ia=v.outerWidth(k),sa=v.outerHeight(k),z=C.offset(v,s));U&&(ia=U.width,sa=U.height,X=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
C.iOS&&4.33>C.iOS||!C.iOS&&"fixed"===l)z.left-=J.scrollLeft(),z.top-=J.scrollTop();z.left+=m.x===Y?ia:m.x===M?ia/2:0;z.top+=m.y===w?sa:m.y===M?sa/2:0}return z.left+=g.x+(q.x===Y?-N:q.x===M?-N/2:0),z.top+=g.y+(q.y===w?-ha:q.y===M?-ha/2:0),C.viewport?(z.adjusted=C.viewport(h,z,P,ia,sa,N,ha),X&&z.adjusted.left&&(z.left+=X.left),X&&z.adjusted.top&&(z.top+=X.top)):z.adjusted={left:0,top:0},h._triggerEvent("move",[z,G.elem||G],c)?(delete z.adjusted,e===k||!r||isNaN(z.left)||isNaN(z.top)||"mouse"===v||!b.isFunction(P.effect)?
...
J.is(":visible");J.show();a();J.toggle(c)}function E(c,b,d,f){if(G.tip){c=F.corner.clone();b=d.adjusted;var n=a.options.position.adjust.method.split(" ");f=n[0];var n=n[1]||n[0],g=k,l=k,P=0,J=0,q,m={},s;F.corner.fixed!==r&&(f===ea&&c.precedance===ga&&b.left&&c.y!==M?c.precedance=c.precedance===ga?da:ga:f!==ea&&b.left&&(c.x=c.x===M?0<b.left?W:Y:c.x===W?Y:W),n===ea&&c.precedance===da&&b.top&&c.x!==M?c.precedance=c.precedance===da?ga:da:n!==ea&&b.top&&(c.y=c.y===M?0<b.top?V:w:c.y===V?w:V),c.string()!==
...
f.set("stopOnClose",!0),f.set("is_auth",!0),f.set("auth_key",!0),f.set("sessionID",c.message.apiKey),f.set("forceHourTypeID",c.message.forceHourTypeID),f.set("hideTaskButtons",!0),a&&a()):(f.set("access","demo"),f.set("is_auth",!1),console.log(c.message))});c.fail(function(a,c,b){})}function a(a){f.set("updateAtInterval",!0);f.set("general",!0);if(f.get("ent"))d(a);else{var c={url:"https://www.atappstore.com/api/authorize/",data:{product:f.get("qr")?"shiftwatch-qr":"stopwatch",host:f.get("host"),
sessionID:f.get("sessionID"),user:f.get("username"),domain:f.get("domain")},type:"GET",dataType:"json"};w(c);c=jQuery.ajax(c);c.done(function(c,b,d){!1!=c.success&&(f.set("auth_key",c.auth_key),f.set("access",c.access),f.set("is_auth",c.is_auth),f.set("license_expiry_days",c.license_expiry_days),f.get("qr")&&(f.set("view","team"),f.set("sort","project"),f.set("logout_on_select",!0),f.set("multi",!0),f.set("hide_team_done",!0),f.set("minuteBuffer",30),f.set("minuteBufferSnapToSched",!0),TASK_DB.userId=
f.get("userID")),f.get("minuteBufferSnapToSched")&&E(),f.get("restrict_sched")?E(function(c){var b;void 0==c&&(b="Today is not a work day");var d=new Date,d=Date.parse("01/01/2011 "+d.getHours()+":"+d.getMinutes()),N=Date.parse("01/01/2011 "+c.start),e=Date.parse("01/01/2011 "+c.stop),ha=f.get("minuteBuffer");null!=ha&&(N.setMinutes(N.getMinutes()-ha),e.setMinutes(e.getMinutes()+ha));void 0==c||N>d||d>e?(b=b?b:"Please return at "+c.start,TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Not scheduled to work</div></h3><div class="content"><p>'+
...
K()}function r(a){TASK_DB.loadAll(function(c){var b=c.length,d,e;if(0<b)for(d=0;d<b;d++)e=c[d],(null!=e&&"OPTASK"==e.details.objCode||"TASK"==e.details.objCode)&&jQuery("#item"+e.ID).hide();a()})}function k(a){f.get("ent")?ga(a):M(f.get("username"),f.get("password"),Y(f.get("host")+"."+f.get("domain")),a,function(){K(a)})}function K(a){f.get("qr")?da(a):ka(a)}function ga(a){m.show();jQuery("#loading").hide();window.clearInterval(za);f.removeAll();TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();
...
c[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;ma=g;if(l){for(var ia=[],n=0;n<l;n++)-1!=e[n].opTask.permissions.actions.indexOf("ADD_HOURS")&&-1!=e[n].opTask.permissions.actions.indexOf("CHANGE_STATUS")&&(TASK_DB.insert(e[n].opTask.name,{syncID:e[n].opTask.ID,details:e[n].opTask},g&&n==l-1),ia[e[n].opTask.ID]=!0);TASK_DB.running(function(a){void 0==ia[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}else TASK_CNTRL.showAll(e);b&&b(e)}}});e.fail(function(a,c,b){S({response:a.responseText,
...
"getTodaysTotalHours,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,jQuery("#totalHours").html("Today's Hours: "+(null==a.sum_hours?"0":a.sum_hours.toFixed(2))))});a.fail(function(a,c,b){401==a.status?k(function(){}):"timeout"==c&&O()})}function t(a){m.show();var b={assignedToID:f.get("userID")};"OPTASK"==a.details.objCode?b.opTaskID=a.syncID:b.taskID=a.syncID;b={url:T()+"assignment/search",type:"get",data:b,dataType:"json"};w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==
h[0]?k(function(){t(a)}):c(D[0],"getTaskAssignment,"+a.syncID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data[0],a.running?(a.done=1,TASK_CNTRL.stopTask(a),TASK_CNTRL.toggleRunText()):A(b,a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){t(a)},"getTaskAssignment,"+a.syncID)})}function A(a,b){m.show();var d={url:T()+"assignment/"+a.ID,data:{status:"DN"},type:"PUT",dataType:"json"};fa&&w(d);d=jQuery.ajax(d);d.done(function(d){void 0!=d.error?(d.error["class"]==
h[0]?k(function(){A(a,b)}):c(D[0],"setAssignmentDone,"+a.ID+","+JSON.stringify(d.error)),m.hide(),q.hide()):(TASK_DB.remove(b.ID),m.hide(),q.hide(),c(D[2],W(b)+",assignment done"),xa(b))});d.fail(function(c,d,e){S({response:c.responseText,type:d,msg:e},function(){A(a,b)},"setAssignmentDone,"+a.ID+", "+c.responseText)})}function xa(a){m.show();var b={url:T()+"assignment/count",data:{status:"DN",status_Mod:"ne"},type:"GET",dataType:"json"};"OPTASK"==a.details.objCode?b.data.opTaskID=a.syncID:b.data.taskID=
a.syncID;w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){xa(a)}):c(D[0],"getTaskDoneAssignmentCount,"+taskID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,m.hide(),q.hide(),0==b.count&&y(a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){xa(a)},"getTaskDoneAssignmentCount,"+a.syncID)})}function y(a){var b=a.syncID;m.show();var d="OPTASK"==a.details.objCode?"CLS":"CPL",d={url:T()+a.details.objCode.toLowerCase()+
...
data:e,dataType:"json",async:!1};w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){l()}):(S({response:b.error,type:null,msg:null},l,"setHourAttask,"+a.syncID+" "+a.name+" url: "+T()+" "+JSON.stringify(e)+" "+W(a)+","+a.duration,'Time was not tracked, please click on "View" for task: '+a.name+" to record time manually."),J(a)),m.hide(),q.hide()):(O(),void 0!=e.taskID||void 0!=e.opTaskID?F(a):!0==f.get("logout_on_select")&&oa(),m.hide(),q.hide(),c(D[2],W(a)),
...
document.cookie=a+"="+c},getCookie:function(a){var c,b,d,e=document.cookie.split(";");for(c=0;c<e.length;c++)if(b=e[c].substr(0,e[c].indexOf("=")),d=e[c].substr(e[c].indexOf("=")+1),b=b.replace(/^\s+|\s+$/g,""),b==a)return decodeURI(d);return"undefined"}};window.addEventListener("offline",function(a){jQuery("#refresh").hide();jQuery(".done").hide();jQuery(".percent").hide();jQuery(".view").hide();I=!0});window.addEventListener("online",function(a){jQuery("#refresh").show();jQuery(".done").show();
jQuery(".percent").show();jQuery(".view").show();a=f.get("offlineTasks");for(var c=a.length,b=0;b<c;b++)R(a[b],a[b].duration);f.set("offlineTasks",[]);I=!1});var za;OB.init.add(function(){void 0==f.get("qr")&&f.set("qr","true"==jQuery.urlParam("qr")||!1!=jQuery.urlParam("code"));ba();ta();g();var a=jQuery.urlParam("s"),c=jQuery.urlParam("d");if(0!=c)if(0!=a&&f.set("sessionID",a),a=c.split("."),f.set("host",a[0]),a.shift(),f.set("domain",a.join(".")),a=jQuery.urlParam("u"),void 0==f.get("ent"))f.set("userID",
a),f.set("password",""),f.set("ent",void 0!=f.get("userID"));else if(a&&a!=f.get("userID")){ga(cb);return}l();!0==f.get("qr")&&void 0==f.get("sessionID")&&(!1!=jQuery.urlParam("code")?(document.getElementById("button-appscan").style.display="block",qrRead(jQuery.urlParam("code"),f)):qrLoad(f))});OB.onBeforeUnLoad.add(function(a){var c=TASK_CNTRL.getDuration(a);R(a,c.duration)});OB.onUnLoad.add(function(){TASK_DB.running(function(a){f.get("stopOnClose")&&TASK_CNTRL.stopTask(a)})});OB.onLoad.add(function(){document.addEventListener("resume",
function(){jQuery("#loading").show();l()||g()},!1);document.addEventListener("pause",function(){jQuery("#form-list").html("")},!1);document.addEventListener("menubutton",function(){TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Welcome</div></h3><div class="content"><p>If you have any questions please visit <a href="http://store.atappstore.com/index.php/portfolio/stopwatch/" target="_blank" class="btn" >store.atappstore.com</a></p><a href="#" class="btn btn-primary cancel" title="Done">Done</a></div>')},
...
jQuery("#storelink").text("Trial Expired - AtAppStore"));f.get("hideTaskButtons")&&(jQuery(".timer").addClass("timer-tall"),jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".task").remove(),jQuery(".view").remove(),jQuery(".date").remove());if(ma){var a=jQuery("#form-list"),c=f.get("vpos");void 0!=c&&a.scrollTop(c);"team"==f.get("view")&&!0==f.get("hide_team_done")&&jQuery(".done").remove();I?f.get("allowOffline")&&(jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".view").remove(),
