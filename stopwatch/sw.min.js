...
cases:e,forceStrings:e,sortFunction:null}};d.fn.extend({tinysort:function(Z,sa){Z&&"string"!=typeof Z&&(sa=Z,Z=null);var ba=d.extend({},d.tinysort.defaults,sa),ta,pa=d(this).length,r={},k=!(!Z||""==Z),K=!(null===ba.attr||""==ba.attr),ga=null!==ba.data,da=k&&":"==Z[0],ka=da?this.filter(Z):this,R=ba.sortFunction,V="asc"==ba.order?1:-1,W=[];d.each(l,function(b,a){a.call(a,ba)});R||(R="rand"==ba.order?function(){return 0.5>Math.random()?1:-1}:function(a,c){var k=e,l=!ba.cases?a.s&&a.s.toLowerCase?a.s.toLowerCase():
a.s:a.s,r=!ba.cases?c.s&&c.s.toLowerCase?c.s.toLowerCase():c.s:c.s;if(!ba.forceStrings){var x=l&&l.match(g),K=r&&r.match(g);if(x&&K){var N=l.substr(0,l.length-x[0].length),M=r.substr(0,r.length-K[0].length);N==M&&(k=!e,l=b(x[0]),r=b(K[0]))}}var R=V*(l<r?-1:l>r?1:0);d.each(oa,function(b,a){R=a.call(a,k,l,r,R)});return R});this.each(function(b,a){var c=d(a),e=k?da?ka.filter(a):c.find(Z):c,g=ga?""+e.data(ba.data):K?e.attr(ba.attr):ba.useVal?e.val():e.text(),l=c.parent();r[l]||(r[l]={s:[],n:[]});0<e.length?
...
/out|leave/.test(Q.event.type)&&n.show.target.add(f.target).length===n.show.target.length&&v.has(f.relatedTarget).length)return h;Q.event=b.extend({},f)}if(!h.rendered)return d?h.render(1):h;var q=d?"show":"hide",m=n[q],g=n.position,u=n.content,L=0<v[0].offsetWidth,ha=d||1===m.target.length,ia=!f||2>m.target.length||Q.target[0]===f.target;return(typeof d).search("boolean|number")&&(d=!L),!v.is(":animated")&&L===d&&ia?h:h._triggerEvent(q,[90])?(b.attr(v[0],"aria-hidden",!d),d?(Q.origin=b.extend({},
ja),h.focus(f),b.isFunction(u.text)&&B(u.text,k),b.isFunction(u.title.text)&&y(u.title.text,k),!ua&&"mouse"===g.target&&g.adjust.mouse&&(b(a).bind("mousemove.qtip",c),ua=r),h.reposition(f,e),!m.solo||b(aa,m.solo).not(v).qtip("hide",b.Event("tooltipsolo"))):(clearTimeout(h.timers.show),delete Q.origin,ua&&!b(aa+'[tracking="true"]:visible',m.solo).not(v).length&&(b(a).unbind("mousemove.qtip"),ua=k),h.blur(f)),m.effect===k||ha===k?(v[q](),w.call(v)):b.isFunction(m.effect)?(v.stop(1,1),m.effect.call(v,
...
"event"!==w&&(Q.target=b(w.jquery?w:I.target));w=Q.target;w=b(w).eq(0);if(0===w.length)return h;w[0]===a||w[0]===d?(ia=C.iOS?d.innerWidth:w.width(),ma=C.iOS?d.innerHeight:w.height(),w[0]===d&&(z={top:(G||w).scrollTop(),left:(G||w).scrollLeft()})):C.imagemap&&w.is("area")?U=C.imagemap(h,w,m,C.viewport?u:k):C.svg&&w[0].ownerSVGElement?U=C.svg(h,w,m,C.viewport?u:k):(ia=w.outerWidth(k),ma=w.outerHeight(k),z=C.offset(w,s));U&&(ia=U.width,ma=U.height,X=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
C.iOS&&4.33>C.iOS||!C.iOS&&"fixed"===l)z.left-=J.scrollLeft(),z.top-=J.scrollTop();z.left+=m.x===Y?ia:m.x===N?ia/2:0;z.top+=m.y===x?ma:m.y===N?ma/2:0}return z.left+=g.x+(q.x===Y?-L:q.x===N?-L/2:0),z.top+=g.y+(q.y===x?-ha:q.y===N?-ha/2:0),C.viewport?(z.adjusted=C.viewport(h,z,P,ia,ma,L,ha),X&&z.adjusted.left&&(z.left+=X.left),X&&z.adjusted.top&&(z.top+=X.top)):z.adjusted={left:0,top:0},h._triggerEvent("move",[z,G.elem||G],c)?(delete z.adjusted,e===k||!r||isNaN(z.left)||isNaN(z.top)||"mouse"===w||!b.isFunction(P.effect)?
...
J.is(":visible");J.show();a();J.toggle(c)}function E(c,b,d,f){if(G.tip){c=F.corner.clone();b=d.adjusted;var n=a.options.position.adjust.method.split(" ");f=n[0];var n=n[1]||n[0],g=k,l=k,P=0,J=0,q,m={},s;F.corner.fixed!==r&&(f===ea&&c.precedance===ga&&b.left&&c.y!==N?c.precedance=c.precedance===ga?da:ga:f!==ea&&b.left&&(c.x=c.x===N?0<b.left?W:Y:c.x===W?Y:W),n===ea&&c.precedance===da&&b.top&&c.x!==N?c.precedance=c.precedance===da?ga:da:n!==ea&&b.top&&(c.y=c.y===N?0<b.top?V:x:c.y===V?x:V),c.string()!==
...
e=a.precedance,h=M(a),n=[a.x,a.y],e===ga&&n.reverse(),b.each(n,function(c,b){var n,w,g;b===N?(n=e===da?W:V,d[n]="50%",d["margin-"+n]=-Math.round(h[e===da?ka:R]/2)+f):(n=O(a,b),w=O(a,b,G.content),g=t(a),d[b]=c?w:f+(g>n?g:-n))}),d[a[e]]-=h[e===ga?ka:R],c.css({top:"",bottom:"",left:"",right:"",margin:""}).css(d),d)},destroy:function(){G.tip&&G.tip.remove();G.tip=!1;J.unbind(".qtip-tip")}});F.init()}function ta(c){function e(){F=b(y,t).not("[disabled]").map(function(){return"function"==typeof this.focus?
...
K()}function r(a){TASK_DB.loadAll(function(c){var b=c.length,d,e;if(0<b)for(d=0;d<b;d++)e=c[d],(null!=e&&"OPTASK"==e.details.objCode||"TASK"==e.details.objCode)&&jQuery("#item"+e.ID).hide();a()})}function k(a){f.get("ent")?ga(a):N(f.get("username"),f.get("password"),Y(f.get("host")+"."+f.get("domain")),a,function(){K(a)})}function K(a){f.get("qr")?da(a):ka(a)}function ga(a){m.show();jQuery("#loading").hide();window.clearInterval(za);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();
...
e.error)e.error["class"]==h[0]?k(function(){aa(a,b,d)}):c(D[0],"addAllAssignedTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else{e=e.data;var g=!0,l=f.get("view");"all"!=l&&"role"!=l&&(g=!1);var ia=f.get("hour_types");if("opTask"!=a)aa("opTask",b,d),r(function(){var a=e.length;if(a){for(var c=[],b=0;b<a;b++){var d=e[b].task;if(-1!=d.permissions.actions.indexOf("ADD_HOURS")&&-1!=d.permissions.actions.indexOf("CHANGE_STATUS")){var u=!1,f=d.project.hourTypes.length;if(f)for(var L=0;L<f;L++){if(void 0!=
ia[d.project.hourTypes[L].ID]){u=!0;break}}else u=!0;u&&(TASK_DB.insert(d.name,{syncID:d.ID,details:d},g&&b==a-1),c[d.ID]=!0)}}TASK_DB.running(function(a){void 0==c[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;na=g;if(l){for(var n=[],s=0;s<l;s++){var t=e[s].opTask;if(-1!=t.permissions.actions.indexOf("ADD_HOURS")&&-1!=t.permissions.actions.indexOf("CHANGE_STATUS")){var ma=!1,v=t.project.hourTypes.length;if(v)for(var w=0;w<v;w++){if(void 0!=ia[t.project.hourTypes[w].ID]){ma=
!0;break}}else ma=!0;ma&&(TASK_DB.insert(t.name,{syncID:t.ID,details:t},g&&s==l-1),n[t.ID]=!0)}}TASK_DB.running(function(a){void 0==n[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}else TASK_CNTRL.showAll(e);b&&b(e)}}});e.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},aa,"addAllAssignedTasks-fail"+c+"-"+b+" "+a.responseText)})}function wa(a){m.show();var b={url:T("v5.0")+"user/search",type:"get",data:{fields:"role:*,roles:*",ID:f.get("userID")},dataType:"json"};x(b);b=jQuery.ajax(b);
b.done(function(b){if(void 0!=b.error)b.error["class"]==h[0]?k(function(){wa(a)}):c(D[0],"addAllRoleTasks,"+JSON.stringify(b.error)),m.hide(),q.hide();else if(b=b.data[0],0==b.length)a&&a(),m.hide(),q.hide();else{var d=jQuery("#role-filter"),e=function(){aa("task",function(){d.css("display","inline-block");m.hide();q.hide()})},g=function(a,c){f.set("currentRole",{name:a,ID:c});d.find("button").html("<strong>Role: </strong><span>"+a+'</span><span class="caret"></span>');e()},L=f.get("currentRole");
void 0!=L?g(L.name,L.ID):void 0!=b.role&&g(b.role.name,b.role.ID);if(void 0!=b.roles&&0!=b.roles.length)for(var L=d.find(".dropdown-menu").html("").first(),l=0,n=b.roles.length;l<n;l++)L.append('<li><a href="#" id="'+b.roles[l].ID+'">'+b.roles[l].name+"</a></li>"),L.find("#"+b.roles[l].ID).click(function(){g(jQuery(this).text(),jQuery(this).attr("id"))});else qa(),m.hide(),q.hide(),Z(),TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Roles</div></h3><div class="content"><div><p>No additional roles available.</strong></h3><div><a href="#" class="btn btn-primary cancel" title="Ok">Ok</a><div>')}});
...
a[0]),a.shift(),f.set("domain",a.join(".")),a=jQuery.urlParam("u"),c=f.get("userID"),void 0==f.get("ent")&&(f.set("userID",a),f.set("password",""),f.set("ent",void 0!=f.get("userID")),a&&c&&a!=c))){ga();return}l();!0==f.get("qr")&&void 0==f.get("sessionID")&&(!1!=jQuery.urlParam("code")?(document.getElementById("button-appscan").style.display="block",qrRead(jQuery.urlParam("code"),f)):qrLoad(f))});OB.onBeforeUnLoad.add(function(a){var c=TASK_CNTRL.getDuration(a);R(a,c.duration)});OB.onUnLoad.add(function(){TASK_DB.running(function(a){f.get("stopOnClose")&&
