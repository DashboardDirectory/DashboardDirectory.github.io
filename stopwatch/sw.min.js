...
cases:e,forceStrings:e,sortFunction:null}};d.fn.extend({tinysort:function(Z,ra){Z&&"string"!=typeof Z&&(ra=Z,Z=null);var ba=d.extend({},d.tinysort.defaults,ra),ta,oa=d(this).length,r={},k=!(!Z||""==Z),K=!(null===ba.attr||""==ba.attr),ia=null!==ba.data,da=k&&":"==Z[0],ka=da?this.filter(Z):this,X=ba.sortFunction,V="asc"==ba.order?1:-1,R=[];d.each(l,function(b,a){a.call(a,ba)});X||(X="rand"==ba.order?function(){return 0.5>Math.random()?1:-1}:function(a,c){var k=e,l=!ba.cases?a.s&&a.s.toLowerCase?a.s.toLowerCase():
...
r[l].s.push({s:g,e:c,n:b}):r[l].n.push({e:c,n:b})});for(ta in r)r[ta].s.sort(X);for(ta in r){var X=r[ta],w=[],Y=oa,M=[0,0],qa;switch(ba.place){case "first":d.each(X.s,function(b,a){Y=c(Y,a.n)});break;case "org":d.each(X.s,function(b,a){w.push(a.n)});break;case "end":Y=X.n.length;break;default:Y=0}for(qa=0;qa<oa;qa++){var ea=a(w,qa)?!e:qa>=Y&&qa<Y+X.s.length,L=(ea?X.s:X.n)[M[ea?0:1]].e;L.parent().append(L);(ea||!ba.returns)&&R.push(L.get(0));M[ea?0:1]++}}this.length=0;Array.prototype.push.apply(this,
...
{"class":"ui-icon ui-icon-close",html:"&times;"}));I.button.appendTo(I.titlebar||u).attr("role","button").click(function(a){return u.hasClass(W)||h.hide(a),k})}function xa(){var a=D+"-title";I.titlebar&&t();I.titlebar=b("<div />",{"class":ca+"-titlebar "+(n.style.widget?E("header"):"")}).append(I.title=b("<div />",{id:a,"class":ca+"-title","aria-atomic":r})).insertBefore(I.content).delegate(".qtip-close","mousedown keydown mouseup keyup mouseout",function(a){b(this).toggleClass("ui-state-active ui-state-focus",
...
!a?k:(b.isFunction(a)&&(a=a.call(v,Q.event,h)||""),d!==k&&F(n.content.deferred),a.jquery&&0<a.length?q.empty().append(a.css({display:"block"})):q.html(a),0>h.rendered?u.queue("fx",f):(fa=0,f(b.noop)),h)}function G(){function e(a){if(u.hasClass(W))return k;clearTimeout(h.timers.show);clearTimeout(h.timers.hide);var x=function(){h.toggle(r,a)};0<n.show.delay?h.timers.show=setTimeout(x,n.show.delay):x()}function g(a){if(u.hasClass(W)||f||fa)return k;var x=b(a.relatedTarget||a.target),c=x.closest(aa)[0]===
u[0],x=x[0]===m.show[0];clearTimeout(h.timers.show);clearTimeout(h.timers.hide);if("mouse"===q.target&&c||n.hide.fixed&&/mouse(out|leave|move)/.test(a.type)&&(c||x))try{a.preventDefault(),a.stopImmediatePropagation()}catch(d){}else 0<n.hide.delay?h.timers.hide=setTimeout(function(){h.hide(a)},n.hide.delay):h.hide(a)}function l(a){if(u.hasClass(W))return k;clearTimeout(h.timers.inactive);h.timers.inactive=setTimeout(function(){h.hide(a)},n.hide.inactive)}function G(a){h.rendered&&0<u[0].offsetWidth&&
...
!a.relatedTarget&&h.hide(a)});n.hide.fixed?(m.hide=m.hide.add(u),u.bind("mouseover"+z,function(){u.hasClass(W)||clearTimeout(h.timers.hide)})):/mouse(over|enter)/i.test(n.show.event)&&m.hide.bind("mouseleave"+z,function(a){clearTimeout(h.timers.show)});-1<(""+n.hide.event).indexOf("unfocus")&&q.container.closest("html").bind("mousedown"+z+" touchstart"+z,function(a){var x=b(a.target);h.rendered&&u.hasClass(W);var c=0<x.parents(aa).filter(u[0]).length;x[0]!==v[0]&&x[0]!==u[0]&&!c&&!v.has(x[0]).length&&
...
return b.attr(v[0],"aria-describedby",D),u=I.tooltip=b("<div/>",{id:D,"class":[ca,wa,n.style.classes,ca+"-pos-"+n.position.my.abbrev()].join(" "),width:n.style.width||"",height:n.style.height||"",tracking:"mouse"===e.target&&e.adjust.mouse,role:"alert","aria-live":"polite","aria-atomic":k,"aria-describedby":D+"-content","aria-hidden":r}).toggleClass(W,Q.disabled).data("qtip",h).appendTo(n.position.container).append(I.content=b("<div />",{"class":ca+"-content",id:D+"-content","aria-atomic":r})),h.rendered=
...
f),h},blur:function(a){return u.removeClass(pa),h._triggerEvent("blur",[u.css("zIndex")],a),h},reposition:function(c,e){if(!h.rendered||f)return h;f=1;var v=n.position.target,P=n.position,q=P.my,m=P.at,g=P.adjust,x=g.method.split(" "),N=u.outerWidth(k),ga=u.outerHeight(k),ha=0,sa=0,l=u.css("position"),G=P.viewport,z={left:0,top:0},s=P.container,r=0<u[0].offsetWidth,y=c&&"scroll"===c.type,J=b(d),U,W;if(b.isArray(v)&&2===v.length)m={x:R,y:V},z={left:v[0],top:v[1]};else if("mouse"===v&&(c&&c.pageX||
Q.event.pageX))m={x:R,y:V},c=ja&&ja.pageX&&(g.mouse||!c||!c.pageX)?{pageX:ja.pageX,pageY:ja.pageY}:(!c||"resize"!==c.type&&"scroll"!==c.type?c&&c.pageX&&"mousemove"===c.type?c:!g.mouse&&Q.origin&&Q.origin.pageX&&n.show.distance?Q.origin:c:Q.event)||c||Q.event||ja||{},"static"!==l&&(z=s.offset()),z={left:c.pageX-z.left,top:c.pageY-z.top},g.mouse&&y&&(z.left-=ja.scrollX-J.scrollLeft(),z.top-=ja.scrollY-J.scrollTop());else{"event"===v&&c&&c.target&&"scroll"!==c.type&&"resize"!==c.type?Q.target=b(c.target):
"event"!==v&&(Q.target=b(v.jquery?v:I.target));v=Q.target;v=b(v).eq(0);if(0===v.length)return h;v[0]===a||v[0]===d?(ha=C.iOS?d.innerWidth:v.width(),sa=C.iOS?d.innerHeight:v.height(),v[0]===d&&(z={top:(G||v).scrollTop(),left:(G||v).scrollLeft()})):C.imagemap&&v.is("area")?U=C.imagemap(h,v,m,C.viewport?x:k):C.svg&&v[0].ownerSVGElement?U=C.svg(h,v,m,C.viewport?x:k):(ha=v.outerWidth(k),sa=v.outerHeight(k),z=C.offset(v,s));U&&(ha=U.width,sa=U.height,W=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
C.iOS&&4.33>C.iOS||!C.iOS&&"fixed"===l)z.left-=J.scrollLeft(),z.top-=J.scrollTop();z.left+=m.x===Y?ha:m.x===M?ha/2:0;z.top+=m.y===w?sa:m.y===M?sa/2:0}return z.left+=g.x+(q.x===Y?-N:q.x===M?-N/2:0),z.top+=g.y+(q.y===w?-ga:q.y===M?-ga/2:0),C.viewport?(z.adjusted=C.viewport(h,z,P,ha,sa,N,ga),W&&z.adjusted.left&&(z.left+=W.left),W&&z.adjusted.top&&(z.top+=W.top)):z.adjusted={left:0,top:0},h._triggerEvent("move",[z,G.elem||G],c)?(delete z.adjusted,e===k||!r||isNaN(z.left)||isNaN(z.top)||"mouse"===v||!b.isFunction(P.effect)?
u.css(z):b.isFunction(P.effect)&&(P.effect.call(u,h,b.extend({},z)),u.queue(function(a){b(this).css({opacity:"",height:""});b.browser.msie&&this.style.removeAttribute("filter");a()})),f=0,h):h},disable:function(a){return"boolean"!=typeof a&&(a=!u.hasClass(W)&&!Q.disabled),h.rendered?(u.toggleClass(W,a),b.attr(u[0],"aria-disabled",a)):Q.disabled=!!a,h},enable:function(){return h.disable(k)},destroy:function(){var a=v[0],c=b.attr(a,la),d=v.data("qtip");h.destroyed=r;h.rendered&&(u.stop(1,0).remove(),
...
e=a.precedance,h=L(a),n=[a.x,a.y],e===ia&&n.reverse(),b.each(n,function(c,b){var n,v,g;b===M?(n=e===da?R:V,d[n]="50%",d["margin-"+n]=-Math.round(h[e===da?ka:X]/2)+f):(n=O(a,b),v=O(a,b,G.content),g=t(a),d[b]=c?v:f+(g>n?g:-n))}),d[a[e]]-=h[e===ia?ka:X],c.css({top:"",bottom:"",left:"",right:"",margin:""}).css(d),d)},destroy:function(){G.tip&&G.tip.remove();G.tip=!1;J.unbind(".qtip-tip")}});F.init()}function ta(c){function e(){F=b(y,t).not("[disabled]").map(function(){return"function"==typeof this.focus?
...
["inherit"])[0].toLowerCase();a=a.charAt(0);this.precedance="t"===a||"b"===a?da:ia;this.string=function(){return this.precedance===da?this.y+this.x:this.x+this.y};this.abbrev=function(){var a=this.x.substr(0,1),c=this.y.substr(0,1);return a===c?a:this.precedance===da?c+a:a+c};this.invertx=function(a){this.x=this.x===R?Y:this.x===Y?R:a||this.x};this.inverty=function(a){this.y=this.y===V?w:this.y===w?V:a||this.y};this.clone=function(){return{x:this.x,y:this.y,precedance:this.precedance,string:this.string,
...
scrolltop:A?0:h.scrollTop(),offset:h.offset()||{left:0,top:0}};D={elem:D,scrollLeft:D.scrollLeft(),scrollTop:D.scrollTop(),offset:D.offset()||{left:0,top:0}};if("shift"!==J||"shift"!==G)u=y.clone();return K={left:"none"!==J?t(ia,da,J,B.x,R,Y,ka,g,k):0,top:"none"!==G?t(da,ia,G,B.y,V,w,X,l,r):0},u&&e.lastClass!==(z=ca+"-pos-"+u.abbrev())&&C.removeClass(c.cache.lastClass).addClass(c.cache.lastClass=z),K};C.imagemap=function(a,c,d,e){function g(a,c,b){for(var d=0,e=1,l=1,k=0,n=0,r=a.width,s=a.height;0<
r&&0<s&&0<e&&0<l;){r=Math.floor(r/2);s=Math.floor(s/2);b.x===R?e=r:b.x===Y?e=a.width-r:e+=Math.floor(r/2);b.y===V?l=s:b.y===w?l=a.height-s:l+=Math.floor(s/2);for(d=c.length;d--&&!(2>c.length);)k=c[d][0]-a.position.left,n=c[d][1]-a.position.top,(b.x===R&&k>=e||b.x===Y&&k<=e||b.x===M&&(k<e||k>a.width-e)||b.y===V&&n>=l||b.y===w&&n<=l||b.y===M&&(n<l||n>a.height-l))&&c.splice(d,1)}return{left:c[0][0],top:c[0][1]}}c.jquery||(c=b(c));a=a.cache.areas={};var l=(c[0].shape||c.attr("shape")).toLowerCase(),k=
...
y.width/2,top:y.position.top+y.height/2}:(a[d+k]||(y.position=g(y,A.slice(),d),e&&("flip"===e[0]||"flip"===e[1])&&(y.offset=g(y,A.slice(),{x:d.x===R?Y:d.x===Y?R:M,y:d.y===V?w:d.y===w?V:M}),y.offset.left-=y.position.left,y.offset.top-=y.position.top),a[d+k]=y),y=a[d+k]),y.width=y.height=0}return y.position.left+=C.left,y.position.top+=C.top,y};C.ie6=function(a){var c=b.browser,d=a.plugins.ie6;return!c.msie||"6"!==(""+c.version).charAt(0)?k:"object"==typeof d?d:a.plugins.ie6=new oa(a)};C.ie6.initialize=
...
type:"get",data:{},dataType:"json"};w(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uInfo,"+JSON.stringify(a.error)),m.hide(),q.hide()):a=a.data});a.fail(function(a,c,x){S({response:a.responseText,type:c,msg:x},b,"uInfo")})}function c(a,c,b){}function g(){if(fa||u||void 0!=f.get("host")&&"full"==getQueryString("view"))window.clearInterval(za),za=window.setInterval(function(){if(u||!document.hasFocus())jQuery("#search-bar").is(":visible")&&jQuery("#search").click(),
...
q.show();M(jQuery.trim(c.find(":input[name='username']").val()),jQuery.trim(c.find(":input[name='password']").val()),Y(jQuery.trim(c.find(":input[name='host']").val())),function(){jQuery("#bar").show();u?c.hide():c.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()})})}function X(a,c){var b=new Date;a.dateStr=b.getFullYear()+"-"+(b.getMonth()+1)+"-"+b.getDate();a.duration=c;I&&f.get("allowOffline")?(a.username=f.get("username"),a.userID=f.get("userID"),b=f.get("offlineTasks"),
void 0==b&&(b=[]),b.push(a),f.set("offlineTasks"),m.hide(),q.hide(),!0==f.get("logout_on_select")&&oa()):J(a,G)}function V(a){return"jan. feb. mar. apr. may june jul. aug. sep. oct. nov. dec.".split(" ")[a]}function R(a){return void 0!=a.details.project?a.details.project.name.split(",").join(" ")+","+a.name.split(",").join(" "):a.details}function w(a,c){void 0==c&&(c=1E4);a.timeout=c;fa&&(!u&&!z)&&(a.dataType="jsonp",a.crossDomain=!0,a.jsonp="jsonp",a.cache=!0);fa&&"PUT"==a.type&&(a.data.method="put");
var b=f.get("sessionID");f.get("ent")?("PUT"==a.type&&(a.type="POST"),a.data.apiKey=b):a.data.sessionID=b}function Y(a){if(1<a.split(".").length){var c=a.split("//");return 1<c.length?(c.shift(),c[0]):a}return a+".attask-ondemand.com"}function M(c,b,d,e,l){if(void 0==c||void 0==b||void 0==d)l();else{var h=jQuery(".error");h.html("");var k={url:"https://"+d+"/attask/api/v2.0/login",type:"get",data:{username:c,password:b},dataType:"json"};w(k);I?f.get("allowOffline")?(k=f.get(c),void 0!=k?(qa(k.sessionID,
k.userID,k.username,k.password,d),q.hide(),L(),g(),void 0!=e&&e()):h.html("OFFLINE")):h.html("OFFLINE"):(k=jQuery.ajax(k),k.done(function(k){void 0!=k.error?(f.get("qr")?h.html("Please Scan Again."):h.html(k.error.message),q.hide(),void 0!==l&&l()):(k=k.data,qa(k.sessionID,k.userID,c,b,d),f.set("userObj",JSON.stringify(k)),q.hide(),a(function(){C();L();g();void 0!=e&&e()}))}),k.fail(function(a,c,b){a.responseText&&""!==a.responseText?-1!=a.responseText.indexOf("<html>")?f.get("qr")?h.html("Please Scan Again."):
h.html("Host Invalid: "+d):f.get("qr")?h.html("Please Scan Again."):h.html(JSON.parse(a.responseText).error.message):f.get("qr")?h.html("Please Scan Again."):h.html("Host Invalid: "+d);q.hide();void 0!==l&&l()}))}}function qa(a,c,b,d,e){f.set("sessionID",a);f.set("userID",c);f.set("username",b);f.set("password",d);e=e.split(".");f.set("host",e[0]);e.shift();f.set("domain",e.join("."));f.get("allowOffline")&&f.set(b,{sessionID:a,userID:c,username:b,password:d})}function ea(c,b){var d=f.get("sessionID");
void 0==d||""==d?k(c):(m.show(),d={url:T()+"user/"+f.get("userID"),data:{updates:"{isActive: true}"},type:"PUT",dataType:"json"},fa&&w(d),d=jQuery.ajax(d),d.done(function(d){void 0!=d.error?(d.error["class"]==h[0]?void 0!=b?b(c):k(c):(f.get("sessionID"),JSON.stringify(d.error)),m.hide(),q.hide()):(m.hide(),q.hide(),void 0==f.get("auth_key")?a(L):L(),void 0!=c&&c())}),d.fail(function(a,d,e){401==a.status?void 0!=b?b(c):k(c):"timeout"==d?ea(c):f.get("sessionID")}))}function L(){m.show();var a=f.get("userID"),
b={url:"https://"+f.get("host")+"."+f.get("domain")+"/attask/api-internal/timeSheet/search",type:"get",data:{startDate:"$$TODAY+1",startDate_Mod:"lte",endDate:"$$TODAY-1",endDate_Mod:"gte",fields:"timesheetProfile:description,timesheetProfile:hourTypes:ID,timesheetProfile:hourTypes:name,timesheetProfile:hourTypes:description,timesheetProfile:hourTypes:isActive",userID:a},dataType:"json"};w(b,35E3);I?f.get("allowOffline")?(a=TASK_DB.userRunningTasks(a),void 0!=a&&(ma=!0,TASK_DB.insert(a.name,{syncID:a.syncID,
...
g.ID:e.data.assignedToID=f.get("userID");"task"==a&&(e.data.fields+=","+a+":percentComplete");w(e);e=jQuery.ajax(e);e.done(function(e){if(void 0!=e.error)e.error["class"]==h[0]?k(function(){aa(a,b,d)}):c(D[0],"addAllAssignedTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else{e=e.data;var g=!0,l=f.get("view");"all"!=l&&"role"!=l&&(g=!1);if("opTask"!=a)aa("opTask",b,d),r(function(){var a=e.length;if(a){for(var c=[],b=0;b<a;b++)-1!=e[b].task.permissions.actions.indexOf("ADD_HOURS")&&-1!=e[b].task.permissions.actions.indexOf("CHANGE_STATUS")&&
(TASK_DB.insert(e[b].task.name,{syncID:e[b].task.ID,details:e[b].task},g&&b==a-1),c[e[b].task.ID]=!0);TASK_DB.running(function(a){void 0==c[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;ma=g;if(l){for(var ha=[],n=0;n<l;n++)-1!=e[n].opTask.permissions.actions.indexOf("ADD_HOURS")&&-1!=e[n].opTask.permissions.actions.indexOf("CHANGE_STATUS")&&(TASK_DB.insert(e[n].opTask.name,{syncID:e[n].opTask.ID,details:e[n].opTask},g&&n==l-1),ha[e[n].opTask.ID]=!0);TASK_DB.running(function(a){void 0==
...
m.hide(),q.hide();else if(b=b.data[0],0==b.length)a&&a(),m.hide(),q.hide();else{var d=jQuery("#role-filter"),e=function(){aa("task",function(){d.css("display","inline-block");m.hide();q.hide()})},g=function(a,c){f.set("currentRole",{name:a,ID:c});d.find("button").html("<strong>Role: </strong><span>"+a+'</span><span class="caret"></span>');e()},N=f.get("currentRole");void 0!=N?g(N.name,N.ID):void 0!=b.role&&g(b.role.name,b.role.ID);if(void 0!=b.roles&&0!=b.roles.length)for(var N=d.find(".dropdown-menu").html("").first(),
l=0,n=b.roles.length;l<n;l++)N.append('<li><a href="#" id="'+b.roles[l].ID+'">'+b.roles[l].name+"</a></li>"),N.find("#"+b.roles[l].ID).click(function(){g(jQuery(this).text(),jQuery(this).attr("id"))});else pa(),m.hide(),q.hide(),Z(),TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Roles</div></h3><div class="content"><div><p>No additional roles available.</strong></h3><div><a href="#" class="btn btn-primary cancel" title="Ok">Ok</a><div>')}});b.fail(function(a,c,b){S({response:a.responseText,
type:c,msg:b},wa,"addAllRoleTasks")})}function pa(){var a=jQuery("#role-filter");a.hide();a.find(".dropdown-menu").first().html("")}function ya(a){m.show();var b={url:T()+"team/search",type:"get",data:{fields:"ID","users:ID":f.get("userID")},dataType:"json"};w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){ya(a)}):c(D[0],"addAllGroupTasks,"+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,0==b.length?(a?a():jQuery("#form-list").hide(),m.hide(),q.hide(),
TASK_CNTRL.showNoTaskWindow()):r(function(){for(var c=b.length,d=function(b,d){if(0==--c){var e=va.length;ma=!0;if(0==e)TASK_CNTRL.showAll(va);else{for(var f=0;f<e;f++)TASK_DB.insert(va[f].name,{syncID:va[f].ID,details:va[f]},f==e-1);va=[];a&&a()}}},e=0;e<c;e++)la("task",b[e].ID,d)}))});b.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ya,"addAllGroupTasks")})}function la(a,b,d){a=void 0==a?"task":a;m.show();var e={url:T()+a+"/search?project:status=INF",type:"get",data:{canStart:!0,
status:"N",status_Mod:"contains","project:status":"CUR",fields:"parent,priority,project,plannedCompletionDate",$$LIMIT:1E3,"team:ID":b},dataType:"json"};"task"==a&&(e.data.fields+=",percentComplete");w(e);e=jQuery.ajax(e);e.done(function(e){if(void 0!=e.error)e.error["class"]==h[0]?k(function(){la(a,b,d)}):c(D[0],"addGroupTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else if(e=e.data,0==e.length)d&&d(a,b);else if("opTask"!=a){la("opTask",b,d);for(var f=e.length,g=0;g<f;g++)va.push(e[g])}else{f=
e.length;for(g=0;g<f;g++)va.push(e[g]);d&&d(a,b)}});e.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},la,"addGroupTasks-fail"+c+"-"+b+" "+a.responseText)})}function ua(){m.show();var a={url:T()+"hour/report",type:"get",data:{"task:canStart":!0,taskID_1_GroupBy:!0,entryDate:"$$TODAY-14d",entryDate_Range:"$$TODAY",entryDate_Mod:"between"},dataType:"json"};w(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){ua()}):c(D[0],"addRecentTasks,"+JSON.stringify(a.error)),
m.hide(),q.hide()):(a=Object.keys(a.data),0==a.length?(ma=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){ma=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))}))});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ua,"addRecentTasks")})}function v(){m.show();var a;f.get("ent")?aa("task",function(a){ma=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))},!0):(a={url:T()+"work/myWork",type:"get",data:{fields:"parent,priority,project,plannedCompletionDate"},dataType:"json"},
w(a),a=jQuery.ajax(a),a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){v()}):c(D[0],"addCurrentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,0==a.length?(ma=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){var c=a.length;ma=!0;for(var b=0;b<c;b++)TASK_DB.insert(a[b].name,{syncID:a[b].ID,details:a[b]},b==c-1)}))}),a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},v,"addCurrentTasks")}))}function n(){m.show();var a={url:T()+"work/workRequests",
...
type:c,msg:b},n,"addRequestedTasks")})}function P(){var a={url:T()+"user/search",data:{"hourTypes:isActive":!0,fields:"hourTypes:ID,hourTypes:name,hourTypes:description,hourTypes:countAsRevenue",ID:f.get("userID")},type:"get",dataType:"json"};w(a);W=!1;a=jQuery.ajax(a);a.done(function(a){if(void 0!=a.error)a.error["class"]==h[0]?k(function(){}):c(D[0],"getHourTypes,"+JSON.stringify(a.error)),m.hide(),q.hide();else if(a=a.data,void 0!=a&&0!=a.length&&0!=a[0].hourTypes.length){W=!0;var b=f.get("sw_hour_types"),
d=jQuery("#hourTypeSelector");d.text("");var e=null;void 0!=b&&void 0!=b["Task Time"]&&(e=b["Task Time"]);d.append(jQuery("<option />").val(e).text("Task Time"));jQuery.each(a[0].hourTypes,function(){var a=this.ID;void 0!=b&&void 0!=b[this.name]&&(a=b[this.name]);d.append($("<option />").val(a).text(this.name))})}});a.fail(function(a,c,b){401==a.status?k(function(){}):"timeout"==c&&P()})}function U(){var a=!1,c=parseInt(/WebKit\/([0-9]+)|$/.exec(navigator.appVersion)[1],10)&&0===navigator.vendor.indexOf("Google"),
c=!(c&&userAgent.indexOf("Android"))&&c&&0===navigator.platform.indexOf("Linux ")&&!(window.chrome||window.chrome.webstore)&&"ontouchstart"in document.documentElement,b=navigator.userAgent||navigator.vendor||window.opera;if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(b)||
...
4)))a=!0;return c||a}function s(a,c){TASK_CNTRL.showWindow(null,"#hourTypeWindow");jQuery("#done_hourType").unbind("click").bind("click",function(b){b.preventDefault();a.details.hourTypeID=jQuery("#hourTypeSelector").find(":selected").val();TASK_DB.updateDetails(a.ID,a.details);c(a);TASK_CNTRL.hideWindow("#hourTypeWindow")})}function E(a){var b=f.get("todaySched");null!=b?a(b):(b={url:T("v4.0")+"user/search?ID=$$USER.ID&fields=schedule:*,schedule:nonWorkDays:*",type:"get",data:{},dataType:"json"},
w(b),b=jQuery.ajax(b),b.done(function(b){if(void 0!=b.error)b.error["class"]==h[0]?k(function(){}):c(D[0],"getSchedule,"+JSON.stringify(b.error)),m.hide(),q.hide();else if(b=b.data[0].schedule,0!=b.length){var d=new Date,e=b["sunday monday tuesday wednesday thursday friday saturday".split(" ")[d.getDay()]],g;if(!0==b.hasNonWorkDays){for(var l=0;l<b.nonWorkDays.length;l++)if(b.nonWorkDays[l].nonWorkDate==d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate()&&(void 0!=g&&null!=b.nonWorkDays[l].userID||
void 0==g))g=b.nonWorkDays[l].scheduleDay;void 0!=g&&(e=g)}void 0!=e&&0!=e.length&&(e={start:e[0],stop:e[1]},f.set("todaySched",e));a&&a(e)}}),b.fail(function(c,b,d){401==c.status?k(function(){}):"timeout"==b&&E(a)}))}function O(){var a=new Date,a={url:T()+"hour/report?hours_AggFunc=sum",type:"get",data:{entryDate:a.getFullYear()+"-"+(a.getMonth()+1)+"-"+a.getDate(),ownerID:f.get("userID")},dataType:"json"};w(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):
c(D[0],"getTodaysTotalHours,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,jQuery("#totalHours").html("Today's Hours: "+(null==a.sum_hours?"0":a.sum_hours.toFixed(2))))});a.fail(function(a,c,b){401==a.status?k(function(){}):"timeout"==c&&O()})}function t(a){m.show();var b={assignedToID:f.get("userID")};"OPTASK"==a.details.objCode?b.opTaskID=a.syncID:b.taskID=a.syncID;b={url:T()+"assignment/search",type:"get",data:b,dataType:"json"};w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?
(b.error["class"]==h[0]?k(function(){t(a)}):c(D[0],"getTaskAssignment,"+a.syncID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data[0],a.running?(a.done=1,TASK_CNTRL.stopTask(a),TASK_CNTRL.toggleRunText()):A(b,a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){t(a)},"getTaskAssignment,"+a.syncID)})}function A(a,b){m.show();var d={url:T()+"assignment/"+a.ID,data:{status:"DN"},type:"PUT",dataType:"json"};fa&&w(d);d=jQuery.ajax(d);d.done(function(d){void 0!=d.error?
(d.error["class"]==h[0]?k(function(){A(a,b)}):c(D[0],"setAssignmentDone,"+a.ID+","+JSON.stringify(d.error)),m.hide(),q.hide()):(TASK_DB.remove(b.ID),m.hide(),q.hide(),c(D[2],R(b)+",assignment done"),xa(b))});d.fail(function(c,d,e){S({response:c.responseText,type:d,msg:e},function(){A(a,b)},"setAssignmentDone,"+a.ID+", "+c.responseText)})}function xa(a){m.show();var b={url:T()+"assignment/count",data:{status:"DN",status_Mod:"ne"},type:"GET",dataType:"json"};"OPTASK"==a.details.objCode?b.data.opTaskID=
a.syncID:b.data.taskID=a.syncID;w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){xa(a)}):c(D[0],"getTaskDoneAssignmentCount,"+taskID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,m.hide(),q.hide(),0==b.count&&y(a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){xa(a)},"getTaskDoneAssignmentCount,"+a.syncID)})}function y(a){var b=a.syncID;m.show();var d="OPTASK"==a.details.objCode?"CLS":"CPL",d={url:T()+a.details.objCode.toLowerCase()+
"/"+b,data:{status:d},type:"PUT",dataType:"json"};fa&&w(d);d=jQuery.ajax(d);d.done(function(d){void 0!=d.error?(d.error["class"]==h[0]?k(function(){y(a)}):S({response:d.error,type:null,msg:null},function(){y(a)},"setTaskDone,"+b),m.hide(),q.hide()):(m.hide(),q.hide(),c(D[2],R(a)+",task done"))});d.fail(function(c,d,e){S({response:c.responseText,type:d,msg:e},function(){y(a)},"setTaskDone,"+b+", "+c.responseText)})}function F(a){m.show();var c={url:T()+a.details.objCode.toLowerCase()+"/"+a.syncID,
data:{status:"INP"},type:"PUT",dataType:"json"};fa&&w(c);c=jQuery.ajax(c);c.done(function(c){void 0!=c.error?(c.error["class"]==h[0]?k(function(){F(a)}):S({response:c.error,type:null,msg:null},function(){F(a)},"setTaskInProgress,"+a.syncID),m.hide(),q.hide()):(m.hide(),q.hide(),!0==f.get("logout_on_select")&&oa())});c.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){F(a)},"setTaskInProgress-fail,"+a.syncID+", "+c.responseText)})}function B(a,c){m.show();var b={url:T()+a.details.objCode.toLowerCase()+
"/"+a.syncID,data:{updates:"{ percentComplete: "+c+"}"},type:"PUT",dataType:"json"};fa&&w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?b.error["class"]==h[0]?k(function(){B(a,c)}):S({response:b.error,type:null,msg:null},function(){B(a,c)},"setTaskPercent,"+a.syncID):(a.details.percentComplete=c,TASK_DB.updateDetails(a.ID,a.details));m.hide();q.hide()});b.fail(function(b,d,e){S({response:b.responseText,type:d,msg:e},function(){B(a,c)},"setTaskPercent-fail,"+a.syncID+", "+b.responseText)})}
...
(e.hourTypeID=a.details.hourTypeID));isNaN(d)&&(d=0);e.entryDate=a.dateStr;var g=Number((a.duration/3600).toFixed(2));f.get("nearestHour")&&(g=Math.round(g));e.hours=parseFloat((g+parseFloat((""+d).split(".").join("."))+" ").split(".").join("."));e.ownerID=f.get("userID");c(D[1],"setHourAttask, oldhr: "+d+" method: "+e.method+" start: "+a.dateStr+" task: "+JSON.stringify(a)+" sid: "+f.get("sessionID")+" time: "+(new Date).toTimeString());var l=function(){m.show();var b={url:T("v5.0")+"hour/",type:"get",
data:e,dataType:"json",async:!1};w(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){l()}):(S({response:b.error,type:null,msg:null},l,"setHourAttask,"+a.syncID+" "+a.name+" url: "+T()+" "+JSON.stringify(e)+" "+R(a)+","+a.duration,'Time was not tracked, please click on "View" for task: '+a.name+" to record time manually."),J(a)),m.hide(),q.hide()):(O(),void 0!=e.taskID||void 0!=e.opTaskID?F(a):!0==f.get("logout_on_select")&&oa(),m.hide(),q.hide(),c(D[2],R(a)),
a.done&&t(a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},l,"setHourAttask-fail,"+a.syncID+" "+a.name+" url: "+T()+" "+JSON.stringify(e)+" "+R(a)+","+a.duration+", "+c.responseText+", "+d,c.responseText+'<br>Time was not tracked, please click on "View" for task: '+a.name+" to record time manually.");J(a)})};l()}function J(a,b){var d={};d.entryDate=a.dateStr;d.fields="permissions,ID,hours";d.ownerID=f.get("userID");"GEN"==a.details.objCode?(d.hourTypeID=a.syncID,d.projectID=null,
d.taskID=null,d.opTaskID=null):("OPTASK"==a.details.objCode?d.opTaskID=a.syncID:d.taskID=a.syncID,void 0!=a.details.hourTypeID&&""!=a.details.hourTypeID&&(d.hourTypeID=a.details.hourTypeID));d={url:T()+"hour/search",type:"get",data:d,dataType:"json",async:!1};w(d);d=jQuery.ajax(d);d.done(function(d){if(void 0!=d.error)d.error["class"]==h[0]?k(function(){J(a,b)}):c(D[0],"getHour,"+a.syncID+","+JSON.stringify(d.error)),m.hide(),q.hide();else if(c(D[1],"getHour "+d.data.length+" "+JSON.stringify(d.data)),
b){for(var e=d.data.length,f=0;f<e;f++)if(-1!=d.data[f].permissions.actions.indexOf("ADD_HOURS")&&-1!=d.data[f].permissions.actions.indexOf("CHANGE_STATUS")){b(a,d.data[f]);return}b(a,d.data[0])}});d.fail(function(a,c,b){JSON.stringify(b)})}var h=["com.attask.common.AuthenticationException"],D=["error","debug","informational"],f,fa=!(window.hasOwnProperty("chrome")&&window.chrome.hasOwnProperty("windows")),u=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://")&&fa,z=-1!==document.URL.indexOf("dashboard.directory"),
W=!1,I=!1,Q="Force Desktop Stopwatch Access",Ba=300,va=[];f=!fa||u?new Store("Attask"):{settings:[],set:function(a,c){switch(a){case "domain":case "host":case "userID":case "sessionID":case "username":case "date":case "view":this.setCookie(a,c,9999999999)}this.settings[a]=c},get:function(a){switch(a){case "domain":case "host":case "userID":case "sessionID":case "username":case "date":case "view":var c=void 0===this.settings[a]?this.getCookie(a):this.settings[a];this.settings[a]=c;return"undefined"==
...
document.cookie=a+"="+c},getCookie:function(a){var c,b,d,e=document.cookie.split(";");for(c=0;c<e.length;c++)if(b=e[c].substr(0,e[c].indexOf("=")),d=e[c].substr(e[c].indexOf("=")+1),b=b.replace(/^\s+|\s+$/g,""),b==a)return decodeURI(d);return"undefined"}};var za;OB.init.add(function(){void 0==f.get("qr")&&f.set("qr","true"==jQuery.urlParam("qr")||!1!=jQuery.urlParam("code"));ba();ta();g();var a=jQuery.urlParam("s"),c=jQuery.urlParam("d");0!=c&&(0!=a&&f.set("sessionID",a),a=c.split("."),f.set("host",
a[0]),a.shift(),f.set("domain",a.join(".")),void 0==f.get("ent")&&(f.set("userID",jQuery.urlParam("u")),f.set("password",""),f.set("ent",void 0!=f.get("userID"))));l();!0==f.get("qr")&&void 0==f.get("sessionID")&&(!1!=jQuery.urlParam("code")?(document.getElementById("button-appscan").style.display="block",qrRead(jQuery.urlParam("code"),f)):qrLoad(f))});OB.onBeforeUnLoad.add(function(a){var c=TASK_CNTRL.getDuration(a);X(a,c.duration)});OB.onUnLoad.add(function(){TASK_DB.running(function(a){f.get("stopOnClose")&&
...
jQuery("#bar").hide()):jQuery("#bar").show();c=f.get("sort");void 0==c&&(c="date");jQuery("#sort-"+c).click();jQuery("#search-bar").is(":visible")&&jQuery("#search").click();jQuery("#loading").hide();a.show();a=jQuery("#holder")[0];a.style.display="none";a.style.display="block";m.hide();ma=!1}});OB.postItemRender.add(function(a){if(0==jQuery("#item"+a.ID+" #buttons .view").length){var c=jQuery("#item"+a.ID+" #buttons"),b=["",{label:"Low",css:"default"},{label:"Normal",css:"success"},{label:"High",
css:"warning"},{label:"Urgent",css:"danger"}];"GEN"!=a.details.objCode&&(void 0!=a.details.priority&&0!=a.details.priority)&&(b=b[a.details.priority],c.append('<span class="task priority '+b.css+'" title="'+b.label+'" data-priority="'+a.details.priority+'">'+b.label[0]+"</span> "));var d;"GEN"==a.details.objCode?(d="General",jQuery("#item"+a.ID).css({background:"aliceblue"})):"OPTASK"==a.details.objCode?(d="Issue",c.append('<span class="issue" title="Issue">I</span> ')):"TASK"==a.details.objCode&&
...
d.getTime())/1E3);jQuery("#item"+a.ID+" .timer .time").text(b.hms(e))},500);b.update_intervals[a.ID]=window.setInterval(function(){var e=Number(a.time)+Math.floor(((new Date).getTime()-d.getTime())/1E3);b.taskDB.updateAtInterval(a.ID,e,300)},3E5)},stopTask:function(a){window.clearInterval(this.display_intervals[a.ID]);window.clearInterval(this.update_intervals[a.ID]);var b=this.getDuration(a);0==b.duration&&jQuery("#item"+a.ID+" .timer").qtip("show");this.taskDB.stop(a.ID,b.total,b.duration,a.done);
jQuery("#item"+a.ID+" .timer .time").text(this.hms(b.total));jQuery("#item"+a.ID+" .timer .timer-info").text("Start")},getDuration:function(a){var b=Number(a.time);a=new Date(a.start);a=Math.floor(((new Date).getTime()-a.getTime())/1E3);30>a?a=0:60>a&&29<a&&(a=60);return{duration:a,total:b+a}},toggleRunText:function(){this.taskDB.running(function(a){e?window.chrome.browserAction.setBadgeText({text:"RUN"}):document.title="Stopwatch - Running: "+a.name;jQuery("#running").text("Running: "+a.name).attr("rel",
a.ID).show();jQuery("#storelink").hide()},function(a){0==a&&(e?window.chrome.browserAction.setBadgeText({text:""}):document.title="Stopwatch",jQuery("#running").hide(),jQuery("#storelink").show())})},hms:function(a){a=[0,0,a];var b;for(b=2;0<b;b--)a[b-1]=Math.floor(a[b]/60),a[b]%=60,10>a[b]&&(a[b]="0"+a[b]);return a.join(":")},sec:function(a){a=String(a).split(":");return Number(parseFloat(3600*a[0])+60*parseFloat(a[1])+parseFloat(a[2]))}}}var QUERY_LOOKUP=null;
