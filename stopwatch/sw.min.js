...
jQuery(document).on("click","#comment_confirm",function(a){a.preventDefault();a=jQuery(this);var c=jQuery("#comment_is_private").is(":checked"),h=jQuery("#comment_message").val();TASK_DB.updateValidate(a.attr("rel"),function(a){W(a,h,c)});TASK_CNTRL.hideWindow()});var m=jQuery("#form-list");m.scroll(function(){f.set("vpos",m.scrollTop())});q=jQuery(".spinner-bottom");n=jQuery(".spinner")}function Da(a,c){var h=jQuery("#"+a+"-filters").first(),k="",m;for(m in c)c.hasOwnProperty(m)&&(k+='<li><a href="#" id="button-'+
m+'" class="filter" title="'+c[m].d+'" >'+c[m].t+"</a></li>");h.append(k);Ca(c)}function Ca(a){var c=jQuery(".filter"),h=jQuery("#active-filter"),k;for(k in a)a.hasOwnProperty(k)&&(jQuery("#active-filter").hide(),jQuery("#button-"+k).attr("name",k),jQuery(document).on("click","#button-"+k,function(){var k=jQuery(this),e=k.attr("name");C=[];c.removeClass("btn");Ea();h.html("Filter: "+k.text());h.css("display","inline-block");k.toggleClass("btn");f.set("view",e);a[e].f()}))}function J(){if(L&&f.get("allowOffline"))f.remove("sessionID"),
f.remove("userID"),f.remove("username"),f.remove("password"),f.remove("host"),f.remove("domain");else{var a=f.get("qr");f.removeAll();f.set("qr",a)}for(var c in TASK_CNTRL.update_intervals)window.clearInterval(TASK_CNTRL.update_intervals[c]);for(c in TASK_CNTRL.display_intervals)window.clearInterval(TASK_CNTRL.display_intervals[c]);ga()}function D(a){TASK_DB.loadAll(function(c){var h=c.length,k,m;if(0<h)for(k=0;k<h;k++)m=c[k],(null!=m&&"OPTASK"==m.details.objCode||"TASK"==m.details.objCode)&&jQuery("#item"+
m.ID).hide();a()})}function v(a){f.get("ent")?ea(a):ha(f.get("username"),f.get("password"),ia(f.get("host")+"."+f.get("domain")),a,function(){ga(a)})}function ga(a){f.get("qr")?Na(a):ba(a)}function ea(a,c){n.show();jQuery("#loading").hide();window.clearInterval(O);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();n.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();b(function(){S(function(){ja();void 0!=a&&a()},function(a){void 0!=c?c():ga(a)})})}
function Na(a){n.show();jQuery("#loading").hide();window.clearInterval(O);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();resetForQR();n.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();jQuery("webcamimg").show();var c=jQuery("#form-qrlogin");jQuery("#form-list").html("");E?c.show():c.slideDown(400,function(){});jQuery("#button-qrlogin").unbind("click").click(function(c){var k=jQuery("#form-qrlogin");c.preventDefault();q.show();ha(f.get("username"),
f.get("password"),ia(f.get("host")),function(){ignoreForQR();jQuery("#bar").show();E?k.hide():k.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()},function(){resetForQR()})})}function ba(a){n.show();jQuery("#loading").hide();window.clearInterval(O);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();n.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();var c=jQuery("#form-login"),h=f.get("domain"),k=f.get("host");c.find(":input[name='username']").focus().val("");
c.find(":input[name='host']").val(void 0==k||"undefined"==k?"":"my.workfront.com"!=h?k+"."+h:k);c.find(":input[name='password']").val("");jQuery("#form-list").html("");var m=jQuery("#username");E?(c.show(),m.focus()):c.slideDown(400,function(){m.focus()});jQuery("#button-login").unbind("click").click(function(h){h.preventDefault();q.show();ha(jQuery.trim(c.find(":input[name='username']").val()),jQuery.trim(c.find(":input[name='password']").val()),ia(jQuery.trim(c.find(":input[name='host']").val())),
function(){jQuery("#bar").show();E?c.hide():c.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()})})}function M(a,c,h){var k=new Date;a.dateStr=k.getFullYear()+"-"+(k.getMonth()+1)+"-"+k.getDate();a.duration=c;navigator.onLine?X(a,Oa,h):(c=f.get("offlineTasks"),void 0==c&&(c=[]),c.push(a),f.set("offlineTasks",c),n.hide(),q.hide(),!0==f.get("logout_on_select")&&J())}function ka(a){X(a,function(a,h){var k=new Date,m={entryDate:k.getFullYear()+"-"+(k.getMonth()+1)+"-"+k.getDate(),
ownerID:f.get("userID")};m.extRefID=h&&h.extRefID?"ON!"+k.getHours()+":"+k.getMinutes()+";"+h.extRefID:"ON~"+k.getHours()+":"+k.getMinutes()+".";m.extRefID=m.extRefID.substring(0,254);void 0==h||-1!=h.permissions.actions.indexOf("ADD_HOURS")&&-1!=h.permissions.actions.indexOf("CHANGE_STATUS")?(m.hours=0.01,m.method="post"):(m.method="put",m.ID=h.ID);"GEN"==a.details.objCode?m.hourTypeID=a.syncID:("OPTASK"==a.details.objCode?m.opTaskID=a.syncID:m.taskID=a.syncID,m.hourTypeID=f.get("forceHourTypeID"),
void 0==m.hourTypeID&&(void 0!=a.details.hourTypeID&&""!=a.details.hourTypeID)&&(m.hourTypeID=a.details.hourTypeID));k={url:t("api/v7.0")+"hour/",type:"get",data:m,dataType:"json",async:!1};w(k);k=jQuery.ajax(k);k.done(function(h){void 0!=h.error?h.error["class"]==y[0]?v(function(){setHourAttask()}):x({response:h.error,type:null,msg:null},function(){ka(a)},"logStart,"+a.syncID+" "+a.name+" url: "+t()+" "+JSON.stringify(m)+" "+F(a)):(void 0!=m.taskID||void 0!=m.opTaskID)&&P(a)});k.fail(function(h,
k,e){x({response:h.responseText,type:k,msg:e},function(){ka(a)},"logStart-fail,"+a.syncID+" "+a.name+" url: "+t()+" "+JSON.stringify(m)+" "+F(a)+","+a.duration+", "+h.responseText+", "+e,h.responseText)})})}function fa(a){return"Jan. Feb. Mar. Apr. May June Jul. Aug. Sep. Oct. Nov. Dec.".split(" ")[a]}function F(a){return void 0!=a.details.project?a.details.project.name.split(",").join(" ")+","+a.name.split(",").join(" "):a.details}function w(a,c){void 0==c&&(c=1E4);a.timeout=c;I&&!Fa&&(a.dataType=
"jsonp",a.crossDomain=!0,a.jsonp="jsonp",a.cache=!0);var h=f.get("sessionID");f.get("ent")?("PUT"==a.data.method&&(a.type="POST"),a.data.apiKey=h):a.data.sessionID=h}function ia(a){if(1<a.split(".").length){var c=a.split("//");return 1<c.length?(c.shift(),c[0]):a}return a+".my.workfront.com"}function ha(a,c,h,k,m){if(void 0==a||void 0==c||void 0==h)m();else{var e=jQuery(".error");e.html("");var b={url:"https://"+h+"/attask/api/v"+(-1!=location.origin.indexOf("localhost")?2:5)+".0/login",type:"post",
data:{username:a,password:c,method:"post"},dataType:"json"};w(b);L?(b=f.get(a),void 0!=b?(Ga(b.sessionID,b.userID,b.username,b.password,h),q.hide(),G(),u(),void 0!=k&&k()):e.html("OFFLINE")):(b=jQuery.ajax(b),b.done(function(b){void 0!=b.error?(f.get("qr")?e.html("Please Scan Again."):e.html(b.error.message),q.hide(),void 0!==m&&m()):(b=b.data,Ga(b.sessionID,b.userID,a,c,h),f.set("userObj",JSON.stringify(b)),q.hide(),d(function(){f.get("ent")||G();u();void 0!=k&&k()}))}),b.fail(function(a,c,k){a.responseText&&
""!==a.responseText?-1!=a.responseText.indexOf("<html>")?f.get("qr")?e.html("Please Scan Again."):e.html("Host Invalid: "+h):f.get("qr")?e.html("Please Scan Again."):e.html(JSON.parse(a.responseText).error.message.replace("Authentication Exception: Authentication Exception: {0}","Wrong login, please try again.")):f.get("qr")?e.html("Please Scan Again."):e.html("Host Invalid: "+h);q.hide();void 0!==m&&m()}))}}function Ga(a,c,h,k,e){f.set("sessionID",a);f.set("userID",c);f.set("username",h);f.set("password",
k);e=e.split(".");f.set("host",e[0]);e.shift();f.set("domain",e.join("."));f.get("allowOffline")&&f.set(h,{sessionID:a,userID:c,username:h,password:k})}function S(a,c){var h=f.get("sessionID");void 0==h||""==h?v(a):(n.show(),h={url:t()+"user/"+f.get("userID"),data:{method:"put",updates:"{isActive: true}"},type:"get",dataType:"json"},w(h),h=jQuery.ajax(h),h.done(function(h){void 0!=h.error?(h.error["class"]==y[0]?void 0!=c?c(a):v(a):e(s[0],"sessionValid,"+f.get("sessionID")+","+JSON.stringify(h.error)),
n.hide(),q.hide()):(n.hide(),q.hide(),void 0==f.get("auth_key")?d(G):G(),void 0!=a&&a())}),h.fail(function(h,m,b){401==h.status?void 0!=c?c(a):v(a):"timeout"==m?S(a):e(s[0],"sessionValid,"+f.get("sessionID")+","+h.responseText)}))}function G(){n.show();la();Ha();var a=f.get("userID"),c={url:t("api-internal")+"timeSheet/search",type:"get",data:{startDate:"$$TODAY+1",startDate_Mod:"lte",endDate:"$$TODAY-1",endDate_Mod:"gte",fields:"extRefID,startDate,timesheetProfile:description,timesheetProfile:hourTypes:ID,timesheetProfile:hourTypes:name,timesheetProfile:hourTypes:description,timesheetProfile:hourTypes:isActive",
...
c,b){x({response:a.responseText,type:c,msg:b},qa,"addRecentTasks")})}function ra(){function a(a){var b={url:t()+"assgn/search?"+a+":statusEquatesWith=NEW&"+a+":statusEquatesWith=INP&"+a+":statusEquatesWith=ROP&project:statusEquatesWith=CUR&project:statusEquatesWith=QUE",type:"get",data:{},dataType:"json"};b.data[a+":statusEquatesWith_Mod"]="in";b.data[a+":isComplete"]=!1;b.data.status="AD";b.data["project:statusEquatesWith_Mod"]="in";b.data["project:projectedCompletionDate"]="$$TODAYb-1w";b.data["project:projectedCompletionDate_Mod"]=
"gt";b.data.assignedToID=f.get("userID");b.data.fields=a+":parent,"+a+":priority,"+a+":project, "+a+":project:hourTypes,"+a+":project:status,"+a+":status, "+a+":extRefID,"+a+":permissions,"+a+":plannedCompletionDate";"task"==a&&(b.data.fields+=",task:percentComplete");w(b);return b}n.show();jQuery.when(jQuery.ajax(a("task")),jQuery.ajax(a("opTask"))).done(function(a,b){a=a[0];b=b[0];var k=void 0!=a.error?a.error:b.error;void 0!=k?(k["class"]==y[0]?v(function(){ra()}):e(s[0],"addCurrentTasks,"+JSON.stringify(k)),
n.hide(),q.hide()):0==a.data.length&&0==b.data.length?(A=!0,D(function(){TASK_CNTRL.showAll()})):D(function(){function e(a,c){for(var b=a.length,h=0;h<b;h++){var k=a[h],k=k[c];TASK_DB.insert(k.name,{syncID:k.ID,details:k},h==b-1)}}A=!0;e(a.data,"task");e(b.data,"opTask")})}).fail(function(a,b,e){x({response:a.responseText,type:b,msg:e},function(){ra()},"addCurrentTasks")})}function sa(){function a(a){var b={url:t()+"assgn/search?"+a+":statusEquatesWith=NEW&"+a+":statusEquatesWith=INP&"+a+":statusEquatesWith=ROP&project:statusEquatesWith=CUR&project:statusEquatesWith=QUE",
...
var l;l=0<p?"days due: "+p:"days late: "+Math.abs(p);e='<div class="date" style="background-color: '+e+';"><div class="days">'+l+"</div><div>"+fa(d.getMonth())+" "+d.getDate()+", "+d.getFullYear()+"</div></div>";b.attr("data-days",p)}else b.attr("data-days",9999);a.disable||(e+='<span class="hour-note" rel="'+a.ID+'">Add Note</span>');d="";null!=a.details.parent&&(d="<br><br>Parent Task: <br>"+a.details.parent.name);p="";a.details.extRefID&&(p="Reference: <br>"+a.details.extRefID);c.find(".name").attr("title",
g+": <br>"+a.name+p+d);"General"!=g?(g=a.details.project.name,b.html(g).attr("title","Project: <br>"+g+"<br>Status: "+a.details.project.status).attr("data-priority",a.details.priority),c.find(".c6").after('<div class="c7">'+e+"</div>"),E||c.find(".c7").mouseover(function(){var a=jQuery(this).find(".hour-note"),b=a.attr("rel");jQuery("#item"+b);if((b=TASK_DB.get(b))&&b.time&&!b.running&&!b.disable){var c=jQuery(this).find(".date").hide(),e=a.show();setTimeout(function(){c.show();e.hide()},3E3)}}).mouseout(function(){jQuery(this).find(".date").show();
jQuery(this).find(".hour-note").hide()})):b.html(a.details.description).attr("title",a.details.description)}});OB.remove.add(function(a){jQuery("#item"+a).remove()});OB.reset.add(function(a,b){OB.update.dispatch(b,-b.time);jQuery("#item"+a+" .timer").text(TASK_CNTRL.hms(0))});var A=!1,q,n}();var QUERY_LOOKUP=null;function getQueryString(b){if(null==QUERY_LOOKUP){var d=location.search.substring(1),g=/([^&=]+)=([^&]*)/g,l;for(QUERY_LOOKUP={};l=g.exec(d);)QUERY_LOOKUP[decodeURIComponent(l[1])]=decodeURIComponent(l[2])}return b in QUERY_LOOKUP?QUERY_LOOKUP[b]:null}
