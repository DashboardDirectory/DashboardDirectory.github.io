...
cases:e,forceStrings:e,sortFunction:null}};d.fn.extend({tinysort:function(Z,sa){Z&&"string"!=typeof Z&&(sa=Z,Z=null);var ba=d.extend({},d.tinysort.defaults,sa),ta,pa=d(this).length,r={},k=!(!Z||""==Z),K=!(null===ba.attr||""==ba.attr),ha=null!==ba.data,da=k&&":"==Z[0],ka=da?this.filter(Z):this,R=ba.sortFunction,V="asc"==ba.order?1:-1,W=[];d.each(l,function(b,a){a.call(a,ba)});R||(R="rand"==ba.order?function(){return 0.5>Math.random()?1:-1}:function(a,c){var k=e,l=!ba.cases?a.s&&a.s.toLowerCase?a.s.toLowerCase():
a.s:a.s,r=!ba.cases?c.s&&c.s.toLowerCase?c.s.toLowerCase():c.s:c.s;if(!ba.forceStrings){var x=l&&l.match(g),K=r&&r.match(g);if(x&&K){var N=l.substr(0,l.length-x[0].length),M=r.substr(0,r.length-K[0].length);N==M&&(k=!e,l=b(x[0]),r=b(K[0]))}}var R=V*(l<r?-1:l>r?1:0);d.each(oa,function(b,a){R=a.call(a,k,l,r,R)});return R});this.each(function(b,a){var c=d(a),e=k?da?ka.filter(a):c.find(Z):c,g=ha?""+e.data(ba.data):K?e.attr(ba.attr):ba.useVal?e.val():e.text(),l=c.parent();r[l]||(r[l]={s:[],n:[]});0<e.length?
r[l].s.push({s:g,e:c,n:b}):r[l].n.push({e:c,n:b})});for(ta in r)r[ta].s.sort(R);for(ta in r){var R=r[ta],x=[],Y=pa,N=[0,0],ra;switch(ba.place){case "first":d.each(R.s,function(b,a){Y=c(Y,a.n)});break;case "org":d.each(R.s,function(b,a){x.push(a.n)});break;case "end":Y=R.n.length;break;default:Y=0}for(ra=0;ra<pa;ra++){var ea=a(x,ra)?!e:ra>=Y&&ra<Y+R.s.length,M=(ea?R.s:R.n)[N[ea?0:1]].e;M.parent().append(M);(ea||!ba.returns)&&W.push(M.get(0));N[ea?0:1]++}}this.length=0;Array.prototype.push.apply(this,
...
{"class":"ui-icon ui-icon-close",html:"&times;"}));I.button.appendTo(I.titlebar||v).attr("role","button").click(function(a){return v.hasClass(X)||h.hide(a),k})}function xa(){var a=D+"-title";I.titlebar&&t();I.titlebar=b("<div />",{"class":ca+"-titlebar "+(n.style.widget?E("header"):"")}).append(I.title=b("<div />",{id:a,"class":ca+"-title","aria-atomic":r})).insertBefore(I.content).delegate(".qtip-close","mousedown keydown mouseup keyup mouseout",function(a){b(this).toggleClass("ui-state-active ui-state-focus",
"down"===a.type.substr(-4))}).delegate(".qtip-close","mouseover mouseout",function(a){b(this).toggleClass("ui-state-hover","mouseover"===a.type)});n.content.title.button&&A()}function y(a,c){var d=I.title;if(!h.rendered||!a)return k;b.isFunction(a)&&(a=a.call(w,Q.event,h));if(a===k||!a&&""!==a)return t(k);a.jquery&&0<a.length?d.empty().append(a.css({display:"block"})):d.html(a);c!==k&&h.rendered&&0<v[0].offsetWidth&&h.reposition(Q.event)}function F(a){a&&b.isFunction(a.done)&&a.done(function(a){B(a,
null,k)})}function B(a,c,d){function f(a){function d(u){u&&(delete L[u.src],clearTimeout(h.timers.img[u.src]),b(u).unbind(z));b.isEmptyObject(L)&&(c!==k&&h.reposition(Q.event),a())}var u,L={};if(0===(u=q.find("img[src]:not([height]):not([width])")).length)return d();u.each(function(a,u){if(L[u.src]===e){var c=0;(function Ca(){if(u.height||u.width||3<c)return d(u);c+=1;h.timers.img[u.src]=setTimeout(Ca,700)})();b(u).bind("error"+z+" load"+z,function(){d(this)});L[u.src]=u}})}var q=I.content;return!h.rendered||
!a?k:(b.isFunction(a)&&(a=a.call(w,Q.event,h)||""),d!==k&&F(n.content.deferred),a.jquery&&0<a.length?q.empty().append(a.css({display:"block"})):q.html(a),0>h.rendered?v.queue("fx",f):(fa=0,f(b.noop)),h)}function G(){function e(u){if(v.hasClass(X))return k;clearTimeout(h.timers.show);clearTimeout(h.timers.hide);var a=function(){h.toggle(r,u)};0<n.show.delay?h.timers.show=setTimeout(a,n.show.delay):a()}function g(u){if(v.hasClass(X)||f||fa)return k;var a=b(u.relatedTarget||u.target),c=a.closest(aa)[0]===
v[0],a=a[0]===m.show[0];clearTimeout(h.timers.show);clearTimeout(h.timers.hide);if("mouse"===q.target&&c||n.hide.fixed&&/mouse(out|leave|move)/.test(u.type)&&(c||a))try{u.preventDefault(),u.stopImmediatePropagation()}catch(d){}else 0<n.hide.delay?h.timers.hide=setTimeout(function(){h.hide(u)},n.hide.delay):h.hide(u)}function l(u){if(v.hasClass(X))return k;clearTimeout(h.timers.inactive);h.timers.inactive=setTimeout(function(){h.hide(u)},n.hide.inactive)}function G(u){h.rendered&&0<v[0].offsetWidth&&
h.reposition(u)}var q=n.position,m={show:n.show.target,hide:n.hide.target,viewport:b(q.viewport),document:b(a),body:b(a.body),window:b(d)},s=b.trim(""+n.show.event).split(" "),u=b.trim(""+n.hide.event).split(" ");b.browser.msie&&parseInt(b.browser.version,10);v.bind("mouseenter"+z+" mouseleave"+z,function(u){var a="mouseenter"===u.type;a&&h.focus(u);v.toggleClass(ya,a)});/mouse(out|leave)/i.test(n.hide.event)&&"window"===n.hide.leave&&m.window.bind("mouseout"+z+" blur"+z,function(u){!/select|option/.test(u.target.nodeName)&&
!u.relatedTarget&&h.hide(u)});n.hide.fixed?(m.hide=m.hide.add(v),v.bind("mouseover"+z,function(){v.hasClass(X)||clearTimeout(h.timers.hide)})):/mouse(over|enter)/i.test(n.show.event)&&m.hide.bind("mouseleave"+z,function(u){clearTimeout(h.timers.show)});-1<(""+n.hide.event).indexOf("unfocus")&&q.container.closest("html").bind("mousedown"+z+" touchstart"+z,function(u){var a=b(u.target);h.rendered&&v.hasClass(X);var c=0<a.parents(aa).filter(v[0]).length;a[0]!==w[0]&&a[0]!==v[0]&&!c&&!w.has(a[0]).length&&
!a.attr("disabled")&&h.hide(u)});"number"==typeof n.hide.inactive&&(m.show.bind("qtip-"+P+"-inactive",l),b.each(M.inactiveEvents,function(u,a){m.hide.add(I.tooltip).bind(a+z+"-inactive",l)}));b.each(u,function(u,a){var c=b.inArray(a,s),d=b(m.hide);-1<c&&d.add(m.show).length===d.length||"unfocus"===a?(m.show.bind(a+z,function(u){0<v[0].offsetWidth?g(u):e(u)}),delete s[c]):m.hide.bind(a+z,g)});b.each(s,function(u,a){m.show.bind(a+z,e)});"number"==typeof n.hide.distance&&m.show.add(v).bind("mousemove"+
z,function(u){var a=Q.origin||{},b=n.hide.distance,c=Math.abs;(c(u.pageX-a.pageX)>=b||c(u.pageY-a.pageY)>=b)&&h.hide(u)});"mouse"===q.target&&(m.show.bind("mousemove"+z,c),q.adjust.mouse&&(n.hide.event&&(v.bind("mouseleave"+z,function(u){(u.relatedTarget||u.target)!==m.show[0]&&h.hide(u)}),I.target.bind("mouseenter"+z+" mouseleave"+z,function(u){Q.onTarget="mouseenter"===u.type})),m.document.bind("mousemove"+z,function(u){h.rendered&&Q.onTarget&&!v.hasClass(X)&&0<v[0].offsetWidth&&h.reposition(u||
ja)})));(q.adjust.resize||m.viewport.length)&&(b.event.special.resize?m.viewport:m.window).bind("resize"+z,G);m.window.bind("scroll"+z,G)}function J(){var c=[n.show.target[0],n.hide.target[0],h.rendered&&I.tooltip[0],n.position.container[0],n.position.viewport[0],n.position.container.closest("html")[0],d,a];h.rendered?b([]).pushStack(b.grep(c,function(a){return"object"==typeof a})).unbind(z):n.show.target.unbind(z+"-create")}var h=this,D=ca+"-"+P,f=0,fa=0,v=b(),z=".qtip-"+P,X="qtip-disabled",I,Q;
h.id=P;h.rendered=k;h.destroyed=k;h.elements=I={target:w};h.timers={img:{}};h.options=n;h.checks={};h.plugins={};h.cache=Q={event:{},target:b(),disabled:k,attr:l,onTarget:k,lastClass:""};h.checks.builtin={"^id$":function(a,c,d){a=d===r?M.nextid:d;c=ca+"-"+a;a!==k&&0<a.length&&!b("#"+c).length&&(v[0].id=c,I.content[0].id=c+"-content",I.title[0].id=c+"-title")},"^content.text$":function(a,b,c){B(n.content.text)},"^content.deferred$":function(a,b,c){F(n.content.deferred)},"^content.title.text$":function(a,
b,c){if(!c)return t();!I.title&&c&&xa();y(c)},"^content.title.button$":function(a,b,c){a=I.button;h.rendered&&(c?A():a.remove())},"^position.(my|at)$":function(a,b,c){"string"==typeof c&&(a[b]=new C.Corner(c))},"^position.container$":function(a,b,c){h.rendered&&v.appendTo(c)},"^show.ready$":function(){h.rendered?h.toggle(r):h.render(1)},"^style.classes$":function(a,b,c){v.attr("class",ca+" qtip "+c)},"^style.width|height":function(a,b,c){v.css(b,c)},"^style.widget|content.title":O,"^events.(render|show|move|hide|focus|blur)$":function(a,
c,d){v[(b.isFunction(d)?"":"un")+"bind"]("tooltip"+c,d)},"^(show|hide|position).(event|target|fixed|inactive|leave|distance|viewport|adjust)":function(){var a=n.position;v.attr("tracking","mouse"===a.target&&a.adjust.mouse);J();G()}};b.extend(h,{_triggerEvent:function(a,c,d){a=b.Event("tooltip"+a);return a.originalEvent=(d?b.extend({},d):K)||Q.event||K,v.trigger(a,[h].concat(c||[])),!a.isDefaultPrevented()},render:function(a){if(h.rendered)return h;var c=n.content.text,d=n.content.title,e=n.position;
return b.attr(w[0],"aria-describedby",D),v=I.tooltip=b("<div/>",{id:D,"class":[ca,wa,n.style.classes,ca+"-pos-"+n.position.my.abbrev()].join(" "),width:n.style.width||"",height:n.style.height||"",tracking:"mouse"===e.target&&e.adjust.mouse,role:"alert","aria-live":"polite","aria-atomic":k,"aria-describedby":D+"-content","aria-hidden":r}).toggleClass(X,Q.disabled).data("qtip",h).appendTo(n.position.container).append(I.content=b("<div />",{"class":ca+"-content",id:D+"-content","aria-atomic":r})),h.rendered=
-1,f=1,d.text?(xa(),b.isFunction(d.text)||y(d.text,k)):d.button&&A(),(!b.isFunction(c)||c.then)&&B(c,k),h.rendered=r,O(),b.each(n.events,function(a,c){b.isFunction(c)&&v.bind("toggle"===a?"tooltipshow tooltiphide":"tooltip"+a,c)}),b.each(C,function(){"render"===this.initialize&&this(h)}),G(),v.queue("fx",function(b){h._triggerEvent("render");f=0;(n.show.ready||a)&&h.toggle(r,Q.event,k);b()}),h},get:function(a){switch(a.toLowerCase()){case "dimensions":a={height:v.outerHeight(k),width:v.outerWidth(k)};
break;case "offset":a=C.offset(v,n.position.container);break;default:a=s(a.toLowerCase()),a=a[0][a[1]],a=a.precedance?a.string():a}return a},set:function(a,c){var d=/^position\.(my|at|adjust|target|container)|style|content|show\.ready/i,e=k,w=h.checks,m;return"string"==typeof a?(m=a,a={},a[m]=c):a=b.extend(r,{},a),b.each(a,function(c,u){var L=s(c.toLowerCase()),ia;ia=L[0][L[1]];L[0][L[1]]="object"==typeof u&&u.nodeType?b(u):u;a[c]=[L[0],L[1],u,ia];e=d.test(c)||e}),g(n),f=1,b.each(a,function(a,u){var b,
c,d;for(b in w)for(c in w[b])if(d=RegExp(c,"i").exec(a))u.push(d),w[b][c].apply(h,u)}),f=0,h.rendered&&0<v[0].offsetWidth&&e&&h.reposition("mouse"===n.position.target?K:Q.event),h},toggle:function(d,f,e){function w(){d?(b.browser.msie&&v[0].style.removeAttribute("filter"),v.css("overflow",""),"string"==typeof m.autofocus&&b(m.autofocus,v).focus(),m.target.trigger("qtip-"+P+"-inactive")):v.css({display:"",visibility:"",opacity:"",left:"",top:""});h._triggerEvent(d?"visible":"hidden")}if(f){if(/over|enter/.test(f.type)&&
/out|leave/.test(Q.event.type)&&n.show.target.add(f.target).length===n.show.target.length&&v.has(f.relatedTarget).length)return h;Q.event=b.extend({},f)}if(!h.rendered)return d?h.render(1):h;var q=d?"show":"hide",m=n[q],g=n.position,u=n.content,L=0<v[0].offsetWidth,ia=d||1===m.target.length,ga=!f||2>m.target.length||Q.target[0]===f.target;return(typeof d).search("boolean|number")&&(d=!L),!v.is(":animated")&&L===d&&ga?h:h._triggerEvent(q,[90])?(b.attr(v[0],"aria-hidden",!d),d?(Q.origin=b.extend({},
ja),h.focus(f),b.isFunction(u.text)&&B(u.text,k),b.isFunction(u.title.text)&&y(u.title.text,k),!ua&&"mouse"===g.target&&g.adjust.mouse&&(b(a).bind("mousemove.qtip",c),ua=r),h.reposition(f,e),!m.solo||b(aa,m.solo).not(v).qtip("hide",b.Event("tooltipsolo"))):(clearTimeout(h.timers.show),delete Q.origin,ua&&!b(aa+'[tracking="true"]:visible',m.solo).not(v).length&&(b(a).unbind("mousemove.qtip"),ua=k),h.blur(f)),m.effect===k||ia===k?(v[q](),w.call(v)):b.isFunction(m.effect)?(v.stop(1,1),m.effect.call(v,
h),v.queue("fx",function(a){w();a()})):v.fadeTo(90,d?1:0,w),d&&m.target.trigger("qtip-"+P+"-inactive"),h):h},show:function(a){return h.toggle(r,a)},hide:function(a){return h.toggle(k,a)},focus:function(a){if(!h.rendered)return h;var c=b(aa),d=parseInt(v[0].style.zIndex,10),f=M.zindex+c.length;a=b.extend({},a);return v.hasClass(qa)||h._triggerEvent("focus",[f],a)&&(d!==f&&(c.each(function(){this.style.zIndex>d&&(this.style.zIndex-=1)}),c.filter("."+qa).qtip("blur",a)),v.addClass(qa)[0].style.zIndex=
f),h},blur:function(a){return v.removeClass(qa),h._triggerEvent("blur",[v.css("zIndex")],a),h},reposition:function(c,e){if(!h.rendered||f)return h;f=1;var w=n.position.target,P=n.position,q=P.my,m=P.at,g=P.adjust,u=g.method.split(" "),L=v.outerWidth(k),ia=v.outerHeight(k),ga=0,ma=0,l=v.css("position"),G=P.viewport,z={left:0,top:0},s=P.container,r=0<v[0].offsetWidth,y=c&&"scroll"===c.type,J=b(d),U,X;if(b.isArray(w)&&2===w.length)m={x:W,y:V},z={left:w[0],top:w[1]};else if("mouse"===w&&(c&&c.pageX||
Q.event.pageX))m={x:W,y:V},c=ja&&ja.pageX&&(g.mouse||!c||!c.pageX)?{pageX:ja.pageX,pageY:ja.pageY}:(!c||"resize"!==c.type&&"scroll"!==c.type?c&&c.pageX&&"mousemove"===c.type?c:!g.mouse&&Q.origin&&Q.origin.pageX&&n.show.distance?Q.origin:c:Q.event)||c||Q.event||ja||{},"static"!==l&&(z=s.offset()),z={left:c.pageX-z.left,top:c.pageY-z.top},g.mouse&&y&&(z.left-=ja.scrollX-J.scrollLeft(),z.top-=ja.scrollY-J.scrollTop());else{"event"===w&&c&&c.target&&"scroll"!==c.type&&"resize"!==c.type?Q.target=b(c.target):
"event"!==w&&(Q.target=b(w.jquery?w:I.target));w=Q.target;w=b(w).eq(0);if(0===w.length)return h;w[0]===a||w[0]===d?(ga=C.iOS?d.innerWidth:w.width(),ma=C.iOS?d.innerHeight:w.height(),w[0]===d&&(z={top:(G||w).scrollTop(),left:(G||w).scrollLeft()})):C.imagemap&&w.is("area")?U=C.imagemap(h,w,m,C.viewport?u:k):C.svg&&w[0].ownerSVGElement?U=C.svg(h,w,m,C.viewport?u:k):(ga=w.outerWidth(k),ma=w.outerHeight(k),z=C.offset(w,s));U&&(ga=U.width,ma=U.height,X=U.offset,z=U.position);if(3.1<C.iOS&&4.1>C.iOS||4.3<=
C.iOS&&4.33>C.iOS||!C.iOS&&"fixed"===l)z.left-=J.scrollLeft(),z.top-=J.scrollTop();z.left+=m.x===Y?ga:m.x===N?ga/2:0;z.top+=m.y===x?ma:m.y===N?ma/2:0}return z.left+=g.x+(q.x===Y?-L:q.x===N?-L/2:0),z.top+=g.y+(q.y===x?-ia:q.y===N?-ia/2:0),C.viewport?(z.adjusted=C.viewport(h,z,P,ga,ma,L,ia),X&&z.adjusted.left&&(z.left+=X.left),X&&z.adjusted.top&&(z.top+=X.top)):z.adjusted={left:0,top:0},h._triggerEvent("move",[z,G.elem||G],c)?(delete z.adjusted,e===k||!r||isNaN(z.left)||isNaN(z.top)||"mouse"===w||!b.isFunction(P.effect)?
v.css(z):b.isFunction(P.effect)&&(P.effect.call(v,h,b.extend({},z)),v.queue(function(a){b(this).css({opacity:"",height:""});b.browser.msie&&this.style.removeAttribute("filter");a()})),f=0,h):h},disable:function(a){return"boolean"!=typeof a&&(a=!v.hasClass(X)&&!Q.disabled),h.rendered?(v.toggleClass(X,a),b.attr(v[0],"aria-disabled",a)):Q.disabled=!!a,h},enable:function(){return h.disable(k)},destroy:function(){var a=w[0],c=b.attr(a,la),d=w.data("qtip");h.destroyed=r;h.rendered&&(v.stop(1,0).remove(),
b.each(h.plugins,function(){this.destroy&&this.destroy()}));clearTimeout(h.timers.show);clearTimeout(h.timers.hide);J();if(!d||h===d)b.removeData(a,"qtip"),n.suppress&&c&&(b.attr(a,"title",c),w.removeAttr(la)),w.removeAttr("aria-describedby");return w.unbind(".qtip-"+P),delete S[h.id],w}})}function oa(c,d){var e,U,s,E,O,t=b(this),A=b(a.body),x=this===a?A:t;U=t.metadata?t.metadata(d.metadata):K;E="html5"===d.metadata.type&&U?U[d.metadata.name]:K;var y=t.data(d.metadata.name||"qtipopts");try{y="string"==
typeof y?b.parseJSON(y):y}catch(F){}E=b.extend(r,{},M.defaults,d,"object"==typeof y?g(y):K,g(E||U));U=E.position;E.id=c;if("boolean"==typeof E.content.text){s=t.attr(E.content.attr);if(E.content.attr===k||!s)return k;E.content.text=s}U.container.length||(U.container=A);U.target===k&&(U.target=x);E.show.target===k&&(E.show.target=x);E.show.solo===r&&(E.show.solo=U.container.closest("body"));E.hide.target===k&&(E.hide.target=x);E.position.viewport===r&&(E.position.viewport=U.container);U.container=
U.container.eq(0);U.at=new C.Corner(U.at);U.my=new C.Corner(U.my);if(b.data(this,"qtip"))if(E.overwrite)t.qtip("destroy");else if(E.overwrite===k)return k;return E.suppress&&(O=b.attr(this,"title"))&&b(this).removeAttr("title").attr(la,O).attr("title",""),e=new l(t,E,c,!!s),b.data(this,"qtip",e),t.bind("remove.qtip-"+c+" removeqtip.qtip-"+c,function(){e.destroy()}),e}function Z(a){var c=this,d=a.elements.tooltip,e=a.options.content.ajax,g=M.defaults.content.ajax,l=/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
O=r,t=k,A;a.checks.ajax={"^content.ajax":function(a,b,w){"ajax"===b&&(e=w);"once"===b?c.init():e&&e.url?c.load():d.unbind(".qtip-ajax")}};b.extend(c,{init:function(){return e&&e.url&&d.unbind(".qtip-ajax")[e.once?"one":"bind"]("tooltipshow.qtip-ajax",c.load),c},load:function(d){function P(){var c;a.destroyed||(O=k,D&&(t=r,a.show(d.originalEvent)),(c=g.complete||e.complete)&&b.isFunction(c)&&c.apply(e.context||a,arguments))}function F(c,d,f){var n;a.destroyed||(h&&"string"==typeof c&&(c=b("<div/>").append(c.replace(l,
...
A.abort();a.destroyed=r}});c.init()}function sa(a,c,b){var d=Math.ceil(c/2),e=Math.ceil(b/2);c={bottomright:[[0,0],[c,b],[c,0]],bottomleft:[[0,0],[c,0],[0,b]],topright:[[0,b],[c,0],[c,b]],topleft:[[0,0],[0,b],[c,b]],topcenter:[[0,b],[d,0],[c,b]],bottomcenter:[[0,0],[c,0],[d,b]],rightcenter:[[0,0],[c,e],[0,b]],leftcenter:[[c,0],[c,b],[0,e]]};return c.lefttop=c.bottomright,c.righttop=c.bottomleft,c.leftbottom=c.topright,c.rightbottom=c.topleft,c[a.string()]}function ba(a,c){var d,g;function l(a){var c=
J.is(":visible");J.show();a();J.toggle(c)}function E(c,b,d,f){if(G.tip){c=F.corner.clone();b=d.adjusted;var n=a.options.position.adjust.method.split(" ");f=n[0];var n=n[1]||n[0],g=k,l=k,P=0,J=0,q,m={},s;F.corner.fixed!==r&&(f===ea&&c.precedance===ha&&b.left&&c.y!==N?c.precedance=c.precedance===ha?da:ha:f!==ea&&b.left&&(c.x=c.x===N?0<b.left?W:Y:c.x===W?Y:W),n===ea&&c.precedance===da&&b.top&&c.x!==N?c.precedance=c.precedance===da?ha:da:n!==ea&&b.top&&(c.y=c.y===N?0<b.top?V:x:c.y===V?x:V),c.string()!==
h.corner.string()&&(h.top!==b.top||h.left!==b.left)&&F.update(c,k));q=F.position(c,b);q[c.x]+=O(c,c.x);q[c.y]+=O(c,c.y);q.right!==e&&(q.left=-q.right);q.bottom!==e&&(q.top=-q.bottom);q.user=Math.max(0,B.offset);if(g=f===ea&&!!b.left)c.x===N?m["margin-left"]=P=q["margin-left"]:(s=q.right!==e?[b.left,-q.left]:[-b.left,q.left],(P=Math.max(s[0],s[1]))>s[0]&&(d.left-=b.left,g=k),m[q.right!==e?Y:W]=P);if(l=n===ea&&!!b.top)c.y===N?m["margin-top"]=J=q["margin-top"]:(s=q.bottom!==e?[b.top,-q.top]:[-b.top,
q.top],(J=Math.max(s[0],s[1]))>s[0]&&(d.top-=b.top,l=k),m[q.bottom!==e?x:V]=J);G.tip.css(m).toggle(!(P&&J||c.x===N&&J||c.y===N&&P));d.left-=q.left.charAt?q.user:f!==ea||l||!g&&!l?q.left:0;d.top-=q.top.charAt?q.user:n!==ea||g||!g&&!l?q.top:0;h.left=b.left;h.top=b.top;h.corner=c.clone()}}function O(a,c,b){c=c?c:a[a.precedance];var d=G.titlebar&&a.y===V?G.titlebar:J,f="border-"+c+"-width",e;return l(function(){e=(b?parseInt(b.css(f),10):parseInt(G.content.css(f),10)||parseInt(d.css(f),10)||parseInt(J.css(f),
...
"border-"+a[a.precedance]+"-color",n="transparent",w=G.titlebar,y=w&&(a.y===V||a.y===N&&f.position().top+D.height/2+B.offset<w.outerHeight(r))?w:G.content;l(function(){d=c(f,"background-color")||c(y,"background-color")||c(G.content,"background-color")||c(J,"background-color")||f.css("background-color");g=c(f,h,"color")||c(y,h,"color")||c(G.content,h,"color")||c(J,h,"color")||J.css(h);b("*",f).add(f).css("cssText","background-color:"+n+" !important;border:0 !important;")})}function M(a){var c=a.precedance===
da,b=D[c?ka:R],d=D[c?R:ka];a=-1<a.string().indexOf(N);var e=b*(a?0.5:1),h=Math.pow,n=Math.round,g,w,k,m=Math.sqrt(h(e,2)+h(d,2)),e=[f/e*m,f/d*m];return e[2]=Math.sqrt(h(e[0],2)-h(f,2)),e[3]=Math.sqrt(h(e[1],2)-h(f,2)),g=m+e[2]+e[3]+(a?0:e[0]),w=g/m,k=[n(w*d),n(w*b)],{height:k[c?0:1],width:k[c?1:0]}}function y(a,c,b){return"<qvml:"+a+' xmlns="urn:schemas-microsoft.com:vml" class="qtip-vml" '+(c||"")+' style="behavior: url(#default#VML); '+(b||"")+'" />'}var F=this,B=a.options.style.tip,G=a.elements,
...
height:c}).prependTo(J);fa?b("<canvas />").appendTo(G.tip)[0].getContext("2d").save():(d=y("shape",'coordorigin="0,0"',"position:absolute;"),G.tip.html(d+d),b("*",G.tip).bind("click mousedown",function(a){a.stopPropagation()}))},update:function(a,c){var e=G.tip,n=e.children(),w=D.width,l=D.height,s=B.mimic,E=Math.round,t,q,m,H;a||(a=h.corner||F.corner);s===k?s=a:(s=new C.Corner(s),s.precedance=a.precedance,"inherit"===s.x?s.x=a.x:"inherit"===s.y?s.y=a.y:s.x===s.y&&(s[a.precedance]=a[a.precedance]));
a.precedance===ha?(D.width=B.height,D.height=B.width):(D.width=B.width,D.height=B.height);G.tip.css({width:w=D.width,height:l=D.height});A(a);"transparent"!==g?(f=O(a,K),0===B.border&&0<f&&(d=g),F.border=f=B.border!==r?B.border:f):F.border=f=0;q=sa(s,w,l);F.size=H=M(a);e.css(H).css("line-height",H.height+"px");a.precedance===da?m=[E(s.x===W?f:s.x===Y?H.width-w-f:(H.width-w)/2),E(s.y===V?H.height-l:0)]:m=[E(s.x===W?H.width-w:0),E(s.y===V?f:s.y===x?H.height-l-f:(H.height-l)/2)];fa?(n.attr(H),t=n[0].getContext("2d"),
...
10)?2:1:0,n.css({coordsize:w+f+" "+(l+f),antialias:""+(-1<s.string().indexOf(N)),left:m[0],top:m[1],width:w+f,height:l+f}).each(function(a){var c=b(this);c[c.prop?"prop":"attr"]({coordsize:w+f+" "+(l+f),path:q,fillcolor:d,filled:!!a,stroked:!a}).toggle(!!f||!!a);!a&&""===c.html()&&c.html(y("stroke",'weight="'+2*f+'px" color="'+g+'" miterlimit="1000" joinstyle="miter"'))}));c!==k&&F.position(a)},position:function(a){var c=G.tip,d={},f=Math.max(0,B.offset),e,h,n;return B.corner===k||!c?k:(a=a||F.corner,
e=a.precedance,h=M(a),n=[a.x,a.y],e===ha&&n.reverse(),b.each(n,function(c,b){var n,w,g;b===N?(n=e===da?W:V,d[n]="50%",d["margin-"+n]=-Math.round(h[e===da?ka:R]/2)+f):(n=O(a,b),w=O(a,b,G.content),g=t(a),d[b]=c?w:f+(g>n?g:-n))}),d[a[e]]-=h[e===ha?ka:R],c.css({top:"",bottom:"",left:"",right:"",margin:""}).css(d),d)},destroy:function(){G.tip&&G.tip.remove();G.tip=!1;J.unbind(".qtip-tip")}});F.init()}function ta(c){function e(){F=b(y,t).not("[disabled]").map(function(){return"function"==typeof this.focus?
this:null})}function g(a){1>F.length&&a.length?a.not("body").blur():F.first().focus()}function l(a){var c=b(a.target),d=c.closest(".qtip");!(1>d.length?k:parseInt(d[0].style.zIndex,10)>parseInt(t[0].style.zIndex,10))&&b(a.target).closest(aa)[0]!==t[0]&&g(c)}var s=this,E=c.options.show.modal,O=c.elements,t=O.tooltip,A=".qtipmodal"+c.id,x=b(a.body),y=C.modal.focusable.join(","),F={},B;c.checks.modal={"^show.modal.(on|blur)$":function(){s.init();O.overlay.toggle(t.is(":visible"))},"^content.text$":function(){e()}};
...
function(a){if(!(a.isDefaultPrevented()||a.target!==t[0])){var c=b(aa).filter("[is-modal-qtip]"),d=C.modal.zindex+c.length,e=parseInt(t[0].style.zIndex,10);B[0].style.zIndex=d-2;c.each(function(){this.style.zIndex>e&&(this.style.zIndex-=1)});c.end().filter("."+qa).qtip("blur",a.originalEvent);t.addClass(qa)[0].style.zIndex=d;try{a.preventDefault()}catch(f){}}}).bind("tooltiphide.qtipmodal",function(a){a.target===t[0]&&b("[is-modal-qtip]").filter(":visible").not(t).last().qtip("focus",a)}),E.escape&&
b(a).unbind(A).bind("keydown"+A,function(a){27===a.keyCode&&t.hasClass(qa)&&c.hide(a)}),E.blur&&O.overlay.unbind(A).bind("click"+A,function(a){t.hasClass(qa)&&c.hide(a)}),e(),s):s},create:function(){function a(){B.css({height:e.height(),width:e.width()})}var c=b("#qtip-overlay"),e=b(d);return c.length?O.overlay=c.insertAfter(b(aa).last()):(B=O.overlay=b("<div />",{id:"qtip-overlay",html:"<div></div>",mousedown:function(){return k}}).hide().insertAfter(b(aa).last()),e.unbind(".qtipmodal").bind("resize.qtipmodal",
a),a(),B)},toggle:function(a,c,d){if(a&&a.isDefaultPrevented())return s;a=E.effect;var e=c?"show":"hide",f=B.is(":visible"),n=b("[is-modal-qtip]").filter(":visible").not(t);return B||(B=s.create()),B.is(":animated")&&f===c&&B.data("toggleState")!==k||!c&&n.length?s:(c?(B.css({left:0,top:0}),B.toggleClass("blurs",E.blur),E.stealfocus!==k&&(x.bind("focusin"+A,l),g(b("body :focus")))):x.unbind("focusin"+A),B.stop(r,k).data("toggleState",c),b.isFunction(a)?a.call(B,c):a===k?B[e]():B.fadeTo(parseInt(d,
10)||90,c?1:0,function(){c||b(this).hide()}),c||B.queue(function(a){B.css({left:"",top:""}).removeData("toggleState");a()}),s)},show:function(a,c){return s.toggle(a,r,c)},hide:function(a,c){return s.toggle(a,k,c)},destroy:function(){var d=B;return d&&(d=1>b("[is-modal-qtip]").not(t).length,d?(O.overlay.remove(),b(a).unbind(".qtipmodal")):O.overlay.unbind(".qtipmodal"+c.id),x.unbind("focusin"+A)),t.removeAttr("is-modal-qtip").unbind(".qtipmodal")}});s.init()}function pa(c){var e=this,g=c.elements,
l=c.options,s=g.tooltip,E=".ie6-"+c.id,O=1>b("select, object").length,t=0,A=k,x;c.checks.ie6={"^content|style$":function(a,c,b){redraw()}};b.extend(e,{init:function(){var c=b(d),w;O&&(g.bgiframe=b('<iframe class="qtip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0); -ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";"></iframe>'),g.bgiframe.appendTo(s),s.bind("tooltipmove"+E,e.adjustBGIFrame));
x=b("<div/>",{id:"qtip-rcontainer"}).appendTo(a.body);e.redraw();g.overlay&&!A&&(w=function(){g.overlay[0].style.top=c.scrollTop()+"px"},c.bind("scroll.qtip-ie6, resize.qtip-ie6",w),w(),g.overlay.addClass("qtipmodal-ie6fix"),A=r)},adjustBGIFrame:function(){var a=c.get("dimensions"),b=c.plugins.tip,d=g.tip,e,n;n=parseInt(s.css("border-left-width"),10)||0;n={left:-n,top:-n};b&&d&&(e="x"===b.corner.precedance?["width","left"]:["height","top"],n[e[1]]-=d[e[0]]());g.bgiframe.css(n).css(a)},redraw:function(){if(1>
c.rendered||t)return e;var a=l.style,b=l.position.container,d,g,k,h;return t=1,a.height&&s.css(R,a.height),a.width?s.css(ka,a.width):(s.css(ka,"").appendTo(x),g=s.width(),1>g%2&&(g+=1),k=s.css("max-width")||"",h=s.css("min-width")||"",d=-1<(k+h).indexOf("%")?b.width()/100:0,k=(-1<k.indexOf("%")?d:1)*parseInt(k,10)||g,h=(-1<h.indexOf("%")?d:1)*parseInt(h,10)||0,g=k+h?Math.min(Math.max(g,h),k):g,s.css(ka,Math.round(g)).appendTo(b)),t=0,e},destroy:function(){O&&g.bgiframe.remove();s.unbind(E)}});e.init()}
var r=!0,k=!1,K=null,ha="x",da="y",ka="width",R="height",V="top",W="left",x="bottom",Y="right",N="center",ra="flipinvert",ea="shift",M,C,ja,ca="qtip",S={},T=["ui-widget","ui-tooltip"],aa="div.qtip."+ca,wa=ca+"-default",qa=ca+"-focus",ya=ca+"-hover",la="oldtitle",ua;M=b.fn.qtip=function(a,c,d){var l=(""+a).toLowerCase(),s=K,E=b.makeArray(arguments).slice(1),x=E[E.length-1],t=this[0]?b.data(this[0],"qtip"):K;if(!arguments.length&&t||"api"===l)return t;if("string"==typeof a)return this.each(function(){var a=
b.data(this,"qtip");if(!a)return r;x&&x.timeStamp&&(a.cache.event=x);if("option"!==l&&"options"!==l||!c)a[l]&&a[l].apply(a[l],E);else{if(!b.isPlainObject(c)&&d===e)return s=a.get(c),k;a.set(c,d)}}),s!==K?s:this;if("object"==typeof a||!arguments.length)return t=g(b.extend(r,{},a)),M.bind.call(this,t,x)};M.bind=function(a,d){return this.each(function(g){function l(a){function c(){A.render("object"==typeof a||s.show.ready);E.show.add(E.hide).unbind(t)}if(A.cache.disabled)return k;A.cache.event=b.extend({},
a);A.cache.target=a?b(a.target):[e];0<s.show.delay?(clearTimeout(A.timers.show),A.timers.show=setTimeout(c,s.show.delay),x.show!==x.hide&&E.hide.bind(x.hide,function(){clearTimeout(A.timers.show)})):c()}var s,E,x,t,A;g=b.isArray(a.id)?a.id[g]:a.id;g=!g||g===k||1>g.length||S[g]?M.nextid++:S[g]=g;t=".qtip-"+g+"-create";A=oa.call(this,g,a);if(A===k)return r;s=A.options;b.each(C,function(){"initialize"===this.initialize&&this(A)});E={show:s.show.target,hide:s.hide.target};x={show:b.trim(""+s.show.event).replace(/ /g,
t+" ")+t,hide:b.trim(""+s.hide.event).replace(/ /g,t+" ")+t};/mouse(over|enter)/i.test(x.show)&&!/mouse(out|leave)/i.test(x.hide)&&(x.hide+=" mouseleave"+t);E.show.bind("mousemove"+t,function(a){c(a);A.cache.onTarget=r});E.show.bind(x.show,l);(s.show.ready||s.prerender)&&l(d)}).attr("data-hasqtip",r)};C=M.plugins={Corner:function(a){a=(""+a).replace(/([A-Z])/," $1").replace(/middle/gi,N).toLowerCase();this.x=(a.match(/left|right/i)||a.match(/center/)||["inherit"])[0].toLowerCase();this.y=(a.match(/top|bottom|center/i)||
["inherit"])[0].toLowerCase();a=a.charAt(0);this.precedance="t"===a||"b"===a?da:ha;this.string=function(){return this.precedance===da?this.y+this.x:this.x+this.y};this.abbrev=function(){var a=this.x.substr(0,1),c=this.y.substr(0,1);return a===c?a:this.precedance===da?c+a:a+c};this.invertx=function(a){this.x=this.x===W?Y:this.x===Y?W:a||this.x};this.inverty=function(a){this.y=this.y===V?x:this.y===x?V:a||this.y};this.clone=function(){return{x:this.x,y:this.y,precedance:this.precedance,string:this.string,
abbrev:this.abbrev,clone:this.clone,invertx:this.invertx,inverty:this.inverty}}},offset:function(c,d){var e=c.offset(),g=c.closest("body"),k=b.browser.msie&&"CSS1Compat"!==a.compatMode,l=d,r,t,x;if(l){do"static"!==l.css("position")&&(t=l.position(),e.left-=t.left+(parseInt(l.css("borderLeftWidth"),10)||0)+(parseInt(l.css("marginLeft"),10)||0),e.top-=t.top+(parseInt(l.css("borderTopWidth"),10)||0)+(parseInt(l.css("marginTop"),10)||0),!r&&"hidden"!==(x=l.css("overflow"))&&"visible"!==x&&(r=l));while((l=
...
b.cleanData,b.cleanData=function(a){for(var c=0,d;(d=a[c])!==e;c++)try{b(d).triggerHandler("removeqtip")}catch(g){}b.cleanData_replacedByqTip(a)});M.version="2.0.1";M.nextid=0;M.inactiveEvents="click dblclick mousedown mouseup mousemove mouseleave mouseenter".split(" ");M.zindex=15E3;M.defaults={prerender:k,id:k,overwrite:r,suppress:r,content:{text:r,attr:"title",deferred:k,title:{text:k,button:k}},position:{my:"top left",at:"bottom right",target:k,container:k,viewport:k,adjust:{x:0,y:0,mouse:r,resize:r,
...
c:a.plugins.ajax=new Z(a)};C.ajax.initialize="render";C.ajax.sanitize=function(a){var c=a.content,b;c&&"ajax"in c&&(b=c.ajax,"object"!=typeof b&&(b=a.content.ajax={url:b}),"boolean"!=typeof b.once&&b.once&&(b.once=!!b.once))};b.extend(r,M.defaults,{content:{ajax:{loading:r,once:r}}});C.tip=function(a){var c=a.plugins.tip;return"object"==typeof c?c:a.plugins.tip=new ba(a)};C.tip.initialize="render";C.tip.sanitize=function(a){var c=a.style,b;c&&"tip"in c&&(b=a.style.tip,"object"!=typeof b&&(a.style.tip=
{corner:b}),/string|boolean/i.test(typeof b.corner)||(b.corner=r),"number"!=typeof b.width&&delete b.width,"number"!=typeof b.height&&delete b.height,"number"!=typeof b.border&&b.border!==r&&delete b.border,"number"!=typeof b.offset&&delete b.offset)};b.extend(r,M.defaults,{style:{tip:{corner:r,mimic:k,width:6,height:6,border:r,offset:0}}});C.modal=function(a){var c=a.plugins.modal;return"object"==typeof c?c:a.plugins.modal=new ta(a)};C.modal.initialize="render";C.modal.sanitize=function(a){a.show&&
("object"!=typeof a.show.modal?a.show.modal={on:!!a.show.modal}:"undefined"==typeof a.show.modal.on&&(a.show.modal.on=r))};C.modal.zindex=M.zindex-200;C.modal.focusable="a[href] area[href] input select textarea button iframe object embed [tabindex] [contenteditable]".split(" ");b.extend(r,M.defaults,{show:{modal:{on:k,effect:r,blur:r,stealfocus:r,escape:r}}});C.viewport=function(c,b,e,g,l,k,r){function t(a,c,d,e,g,l,k,q,m){var r=b[g],u=y[a],L=F[a],ia=d===ea,ga=-D.offset[g]+h.offset[g]+h["scroll"+
g],ma=u===g?m:u===l?-m:-m/2,Aa=f&&f.size?f.size[k]||0:0,t=f&&f.corner&&f.corner.precedance===a&&!ia?Aa:0,s=ga-r+t;m=r+m-h[k]-ga+t;q=ma-(y.precedance===a||u===y[c]?L===g?q:L===l?-q:-q/2:0)-(L===N?q/2:0);return ia?(t=f&&f.corner&&f.corner.precedance===c?Aa:0,q=(u===g?1:-1)*ma-t,b[g]+=0<s?s:0<m?-m:0,b[g]=Math.max(-D.offset[g]+h.offset[g]+(t&&f.corner[a]===N?f.offset:0),r-q,Math.min(Math.max(-D.offset[g]+h.offset[g]+h[k],r+q),b[g]))):(e*=d===ra?2:0,0<s&&(u!==g||0<m)?(b[g]-=q+e,v["invert"+a](g)):0<m&&
(u!==l||0<s)&&(b[g]-=(u===N?-q:q)+e,v["invert"+a](l)),b[g]<ga&&-b[g]>m&&(b[g]=r,v=y.clone())),b[g]-r}var A=e.target,C=c.elements.tooltip,y=e.my,F=e.at,B=e.adjust,G=B.method.split(" "),J=G[0],G=G[1]||G[0],h=e.viewport,D=e.container;e=c.cache;var f=c.plugins.tip,K={left:0,top:0},v,z;if(!h.jquery||A[0]===d||A[0]===a.body||"none"===B.method)return K;A="fixed"===C.css("position");h={elem:h,height:h[(h[0]===d?"h":"outerH")+"eight"](),width:h[(h[0]===d?"w":"outerW")+"idth"](),scrollleft:A?0:h.scrollLeft(),
scrolltop:A?0:h.scrollTop(),offset:h.offset()||{left:0,top:0}};D={elem:D,scrollLeft:D.scrollLeft(),scrollTop:D.scrollTop(),offset:D.offset()||{left:0,top:0}};if("shift"!==J||"shift"!==G)v=y.clone();return K={left:"none"!==J?t(ha,da,J,B.x,W,Y,ka,g,k):0,top:"none"!==G?t(da,ha,G,B.y,V,x,R,l,r):0},v&&e.lastClass!==(z=ca+"-pos-"+v.abbrev())&&C.removeClass(c.cache.lastClass).addClass(c.cache.lastClass=z),K};C.imagemap=function(a,c,d,e){function g(a,c,b){for(var d=0,e=1,l=1,k=0,n=0,r=a.width,s=a.height;0<
r&&0<s&&0<e&&0<l;){r=Math.floor(r/2);s=Math.floor(s/2);b.x===W?e=r:b.x===Y?e=a.width-r:e+=Math.floor(r/2);b.y===V?l=s:b.y===x?l=a.height-s:l+=Math.floor(s/2);for(d=c.length;d--&&!(2>c.length);)k=c[d][0]-a.position.left,n=c[d][1]-a.position.top,(b.x===W&&k>=e||b.x===Y&&k<=e||b.x===N&&(k<e||k>a.width-e)||b.y===V&&n>=l||b.y===x&&n<=l||b.y===N&&(n<l||n>a.height-l))&&c.splice(d,1)}return{left:c[0][0],top:c[0][1]}}c.jquery||(c=b(c));a=a.cache.areas={};var l=(c[0].shape||c.attr("shape")).toLowerCase(),k=
...
y.width/2,top:y.position.top+y.height/2}:(a[d+k]||(y.position=g(y,A.slice(),d),e&&("flip"===e[0]||"flip"===e[1])&&(y.offset=g(y,A.slice(),{x:d.x===W?Y:d.x===Y?W:N,y:d.y===V?x:d.y===x?V:N}),y.offset.left-=y.position.left,y.offset.top-=y.position.top),a[d+k]=y),y=a[d+k]),y.width=y.height=0}return y.position.left+=C.left,y.position.top+=C.top,y};C.ie6=function(a){var c=b.browser,d=a.plugins.ie6;return!c.msie||"6"!==(""+c.version).charAt(0)?k:"object"==typeof d?d:a.plugins.ie6=new pa(a)};C.ie6.initialize=
...
data:{product:f.get("qr")?"shiftwatch-qr":"stopwatch",host:f.get("host"),sessionID:f.get("sessionID"),user:f.get("username"),domain:f.get("domain")},type:"GET",dataType:"json"};x(c);c=jQuery.ajax(c);c.done(function(c,b,d){!1!=c.success&&(f.set("auth_key",c.auth_key),f.set("access",c.access),f.set("is_auth",c.is_auth),f.set("license_expiry_days",c.license_expiry_days),f.get("qr")&&(f.set("view","team"),f.set("sort","project"),f.set("logout_on_select",!0),f.set("multi",!0),f.set("hide_team_done",!0),
f.set("minuteBuffer",30),f.set("minuteBufferSnapToSched",!0),TASK_DB.userId=f.get("userID")),f.get("minuteBufferSnapToSched")&&E(),f.get("restrict_sched")?E(function(c){var b;void 0==c&&(b="Today is not a work day");var d=new Date,d=Date.parse("01/01/2011 "+d.getHours()+":"+d.getMinutes()),L=Date.parse("01/01/2011 "+c.start),e=Date.parse("01/01/2011 "+c.stop),ia=f.get("minuteBuffer");null!=ia&&(L.setMinutes(L.getMinutes()-ia),e.setMinutes(e.getMinutes()+ia));void 0==c||L>d||d>e?(b=b?b:"Please return at "+
c.start,TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Not scheduled to work</div></h3><div class="content"><p>'+b+'.</p><a href="#" class="btn btn-primary logout cancel" title="Logout">Logout</a></div>')):a()}):(a&&a(),sa()))});c.fail(function(a,c,b){})}}function e(){var a={url:T()+"user/count",type:"get",data:{},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uCount,"+JSON.stringify(a.error)),
m.hide(),q.hide()):a=a.data});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},e,"uCount")})}function b(){var a={url:T()+"user/search?ID=$$USER.ID&fields=customer:name,customer:fullUsers,customer:limitedUsers,customer:teamUsers",type:"get",data:{},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"uInfo,"+JSON.stringify(a.error)),m.hide(),q.hide()):a=a.data});a.fail(function(a,c,u){S({response:a.responseText,type:c,
msg:u},b,"uInfo")})}function c(a,c,b){}function g(){if(fa||v||void 0!=f.get("host")&&"full"==getQueryString("view"))window.clearInterval(za),za=window.setInterval(function(){if(navigator.onLine&&(v||!document.hasFocus()))jQuery("#search-bar").is(":visible")&&jQuery("#search").click(),jQuery("#form-list").hide(),void 0==f.get("sessionID")?pa():l()},1E3*(Ba+120))}function l(){var a=new Date,c=!1;f.get("date")!=a.getDay()&&(jQuery("#form-list").empty(),f.set("notified",!1),TASK_DB.dropTaskTable(),TASK_DB.createDB(),
jQuery("#todayDate").html("Date: "+V(a.getMonth())+" "+a.getDate()),f.set("date",a.getDay()),c=!0);ea();return c}function oa(){var a=f.get("view");void 0==a?aa("task",function(){Z()}):"current"==a?aa("task",function(){jQuery("#button-current").click()}):"requested"==a?aa("task",function(){jQuery("#button-requested").click()}):"all"==a?jQuery("#button-all").click():"team"==a?jQuery("#button-team").click():"role"==a?jQuery("#button-role").click():"recent"==a&&jQuery("#button-recent").click()}function Z(){jQuery("#button-current").click()}
function sa(){if(""==f.get("access")&&!f.get("notified")){f.set("notified",!0);var a='<h3><div style="text-align: center; font-weight: bold;">Expired for '+f.get("host")+'</div></h3><div class="content"><div><p>To keep the ease of tracking your time, see due dates, and completing your tasks, please visit our site below to see licencing options for your company or to arrange an extended demo. If seeing this for the first time, another employee at your company has used this service 30 days ago, please contact info@atappstore.com to extend.</p></div><a href="http://store.atappstore.com/index.php/portfolio/stopwatch/" target="_blank"><img src="icons/AtAppStore.png" style=""></a><div style="font-size: 10px;">* this notification will only appear once daily.</div></div><div id="close" class="btn btn-primary" style="width:230px"><h3 style="line-height:0px">close</h3></div>';
...
a='<div id="hourTypeWindow" class="window"><h3><div style="text-align: center; font-weight: bold;">Track as Type:</div></h3><div class="content"><div><select id="hourTypeSelector" ></select><div><a href="#" id="done_hourType" class="btn btn-primary" rel="'+jQuery(this).attr("rel")+'" title="Done">Done</a><div></div>';jQuery("#window").after(a);if(fa||"full"==getQueryString("view")||v)jQuery("#popout").replaceWith('<span id="refresh" style="float: right;" title="Manual Refresh"><img src="core/img/refresh.gif"></span>'),
jQuery("html").css("width","100%"),jQuery("#refresh").live("click",function(a){a.preventDefault();jQuery("#form-list").hide();jQuery("#search-bar").is(":visible")&&jQuery("#search").click();M()});a=new Date;a='<div id="top-menu"><div class="btn-group"><a id="filter" title="Filter for other assigned tasks and issues" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">Filters<span class="caret"></span></a><ul class="dropdown-menu"><li><a href="#" id="button-current" title="Tasks and issues you\'re working on" >Current</a></li><li><a href="#" id="button-recent" title="All tasks and issues tracked within 14 days" >Recent</a></li><li><a href="#" id="button-requested" title="Tasks and issues you can start on" >Requested</a></li><li><a href="#" id="button-all" title="All tasks and issues assigned to you" >All Assigned</a></li><li><a href="#" id="button-team" title="All tasks and issues assigned to your Teams" >Team Assigned</a></li><li><a href="#" id="button-role" title="All tasks and issues available to your Roles" >Role Assigned</a></li><li><a href="#" class="logout" id="button-logout" >Login as New User</a></li></ul></div><span id="totalHours" title="Go To Timesheets" class="btn" style="">Today\'s Hours: 0</span><span id="todayDate" class="btn disabled" style="">Date: '+
V(a.getMonth())+" "+a.getDate()+"</span></div>";jQuery("#bar").html(a);jQuery("#totalHours").live("click",function(a){a.preventDefault();a="https://"+f.get("host")+"."+f.get("domain")+"/timesheets/current";v?window.open(a,"_blank","location=yes"):window.open(a);return!1});jQuery("#sort-task").live("click",function(a){a.preventDefault();H.priority="asc";H.project=H.date=H.priority="desc";H.task="asc"==H.task?"desc":"asc";jQuery("div#form-list>div").tsort({order:H.task})});jQuery("#sort-project").live("click",
...
jQuery("#button-recent").removeClass("btn");qa();jQuery(a).toggleClass("btn")};jQuery("#button-current").live("click",function(){H=[];a("#button-current");f.set("view","current");w()});jQuery("#button-requested").live("click",function(){H=[];a("#button-requested");f.set("view","requested");n()});jQuery("#button-all").live("click",function(){H=[];a("#button-all");f.set("view","all");aa()});jQuery("#button-team").live("click",function(){H=[];a("#button-team");f.set("view","team");ya()});jQuery("#button-role").live("click",
function(){H=[];a("#button-role");f.set("view","role");wa()});jQuery("#button-recent").live("click",function(){H=[];a("#button-recent");f.set("view","recent");ua()});jQuery(".logout").live("click",function(){pa()})}function pa(){if(I&&f.get("allowOffline"))f.remove("sessionID"),f.remove("userID"),f.remove("username"),f.remove("password"),f.remove("host"),f.remove("domain");else{var a=f.get("qr");f.removeAll();f.set("qr",a)}for(var c in TASK_CNTRL.intervals)window.clearInterval(TASK_CNTRL.intervals[c]);
K()}function r(a){TASK_DB.loadAll(function(c){var b=c.length,d,e;if(0<b)for(d=0;d<b;d++)e=c[d],(null!=e&&"OPTASK"==e.details.objCode||"TASK"==e.details.objCode)&&jQuery("#item"+e.ID).hide();a()})}function k(a){f.get("ent")?ha(a):N(f.get("username"),f.get("password"),Y(f.get("host")+"."+f.get("domain")),a,function(){K(a)})}function K(a){f.get("qr")?da(a):ka(a)}function ha(a){m.show();jQuery("#loading").hide();window.clearInterval(za);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();
m.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();d(function(){ea(function(){ja();void 0!=a&&a()},function(a){K(a)})})}function da(a){m.show();jQuery("#loading").hide();window.clearInterval(za);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();resetForQR();m.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();jQuery("webcamimg").show();var c=jQuery("#form-qrlogin");jQuery("#form-list").html("");v?c.show():
c.slideDown(400,function(){});jQuery("#button-qrlogin").unbind("click").click(function(c){var b=jQuery("#form-qrlogin");c.preventDefault();q.show();N(f.get("username"),f.get("password"),Y(f.get("host")),function(){ignoreForQR();jQuery("#bar").show();v?b.hide():b.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()},function(){resetForQR()})})}function ka(a){m.show();jQuery("#loading").hide();window.clearInterval(za);TASK_DB.dropTaskTable();TASK_DB.createDB();TASK_CNTRL.toggleRunText();
m.hide();q.hide();jQuery("#top-bar").hide();jQuery("#bar").hide();jQuery(".form").hide();var c=jQuery("#form-login"),b=f.get("domain"),d=f.get("host");c.find(":input[name='username']").focus().val("");c.find(":input[name='host']").val(void 0==d||"undefined"==d?"":"attask-ondemand.com"!=b?d+"."+b:d);c.find(":input[name='password']").val("");jQuery("#form-list").html("");var e=jQuery("#username");v?(c.show(),e.focus()):c.slideDown(400,function(){e.focus()});jQuery("#button-login").unbind("click").click(function(b){b.preventDefault();
q.show();N(jQuery.trim(c.find(":input[name='username']").val()),jQuery.trim(c.find(":input[name='password']").val()),Y(jQuery.trim(c.find(":input[name='host']").val())),function(){jQuery("#bar").show();v?c.hide():c.slideUp();jQuery("#top-bar").show();jQuery("#loading").show();void 0!=a&&a()})})}function R(a,c){var b=new Date;a.dateStr=b.getFullYear()+"-"+(b.getMonth()+1)+"-"+b.getDate();a.duration=c;navigator.onLine?J(a,G):(b=f.get("offlineTasks"),void 0==b&&(b=[]),b.push(a),f.set("offlineTasks",
b),m.hide(),q.hide(),!0==f.get("logout_on_select")&&pa())}function V(a){return"jan. feb. mar. apr. may june jul. aug. sep. oct. nov. dec.".split(" ")[a]}function W(a){return void 0!=a.details.project?a.details.project.name.split(",").join(" ")+","+a.name.split(",").join(" "):a.details}function x(a,c){void 0==c&&(c=1E4);a.timeout=c;fa&&(!v&&!z)&&(a.dataType="jsonp",a.crossDomain=!0,a.jsonp="jsonp",a.cache=!0);fa&&"PUT"==a.type&&(a.data.method="put");var b=f.get("sessionID");f.get("ent")?("PUT"==a.type&&
(a.type="POST"),a.data.apiKey=b):a.data.sessionID=b}function Y(a){if(1<a.split(".").length){var c=a.split("//");return 1<c.length?(c.shift(),c[0]):a}return a+".attask-ondemand.com"}function N(c,b,d,e,l){if(void 0==c||void 0==b||void 0==d)l();else{var h=jQuery(".error");h.html("");var k={url:"https://"+d+"/attask/api/v2.0/login",type:"get",data:{username:c,password:b},dataType:"json"};x(k);I?(k=f.get(c),void 0!=k?(ra(k.sessionID,k.userID,k.username,k.password,d),q.hide(),M(),g(),void 0!=e&&e()):h.html("OFFLINE")):
(k=jQuery.ajax(k),k.done(function(k){void 0!=k.error?(f.get("qr")?h.html("Please Scan Again."):h.html(k.error.message),q.hide(),void 0!==l&&l()):(k=k.data,ra(k.sessionID,k.userID,c,b,d),f.set("userObj",JSON.stringify(k)),q.hide(),a(function(){C();M();g();void 0!=e&&e()}))}),k.fail(function(a,c,b){a.responseText&&""!==a.responseText?-1!=a.responseText.indexOf("<html>")?f.get("qr")?h.html("Please Scan Again."):h.html("Host Invalid: "+d):f.get("qr")?h.html("Please Scan Again."):h.html(JSON.parse(a.responseText).error.message):
f.get("qr")?h.html("Please Scan Again."):h.html("Host Invalid: "+d);q.hide();void 0!==l&&l()}))}}function ra(a,c,b,d,e){f.set("sessionID",a);f.set("userID",c);f.set("username",b);f.set("password",d);e=e.split(".");f.set("host",e[0]);e.shift();f.set("domain",e.join("."));f.get("allowOffline")&&f.set(b,{sessionID:a,userID:c,username:b,password:d})}function ea(c,b){var d=f.get("sessionID");void 0==d||""==d?k(c):(m.show(),d={url:T()+"user/"+f.get("userID"),data:{updates:"{isActive: true}"},type:"PUT",
dataType:"json"},fa&&x(d),d=jQuery.ajax(d),d.done(function(d){void 0!=d.error?(d.error["class"]==h[0]?void 0!=b?b(c):k(c):(f.get("sessionID"),JSON.stringify(d.error)),m.hide(),q.hide()):(m.hide(),q.hide(),void 0==f.get("auth_key")?a(M):M(),void 0!=c&&c())}),d.fail(function(a,d,e){401==a.status?void 0!=b?b(c):k(c):"timeout"==d?ea(c):f.get("sessionID")}))}function M(){m.show();var a=f.get("userID"),b={url:"https://"+f.get("host")+"."+f.get("domain")+"/attask/api-internal/timeSheet/search",type:"get",
data:{startDate:"$$TODAY+1",startDate_Mod:"lte",endDate:"$$TODAY-1",endDate_Mod:"gte",fields:"timesheetProfile:description,timesheetProfile:hourTypes:ID,timesheetProfile:hourTypes:name,timesheetProfile:hourTypes:description,timesheetProfile:hourTypes:isActive",userID:a},dataType:"json"};x(b,35E3);I?f.get("allowOffline")?(a=TASK_DB.userRunningTasks(a),void 0!=a&&(na=!0,TASK_DB.insert(a.name,{syncID:a.syncID,details:a.details},!0))):TASK_CNTRL.displayError("OFFLINE"):(a=jQuery.ajax(b),a.done(function(a){if(void 0!=
a.error)a.error["class"]==h[0]?k(function(){M()}):c(D[0],"addGeneralTasks-done,"+JSON.stringify(a.error));else if(a=a.data,0!=a.length&&void 0!=a[0].timesheetProfile&&void 0!=a[0].timesheetProfile.hourTypes)if(a[0].timesheetProfile.description==Q&&U())jQuery(".holder").html(""),TASK_CNTRL.showWindow('<div class="content"><h3>Please use stopwatch on a desktop computer.</h3></div>');else{var b=f.get("userHostName");if(void 0!=b&&-1==a[0].timesheetProfile.description.indexOf(b)&&!jQuery.urlParam("force_host"))jQuery(".holder").html(""),
TASK_CNTRL.showWindow('<div class="content"><h3>Please use stopwatch on a work computer.</h3></div>');else{if(0!=a[0].timesheetProfile.hourTypes.length){b=0;a=a[0].timesheetProfile.hourTypes;for(var d in a)a.hasOwnProperty(d)&&!0==a[d].isActive&&(b++,TASK_DB.insert(a[d].name,{syncID:a[d].ID,details:{objCode:"GEN",description:null!=a[d].description?"General - "+a[d].description:"'General"}},0))}O();P();oa()}}else f.get("general")?ca():(O(),P(),oa())}),a.fail(function(a,c,d){S({response:a.responseText,
type:c,msg:d,status:a.status},M,"addGeneralTasks-fail"+c+"-"+d+" "+a.responseText+" "+a.status+" "+JSON.stringify(b))}))}function C(){var a=new Date,b={url:T()+"hour/search",type:"get",data:{ownerID:f.get("userID"),entryDate:a.getFullYear()+"-"+(a.getMonth()+1)+"-"+a.getDate()},dataType:"json"};x(b);a=jQuery.ajax(b);a.done(function(a){if(void 0!=a.error)a.error["class"]==h[0]?k(function(){C()}):c(D[0],"getTaskHours-done,"+JSON.stringify(a.error));else if(0!==a.data.length){var b={};a.data.forEach(function(a,
c){b[a.ID]=a.hours});f.set("initial_hours",b)}});a.fail(function(a,c,d){S({response:a.responseText,type:c,msg:d,status:a.status},C,"getTaskHours-fail"+c+"-"+d+" "+a.responseText+" "+a.status+" "+JSON.stringify(b))})}function ja(){var a={url:T()+"HourType/search",type:"get",data:{fields:"ID",description:"Stopwatch",description_Mod:"cicontains"},dataType:"json"};x(a);var b=jQuery.ajax(a);b.done(function(a){if(void 0!=a.error)a.error["class"]==h[0]?k(function(){ja()}):c(D[0],"getStopwatchHourType-done,"+
JSON.stringify(a.error));else if(0!==a.data.length){var b={};a.data.forEach(function(a,c){b[a.name]=a.ID});f.set("sw_hour_types",b)}});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d,status:c.status},ja,"getStopwatchHourType-fail"+b+"-"+d+" "+c.responseText+" "+c.status+" "+JSON.stringify(a))})}function ca(){m.show();var a={url:T()+"HourType/search",type:"get",data:{fields:"ID,name,description",isActive:!0},dataType:"json"};x(a,35E3);var b=jQuery.ajax(a);b.done(function(a){if(void 0!=
a.error)a.error["class"]==h[0]?k(function(){ca()}):c(D[0],"addAllGeneralTasks-done,"+JSON.stringify(a.error));else{a=a.data;var b=0,d;for(d in a)a.hasOwnProperty(d)&&("Task Time"!=a[d].name&&"Issue Time"!=a[d].name)&&(b++,TASK_DB.insert(a[d].name,{syncID:a[d].ID,details:{objCode:"GEN",description:null!=a[d].description?"General - "+a[d].description:"'General"}},0));O();P();oa()}});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d,status:c.status},ca,"addAllGeneralTasks-fail"+b+"-"+d+
" "+c.responseText+" "+c.status+" "+JSON.stringify(a))})}function S(a,b,d,e){if(401==a.status)k(function(){b()});else if("timeout"==a.type)b();else{if(void 0!=a.response&&""!=a.response){var f;f="message"in a.response?a.response.message:JSON.stringify(a.response);TASK_CNTRL.displayError("Workfront Error: "+f+"<br>"+(void 0!=e&&""!=e?e:""))}c(D[0],d+","+a.msg+" "+a.type+" "+JSON.stringify(a.response)+" "+e)}}function T(a){void 0==a&&(a="v7.0");return"https://"+f.get("host")+"."+f.get("domain")+"/attask/api/"+
a+"/"}function aa(a,b,d){a=void 0==a?"task":a;d=void 0==d?!1:d;m.show();var e={url:T()+"assgn/search",type:"get",data:{},dataType:"json"};e.data[a+":canStart"]=!0;e.data[a+":status"]="N";e.data[a+":status_Mod"]="contains";e.data["project:status"]="CUR";e.data.fields=a+":parent,"+a+":priority,"+a+":project, "+a+":project:hourTypes,"+a+":plannedCompletionDate,"+a+":permissions";e.data.status="DN";e.data.status_Mod="ne";e.data.$$LIMIT=2E3;d&&(e.data["project:projectedCompletionDate"]="$$TODAYb-1w",e.data["project:projectedCompletionDate_Mod"]=
"gt");var g=f.get("currentRole");"role"==f.get("view")&&void 0!=g?e.data[a+":roleID"]=g.ID:e.data.assignedToID=f.get("userID");"task"==a&&(e.data.fields+=","+a+":percentComplete");x(e);e=jQuery.ajax(e);e.done(function(e){if(void 0!=e.error)e.error["class"]==h[0]?k(function(){aa(a,b,d)}):c(D[0],"addAllAssignedTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else{e=e.data;var g=!0,l=f.get("view");"all"!=l&&"role"!=l&&(g=!1);var ga=f.get("hour_types");if("opTask"!=a)aa("opTask",b,d),r(function(){var a=
e.length;if(a){for(var c=[],b=0;b<a;b++){var d=e[b].task;if(-1!=d.permissions.actions.indexOf("ADD_HOURS")&&-1!=d.permissions.actions.indexOf("CHANGE_STATUS")){var u=!1,f=d.project.hourTypes.length;if(void 0!=ga&&f)for(var L=0;L<f;L++){if(void 0!=ga[d.project.hourTypes[L].ID]){u=!0;break}}else u=!0;u&&(TASK_DB.insert(d.name,{syncID:d.ID,details:d},g&&b==a-1),c[d.ID]=!0)}}TASK_DB.running(function(a){void 0==c[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}});else{l=e.length;na=g;if(l){for(var n=
[],s=0;s<l;s++){var t=e[s].opTask;if(-1!=t.permissions.actions.indexOf("ADD_HOURS")&&-1!=t.permissions.actions.indexOf("CHANGE_STATUS")){var ma=!1,v=t.project.hourTypes.length;if(void 0!=ga&&v)for(var w=0;w<v;w++){if(void 0!=ga[t.project.hourTypes[w].ID]){ma=!0;break}}else ma=!0;ma&&(TASK_DB.insert(t.name,{syncID:t.ID,details:t},g&&s==l-1),n[t.ID]=!0)}}TASK_DB.running(function(a){void 0==n[a.syncID]&&TASK_CNTRL.toggleTimer(task)},function(a){})}else TASK_CNTRL.showAll(e);b&&b(e)}}});e.fail(function(a,
c,b){S({response:a.responseText,type:c,msg:b},aa,"addAllAssignedTasks-fail"+c+"-"+b+" "+a.responseText)})}function wa(a){m.show();var b={url:T("v5.0")+"user/search",type:"get",data:{fields:"role:*,roles:*",ID:f.get("userID")},dataType:"json"};x(b);b=jQuery.ajax(b);b.done(function(b){if(void 0!=b.error)b.error["class"]==h[0]?k(function(){wa(a)}):c(D[0],"addAllRoleTasks,"+JSON.stringify(b.error)),m.hide(),q.hide();else if(b=b.data[0],0==b.length)a&&a(),m.hide(),q.hide();else{var d=jQuery("#role-filter"),
e=function(){aa("task",function(){d.css("display","inline-block");m.hide();q.hide()})},g=function(a,c){f.set("currentRole",{name:a,ID:c});d.find("button").html("<strong>Role: </strong><span>"+a+'</span><span class="caret"></span>');e()},L=f.get("currentRole");void 0!=L?g(L.name,L.ID):void 0!=b.role&&g(b.role.name,b.role.ID);if(void 0!=b.roles&&0!=b.roles.length)for(var L=d.find(".dropdown-menu").html("").first(),l=0,n=b.roles.length;l<n;l++)L.append('<li><a href="#" id="'+b.roles[l].ID+'">'+b.roles[l].name+
"</a></li>"),L.find("#"+b.roles[l].ID).click(function(){g(jQuery(this).text(),jQuery(this).attr("id"))});else qa(),m.hide(),q.hide(),Z(),TASK_CNTRL.showWindow('<h3><div style="text-align: center; font-weight: bold;">Roles</div></h3><div class="content"><div><p>No additional roles available.</strong></h3><div><a href="#" class="btn btn-primary cancel" title="Ok">Ok</a><div>')}});b.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},wa,"addAllRoleTasks")})}function qa(){var a=jQuery("#role-filter");
a.hide();a.find(".dropdown-menu").first().html("")}function ya(a){m.show();var b={url:T()+"team/search",type:"get",data:{fields:"ID","users:ID":f.get("userID")},dataType:"json"};x(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){ya(a)}):c(D[0],"addAllGroupTasks,"+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,0==b.length?(a?a():jQuery("#form-list").hide(),m.hide(),q.hide(),TASK_CNTRL.showNoTaskWindow()):r(function(){for(var c=b.length,d=function(b,
d){if(0==--c){var e=va.length;na=!0;if(0==e)TASK_CNTRL.showAll(va);else{for(var f=0;f<e;f++)TASK_DB.insert(va[f].name,{syncID:va[f].ID,details:va[f]},f==e-1);va=[];a&&a()}}},e=0;e<c;e++)la("task",b[e].ID,d)}))});b.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ya,"addAllGroupTasks")})}function la(a,b,d){a=void 0==a?"task":a;m.show();var e={url:T()+a+"/search?project:status=INF",type:"get",data:{canStart:!0,status:"N",status_Mod:"contains","project:status":"CUR",fields:"parent,priority,project,plannedCompletionDate",
$$LIMIT:1E3,"team:ID":b},dataType:"json"};"task"==a&&(e.data.fields+=",percentComplete");x(e);e=jQuery.ajax(e);e.done(function(e){if(void 0!=e.error)e.error["class"]==h[0]?k(function(){la(a,b,d)}):c(D[0],"addGroupTasks-"+a+","+JSON.stringify(e.error)),m.hide(),q.hide();else if(e=e.data,0==e.length)d&&d(a,b);else if("opTask"!=a){la("opTask",b,d);for(var f=e.length,g=0;g<f;g++)va.push(e[g])}else{f=e.length;for(g=0;g<f;g++)va.push(e[g]);d&&d(a,b)}});e.fail(function(a,c,b){S({response:a.responseText,
type:c,msg:b},la,"addGroupTasks-fail"+c+"-"+b+" "+a.responseText)})}function ua(){m.show();var a={url:T()+"hour/report",type:"get",data:{"task:canStart":!0,taskID_1_GroupBy:!0,entryDate:"$$TODAY-14d",entryDate_Range:"$$TODAY",entryDate_Mod:"between"},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){ua()}):c(D[0],"addRecentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=Object.keys(a.data),0==a.length?(na=!0,r(function(){TASK_CNTRL.showAll(a)})):
r(function(){na=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))}))});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},ua,"addRecentTasks")})}function w(){m.show();var a;f.get("ent")?aa("task",function(a){na=!0;TASK_CNTRL.showAll(TASK_CNTRL.taskDB.loadSyncIds(a))},!0):(a={url:T()+"work/myWork",type:"get",data:{fields:"parent,priority,project,plannedCompletionDate"},dataType:"json"},x(a),a=jQuery.ajax(a),a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){w()}):
c(D[0],"addCurrentTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,0==a.length?(na=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){var c=a.length;na=!0;for(var b=0;b<c;b++)TASK_DB.insert(a[b].name,{syncID:a[b].ID,details:a[b]},b==c-1)}))}),a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},w,"addCurrentTasks")}))}function n(){m.show();var a={url:T()+"work/workRequests",type:"get",data:{canStart:!0,fields:"parent,priority,project,plannedCompletionDate"},dataType:"json"};
x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){n()}):c(D[0],"addRequestedTasks,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,0==a.length?(na=!0,r(function(){TASK_CNTRL.showAll(a)})):r(function(){var c=a.length;na=!0;for(var b=0;b<c;b++)TASK_DB.insert(a[b].name,{syncID:a[b].ID,details:a[b]},b==c-1)}))});a.fail(function(a,c,b){S({response:a.responseText,type:c,msg:b},n,"addRequestedTasks")})}function P(){var a={url:T()+"user/search",data:{"hourTypes:isActive":!0,
fields:"hourTypes:ID,hourTypes:name,hourTypes:description,hourTypes:countAsRevenue",ID:f.get("userID")},type:"get",dataType:"json"};x(a);X=!1;a=jQuery.ajax(a);a.done(function(a){if(void 0!=a.error)a.error["class"]==h[0]?k(function(){}):c(D[0],"getHourTypes,"+JSON.stringify(a.error)),m.hide(),q.hide();else if(a=a.data,void 0!=a&&0!=a.length&&0!=a[0].hourTypes.length){for(var b=[],d=0;d<a[0].hourTypes.length;d++)b[a[0].hourTypes[d].ID]=!0;f.set("hour_types",b);X=!0;var e=f.get("sw_hour_types"),u=jQuery("#hourTypeSelector");
u.text("");jQuery.each(a[0].hourTypes,function(){var a=this.ID;void 0!=e&&void 0!=e[this.name]&&(a=e[this.name]);u.append($("<option />").val(a).text(this.name))})}});a.fail(function(a,c,b){401==a.status?k(function(){}):"timeout"==c&&P()})}function U(){var a=!1,c=navigator.userAgent||navigator.vendor||window.opera;if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(c)||
...
4)))a=!0;return a}function s(a,c){1==jQuery("#hourTypeSelector").children().length?(a.details.hourTypeID=jQuery("#hourTypeSelector").children()[0].value,TASK_DB.updateDetails(a.ID,a.details),c(a)):1==a.details.project.hourTypes.length?(a.details.hourTypeID=a.details.project.hourTypes[0].ID,TASK_DB.updateDetails(a.ID,a.details),c(a)):(TASK_CNTRL.showWindow(null,"#hourTypeWindow"),jQuery("#done_hourType").unbind("click").bind("click",function(b){b.preventDefault();a.details.hourTypeID=jQuery("#hourTypeSelector").find(":selected").val();
TASK_DB.updateDetails(a.ID,a.details);c(a);TASK_CNTRL.hideWindow("#hourTypeWindow")}))}function E(a){var b=f.get("todaySched");null!=b?a(b):(b={url:T("v4.0")+"user/search?ID=$$USER.ID&fields=schedule:*,schedule:nonWorkDays:*",type:"get",data:{},dataType:"json"},x(b),b=jQuery.ajax(b),b.done(function(b){if(void 0!=b.error)b.error["class"]==h[0]?k(function(){}):c(D[0],"getSchedule,"+JSON.stringify(b.error)),m.hide(),q.hide();else if(b=b.data[0].schedule,0!=b.length){var d=new Date,e=b["sunday monday tuesday wednesday thursday friday saturday".split(" ")[d.getDay()]],
g;if(!0==b.hasNonWorkDays){for(var l=0;l<b.nonWorkDays.length;l++)if(b.nonWorkDays[l].nonWorkDate==d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate()&&(void 0!=g&&null!=b.nonWorkDays[l].userID||void 0==g))g=b.nonWorkDays[l].scheduleDay;void 0!=g&&(e=g)}void 0!=e&&0!=e.length&&(e={start:e[0],stop:e[1]},f.set("todaySched",e));a&&a(e)}}),b.fail(function(c,b,d){401==c.status?k(function(){}):"timeout"==b&&E(a)}))}function O(){var a=new Date,a={url:T()+"hour/report?hours_AggFunc=sum",type:"get",data:{entryDate:a.getFullYear()+
"-"+(a.getMonth()+1)+"-"+a.getDate(),ownerID:f.get("userID")},dataType:"json"};x(a);a=jQuery.ajax(a);a.done(function(a){void 0!=a.error?(a.error["class"]==h[0]?k(function(){}):c(D[0],"getTodaysTotalHours,"+JSON.stringify(a.error)),m.hide(),q.hide()):(a=a.data,jQuery("#totalHours").html("Today's Hours: "+(null==a.sum_hours?"0":a.sum_hours.toFixed(2))))});a.fail(function(a,c,b){401==a.status?k(function(){}):"timeout"==c&&O()})}function t(a){m.show();var b={assignedToID:f.get("userID")};"OPTASK"==a.details.objCode?
b.opTaskID=a.syncID:b.taskID=a.syncID;b={url:T()+"assignment/search",type:"get",data:b,dataType:"json"};x(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){t(a)}):c(D[0],"getTaskAssignment,"+a.syncID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data[0],a.running?(a.done=1,TASK_CNTRL.stopTask(a),TASK_CNTRL.toggleRunText()):A(b,a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){t(a)},"getTaskAssignment,"+a.syncID)})}function A(a,
b){m.show();var d={url:T()+"assignment/"+a.ID,data:{status:"DN"},type:"PUT",dataType:"json"};fa&&x(d);d=jQuery.ajax(d);d.done(function(d){void 0!=d.error?(d.error["class"]==h[0]?k(function(){A(a,b)}):c(D[0],"setAssignmentDone,"+a.ID+","+JSON.stringify(d.error)),m.hide(),q.hide()):(TASK_DB.remove(b.ID),m.hide(),q.hide(),c(D[2],W(b)+",assignment done"),xa(b))});d.fail(function(c,d,e){S({response:c.responseText,type:d,msg:e},function(){A(a,b)},"setAssignmentDone,"+a.ID+", "+c.responseText)})}function xa(a){m.show();
var b={url:T()+"assignment/count",data:{status:"DN",status_Mod:"ne"},type:"GET",dataType:"json"};"OPTASK"==a.details.objCode?b.data.opTaskID=a.syncID:b.data.taskID=a.syncID;x(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?(b.error["class"]==h[0]?k(function(){xa(a)}):c(D[0],"getTaskDoneAssignmentCount,"+taskID+","+JSON.stringify(b.error)),m.hide(),q.hide()):(b=b.data,m.hide(),q.hide(),0==b.count&&y(a))});b.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},function(){xa(a)},"getTaskDoneAssignmentCount,"+
a.syncID)})}function y(a){var b=a.syncID;m.show();var d="OPTASK"==a.details.objCode?"CLS":"CPL",d={url:T()+a.details.objCode.toLowerCase()+"/"+b,data:{status:d},type:"PUT",dataType:"json"};fa&&x(d);d=jQuery.ajax(d);d.done(function(d){void 0!=d.error?(d.error["class"]==h[0]?k(function(){y(a)}):S({response:d.error,type:null,msg:null},function(){y(a)},"setTaskDone,"+b),m.hide(),q.hide()):(m.hide(),q.hide(),c(D[2],W(a)+",task done"))});d.fail(function(c,d,e){S({response:c.responseText,type:d,msg:e},function(){y(a)},
"setTaskDone,"+b+", "+c.responseText)})}function F(a){m.show();var c={url:T()+a.details.objCode.toLowerCase()+"/"+a.syncID,data:{status:"INP"},type:"PUT",dataType:"json"};fa&&x(c);c=jQuery.ajax(c);c.done(function(c){void 0!=c.error?(c.error["class"]==h[0]?k(function(){F(a)}):S({response:c.error,type:null,msg:null},function(){F(a)},"setTaskInProgress,"+a.syncID),m.hide(),q.hide()):(m.hide(),q.hide(),!0==f.get("logout_on_select")&&pa())});c.fail(function(c,b,d){S({response:c.responseText,type:b,msg:d},
function(){F(a)},"setTaskInProgress-fail,"+a.syncID+", "+c.responseText)})}function B(a,c){m.show();var b={url:T()+a.details.objCode.toLowerCase()+"/"+a.syncID,data:{updates:"{ percentComplete: "+c+"}"},type:"PUT",dataType:"json"};fa&&x(b);b=jQuery.ajax(b);b.done(function(b){void 0!=b.error?b.error["class"]==h[0]?k(function(){B(a,c)}):S({response:b.error,type:null,msg:null},function(){B(a,c)},"setTaskPercent,"+a.syncID):(a.details.percentComplete=c,TASK_DB.updateDetails(a.ID,a.details));m.hide();
...
window.chrome.hasOwnProperty("windows")),v=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://")&&fa,z=-1!==document.URL.indexOf("dashboard.directory"),X=!1,I=!1,Q="Force Desktop Stopwatch Access",Ba=300,va=[];f=!fa||v?new Store("Attask"):{settings:[],set:function(a,c){switch(a){case "domain":case "host":case "userID":case "sessionID":case "username":case "date":case "view":this.setCookie(a,c,9999999999)}this.settings[a]=c},get:function(a){switch(a){case "domain":case "host":case "userID":case "sessionID":case "username":case "date":case "view":var c=
...
window.addEventListener("online",function(a){jQuery("#refresh").show();jQuery(".done").show();jQuery(".percent").show();jQuery(".view").show();a=f.get("offlineTasks");for(var c=a.length,b=0;b<c;b++)R(a[b],a[b].duration);f.set("offlineTasks",[]);I=!1});var za;OB.init.add(function(){void 0==f.get("qr")&&f.set("qr","true"==jQuery.urlParam("qr")||!1!=jQuery.urlParam("code"));ba();ta();g();var a=jQuery.urlParam("s"),c=jQuery.urlParam("d");if(0!=c&&(0!=a&&f.set("sessionID",a),a=c.split("."),f.set("host",
a[0]),a.shift(),f.set("domain",a.join(".")),a=jQuery.urlParam("u"),c=f.get("userID"),void 0==f.get("ent")&&(f.set("userID",a),f.set("password",""),f.set("ent",void 0!=f.get("userID")),a&&c&&a!=c))){ha();return}l();!0==f.get("qr")&&void 0==f.get("sessionID")&&(!1!=jQuery.urlParam("code")?(document.getElementById("button-appscan").style.display="block",qrRead(jQuery.urlParam("code"),f)):qrLoad(f))});OB.onBeforeUnLoad.add(function(a){var c=TASK_CNTRL.getDuration(a);R(a,c.duration)});OB.onUnLoad.add(function(){TASK_DB.running(function(a){f.get("stopOnClose")&&
...
!e&&!0==f.get("logout_on_select")&&pa())})}()});OB.postAllItemRender.add(function(){jQuery(".remove").css("display","none");jQuery(".update").css("display","none");jQuery(".reset").css("display","none");f.get("is_auth")?jQuery("#storelink").text("AtAppStore"):(jQuery("#sort-date").remove(),jQuery(".timer").remove(),jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".date").removeClass("date").addClass("date-tall").html('<a href="http://store.atappstore.com/index.php/portfolio/stopwatch/" target="_blank">Trial Expired</a>'),
jQuery("#storelink").text("Trial Expired - AtAppStore"));f.get("hideTaskButtons")&&(jQuery(".timer").addClass("timer-tall"),jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".task").remove(),jQuery(".view").remove(),jQuery(".date").remove(),jQuery("#sort-date").remove());f.get("ent")&&jQuery("#totalHours").attr("title","").attr("data-hasqtip",!1).unbind("click").addClass("disabled");if(na){var a=jQuery("#form-list"),c=f.get("vpos");void 0!=c&&a.scrollTop(c);"team"==f.get("view")&&!0==f.get("hide_team_done")&&
jQuery(".done").remove();I?f.get("allowOffline")&&(jQuery(".done").remove(),jQuery(".percent").remove(),jQuery(".view").remove(),jQuery("#bar").hide()):jQuery("#bar").show();c=f.get("sort");void 0==c&&(c="date");jQuery("#sort-"+c).click();jQuery("#search-bar").is(":visible")&&jQuery("#search").click();jQuery("#loading").hide();a.show();a=jQuery("#holder")[0];a.style.display="none";a.style.display="block";m.hide();na=!1}});OB.postItemRender.add(function(a){if(0==jQuery("#item"+a.ID+" #buttons .view").length){var c=
...
a.details.objCode?(d="Issue",c.append('<span class="issue" title="Issue">I</span> ')):"TASK"==a.details.objCode&&(d="Task",c.append('<span class="task" title="Task">T</span> '));"General"!=d&&(c.append('<a href="https://'+f.get("host")+"."+f.get("domain")+"/"+d.toLowerCase()+"/view?ID="+a.syncID+'" target="_blank" class="btn btn-mini view" rel="'+a.ID+'" title="View '+d+'">View</a> '),c.append('<a href="#" class="btn btn-mini done" rel="'+a.ID+'" title="Done '+d+'">Done</a> '));"Task"==d&&void 0!=
a.details.percentComplete&&c.append('<a href="#" class="btn btn-mini percent" rel="'+a.ID+'" title="Percent Completed">'+a.details.percentComplete+"%</a> ");c="#D2E0FF";b='<span class="date" style="background-color: #EEE;"><span class="days" data-days="9999"></span></span>';if("General"!=d&&void 0!=a.details.plannedCompletionDate){var b=a.details.plannedCompletionDate.split("T")[0],b=b.split("-"),b=new Date(b[0],Number(b[1])-1,b[2]),e=new Date;e.setHours(0);e.setMinutes(0);e.setSeconds(0);e=Math.round((b-
e)/1E3/60/60/24);0>e?c="#FFCDCD":1>=e&&(c="#FFD7AB");var g;g=0<e?"days due: "+e:"days late: "+Math.abs(e);b='<div class="date" style="background-color: '+c+';"><div class="days" data-days="'+e+'">'+g+"</div><div>"+V(b.getMonth())+" "+b.getDate()+", "+b.getFullYear()+"</div></div>"}c="";null!=a.details.parent&&(c="<br><br>Parent Task: <br>"+a.details.parent.name);jQuery("#item"+a.ID+" .name").attr("title",d+": <br>"+a.name+c);"General"!=d?(d=a.details.project.name,jQuery("#item"+a.ID+" .details").html(d).attr("title",
"Project: <br>"+d).attr("data-priority",a.details.priority),jQuery("#item"+a.ID+" .c6").after(b)):jQuery("#item"+a.ID+" .details").html("~"+a.details.description).attr("title",a.details.description)}});OB.remove.add(function(a){jQuery("#item"+a).remove()});OB.reset.add(function(a,c){OB.update.dispatch(c,-c.time);jQuery("#item"+a+" .timer").text(TASK_CNTRL.hms(0))});var na=!1,q,m,H=[]}();var QUERY_LOOKUP=null;function getQueryString(d){if(null==QUERY_LOOKUP){var a=location.search.substring(1),e=/([^&=]+)=([^&]*)/g,b;for(QUERY_LOOKUP={};b=e.exec(a);)QUERY_LOOKUP[decodeURIComponent(b[1])]=decodeURIComponent(b[2])}return d in QUERY_LOOKUP?QUERY_LOOKUP[d]:null}
...
start:new Date,running:0};if(null!=b.userId){var l=b.ps.get(b.userId+""+a);null!=l&&l.running&&(k=l)}b.ls.set(g,k);l=b.ls.get("list");l[g]=a;b.ls.set("list",l);OB.insert.dispatch(c,{syncID:a,details:d});TASK_CNTRL.showTask(k);e&&OB.postAllItemRender.dispatch()};if("null"!=c){var l=b.ls.get("list"),oa=!1,Z;for(Z in l)if(l.hasOwnProperty(Z)&&null!=l[Z]&&l[Z]==c){oa=!0;break}oa?(c=b.ls.get(Z),c.name=d,g&&Object.assign(c.details,g),b.ls.set(Z,c),TASK_CNTRL.showTask(c),e&&OB.postAllItemRender.dispatch()):
